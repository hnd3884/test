...
public final class Pattern implements Serializable
{
    ...
    public String[] split(final CharSequence charSequence, final int n) {
        ...
        int size = list.size();
        if (n == 0) {
            while (size > 0 && ((String)list.get(size - 1)).equals("")) {
                --size;
            }
        }
        return (String[])list.subList(0, size).toArray(new String[size]);
    }
