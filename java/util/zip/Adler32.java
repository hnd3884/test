...
public class Adler32 implements Checksum
{
    ...
    public void update(final int n) {
        this.adler = update(this.adler, n);
    }
    ...
    public void update(final byte[] array, final int n, final int n2) {
        ...
        this.adler = updateBytes(this.adler, array, n, n2);
    }
    
    public void update(final byte[] array) {
        this.adler = updateBytes(this.adler, array, 0, array.length);
    }
    ...
    public void update(final ByteBuffer byteBuffer) {
        ...
        if (byteBuffer instanceof DirectBuffer) {
            this.adler = updateByteBuffer(this.adler, ((DirectBuffer)byteBuffer).address(), position, n);
        }
        else if (byteBuffer.hasArray()) {
            this.adler = updateBytes(this.adler, byteBuffer.array(), position + byteBuffer.arrayOffset(), n);
        }
        else {
