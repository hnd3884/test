        ...
        return new ReferencePipeline.StatefulOp<T, T>(abstractPipeline, StreamShape.REFERENCE, flags(n2)) {
            ...
            @Override
             <P_IN> Node<T> opEvaluateParallel(final PipelineHelper<T> pipelineHelper, final Spliterator<P_IN> spliterator, final IntFunction<T[]> intFunction) {
                ...
                if (!StreamOpFlag.ORDERED.isKnown(pipelineHelper.getStreamAndOpFlags())) {
                    return Nodes.collect((PipelineHelper<T>)this, (Spliterator<T>)this.unorderedSkipLimitSpliterator(pipelineHelper.wrapSpliterator(spliterator), n, n2, exactOutputSizeIfKnown), true, intFunction);
                }
    ...
    private static final class SliceTask<P_IN, P_OUT> extends AbstractShortCircuitTask<P_IN, P_OUT, Node<P_OUT>, SliceTask<P_IN, P_OUT>>
    {
        ...
        public final void onCompletion(final CountedCompleter<?> countedCompleter) {
            if (!this.isLeaf()) {
                ...
                if (this.canceled) {
                    ...
                    o = this.getEmptyResult();
                }
                else if (this.thisNodeSize == 0L) {
                    o = this.getEmptyResult();
                }
                else if (((SliceTask)this.leftChild).thisNodeSize == 0L) {
                    o = ((SliceTask)this.rightChild).getLocalResult();
                }
                else {
                    o = Nodes.conc(this.op.getOutputShape(), (Node<P_OUT>)((SliceTask)this.leftChild).getLocalResult(), (Node<P_OUT>)((SliceTask)this.rightChild).getLocalResult());
                }
                this.setLocalResult((Node<P_OUT>)(this.isRoot() ? this.doTruncate((Node<P_OUT>)o) : o));
                this.completed = true;
            }
        ...
        private boolean isLeftCompleted(final long n) {
            ...
            while (sliceTask != null) {
                if (sliceTask2 == sliceTask.rightChild) {
                    final SliceTask sliceTask3 = (SliceTask)sliceTask.leftChild;
                    if (sliceTask3 != null) {
                        n2 += sliceTask3.completedSize(n);
                        if (n2 >= n) {
                            return true;
                        }
                    }
                ...
                sliceTask = (SliceTask)sliceTask.getParent();
            }
            return n2 >= n;
        }
