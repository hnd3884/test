...
public class Level implements Serializable
{
    ...
    static final class KnownLevel
    {
        ...
        static synchronized void add(final Level level) {
            ...
            List list = KnownLevel.nameToLevels.get(level.name);
            if (list == null) {
                list = new ArrayList();
                KnownLevel.nameToLevels.put(level.name, list);
            }
            list.add(knownLevel);
            List list2 = KnownLevel.intToLevels.get(level.value);
            if (list2 == null) {
                list2 = new ArrayList();
        ...
        static synchronized KnownLevel findByName(final String s) {
            final List list = KnownLevel.nameToLevels.get(s);
            if (list != null) {
        ...
        static synchronized KnownLevel findByValue(final int n) {
            final List list = KnownLevel.intToLevels.get(n);
            if (list != null) {
                return (KnownLevel)list.get(0);
            }
            return null;
        ...
        static synchronized KnownLevel matches(final Level level) {
            final List list = KnownLevel.nameToLevels.get(level.name);
            if (list != null) {
