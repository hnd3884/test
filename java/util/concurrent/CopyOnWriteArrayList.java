...
public class CopyOnWriteArrayList<E> implements List<E>, RandomAccess, Cloneable, Serializable
{
    ...
    public CopyOnWriteArrayList(final Collection<? extends E> collection) {
        this.lock = new ReentrantLock();
        Object[] array;
        if (((CopyOnWriteArrayList)collection).getClass() == CopyOnWriteArrayList.class) {
            array = ((CopyOnWriteArrayList)collection).getArray();
        }
        else {
            array = collection.toArray();
            if (((CopyOnWriteArrayList)collection).getClass() != ArrayList.class) {
    ...
    public boolean addAll(final Collection<? extends E> collection) {
        final Object[] array = (((CopyOnWriteArrayList)collection).getClass() == CopyOnWriteArrayList.class) ? ((CopyOnWriteArrayList)collection).getArray() : collection.toArray();
        if (array.length == 0) {
            return false;
        }
        final ReentrantLock lock = this.lock;
        ...
        try {
            final Object[] array2 = this.getArray();
            final int length = array2.length;
            if (length == 0 && (((CopyOnWriteArrayList)collection).getClass() == CopyOnWriteArrayList.class || ((CopyOnWriteArrayList)collection).getClass() == ArrayList.class)) {
                this.setArray(array);
            }
            else {
