...
public class ScheduledThreadPoolExecutor extends ThreadPoolExecutor implements ScheduledExecutorService
{
    ...
    public ScheduledFuture<?> scheduleAtFixedRate(final Runnable runnable, final long n, final long n2, final TimeUnit timeUnit) {
        ...
        final ScheduledFutureTask scheduledFutureTask = new ScheduledFutureTask(runnable, null, this.triggerTime(n, timeUnit), timeUnit.toNanos(n2));
        final RunnableScheduledFuture<V> decorateTask = this.decorateTask(runnable, (RunnableScheduledFuture<V>)scheduledFutureTask);
        this.delayedExecute(scheduledFutureTask.outerTask = decorateTask);
        return decorateTask;
    }
    ...
    static class DelayedWorkQueue extends AbstractQueue<Runnable> implements BlockingQueue<Runnable>
    {
        ...
        public boolean offer(final Runnable runnable) {
            ...
            try {
                ...
                this.size = size + 1;
                if (size == 0) {
                    this.setIndex(this.queue[0] = runnableScheduledFuture, 0);
                }
                else {
