...
public abstract class AbstractQueuedLongSynchronizer extends AbstractOwnableSynchronizer implements Serializable
{
    ...
    protected final void setState(final long state) {
        this.state = state;
    }
    
    protected final boolean compareAndSetState(final long n, final long n2) {
        return AbstractQueuedLongSynchronizer.unsafe.compareAndSwapLong(this, AbstractQueuedLongSynchronizer.stateOffset, n, n2);
    }
    ...
    private void setHeadAndPropagate(final Node head, final long n) {
        final Node head2 = this.head;
        this.setHead(head);
        final Node head3;
        if (n > 0L || head2 == null || head2.waitStatus < 0 || (head3 = this.head) == null || head3.waitStatus < 0) {
    ...
    final boolean acquireQueued(final Node head, final long n) {
        boolean b = true;
        try {
            boolean b2 = false;
            Node predecessor;
    ...
    private void doAcquireInterruptibly(final long n) throws InterruptedException {
    ...
    private boolean doAcquireNanos(final long n, long n2) throws InterruptedException {
        if (n2 <= 0L) {
            return false;
        }
    ...
    private void doAcquireShared(final long n) {
    ...
    private void doAcquireSharedInterruptibly(final long n) throws InterruptedException {
        final Node addWaiter = this.addWaiter(Node.SHARED);
        boolean b = true;
        try {
            while (true) {
                final Node predecessor = addWaiter.predecessor();
                if (predecessor == this.head) {
                    final long tryAcquireShared = this.tryAcquireShared(n);
                    if (tryAcquireShared >= 0L) {
                        this.setHeadAndPropagate(addWaiter, tryAcquireShared);
                        predecessor.next = null;
                        b = false;
    ...
    private boolean doAcquireSharedNanos(final long n, long n2) throws InterruptedException {
        if (n2 <= 0L) {
            return false;
        }
        final long n3 = System.nanoTime() + n2;
    ...
    protected boolean tryAcquire(final long n) {
        throw new UnsupportedOperationException();
    }
    
    protected boolean tryRelease(final long n) {
        throw new UnsupportedOperationException();
    }
    ...
    public final void acquireInterruptibly(final long n) throws InterruptedException {
        if (Thread.interrupted()) {
            throw new InterruptedException();
        }
    ...
    public final boolean tryAcquireNanos(final long n, final long n2) throws InterruptedException {
        if (Thread.interrupted()) {
            throw new InterruptedException();
        }
    ...
    public final boolean release(final long n) {
    ...
    public final void acquireShared(final long n) {
        if (this.tryAcquireShared(n) < 0L) {
            this.doAcquireShared(n);
        }
    }
    
    public final void acquireSharedInterruptibly(final long n) throws InterruptedException {
        if (Thread.interrupted()) {
            throw new InterruptedException();
        }
        if (this.tryAcquireShared(n) < 0L) {
    ...
    public final boolean tryAcquireSharedNanos(final long n, final long n2) throws InterruptedException {
        if (Thread.interrupted()) {
            throw new InterruptedException();
        }
        return this.tryAcquireShared(n) >= 0L || this.doAcquireSharedNanos(n, n2);
    }
    
    public final boolean releaseShared(final long n) {
    ...
    final long fullyRelease(final Node node) {
        boolean b = true;
        try {
    ...
    private final boolean compareAndSetHead(final Node node) {
        return AbstractQueuedLongSynchronizer.unsafe.compareAndSwapObject(this, AbstractQueuedLongSynchronizer.headOffset, null, node);
    }
    
    private final boolean compareAndSetTail(final Node node, final Node node2) {
        return AbstractQueuedLongSynchronizer.unsafe.compareAndSwapObject(this, AbstractQueuedLongSynchronizer.tailOffset, node, node2);
    }
    
    private static final boolean compareAndSetWaitStatus(final Node node, final int n, final int n2) {
        return AbstractQueuedLongSynchronizer.unsafe.compareAndSwapInt(node, AbstractQueuedLongSynchronizer.waitStatusOffset, n, n2);
    }
    
    private static final boolean compareAndSetNext(final Node node, final Node node2, final Node node3) {
        return AbstractQueuedLongSynchronizer.unsafe.compareAndSwapObject(node, AbstractQueuedLongSynchronizer.nextOffset, node2, node3);
    }
    
    static {
        unsafe = Unsafe.getUnsafe();
        try {
            stateOffset = AbstractQueuedLongSynchronizer.unsafe.objectFieldOffset(AbstractQueuedLongSynchronizer.class.getDeclaredField("state"));
            headOffset = AbstractQueuedLongSynchronizer.unsafe.objectFieldOffset(AbstractQueuedLongSynchronizer.class.getDeclaredField("head"));
            tailOffset = AbstractQueuedLongSynchronizer.unsafe.objectFieldOffset(AbstractQueuedLongSynchronizer.class.getDeclaredField("tail"));
            waitStatusOffset = AbstractQueuedLongSynchronizer.unsafe.objectFieldOffset(Node.class.getDeclaredField("waitStatus"));
            nextOffset = AbstractQueuedLongSynchronizer.unsafe.objectFieldOffset(Node.class.getDeclaredField("next"));
        }
    ...
    public class ConditionObject implements Condition, Serializable
    {
        ...
        private void doSignal(Node firstWaiter) {
            ...
            } while (!AbstractQueuedLongSynchronizer.this.transferForSignal(firstWaiter) && (firstWaiter = this.firstWaiter) != null);
        }
        ...
        public final void awaitUninterruptibly() {
            ...
            final long fullyRelease = AbstractQueuedLongSynchronizer.this.fullyRelease(addConditionWaiter);
            boolean b = false;
            ...
            if (AbstractQueuedLongSynchronizer.this.acquireQueued(addConditionWaiter, fullyRelease) || b) {
        ...
        public final void await() throws InterruptedException {
            ...
            if (AbstractQueuedLongSynchronizer.this.acquireQueued(addConditionWaiter, fullyRelease) && checkInterruptWhileWaiting != -1) {
        ...
        public final boolean awaitUntil(final Date date) throws InterruptedException {
            ...
            while (!AbstractQueuedLongSynchronizer.this.isOnSyncQueue(addConditionWaiter)) {
                if (System.currentTimeMillis() > time) {
                    transferAfterCancelledWait = AbstractQueuedLongSynchronizer.this.transferAfterCancelledWait(addConditionWaiter);
                    break;
                }
        ...
        final boolean isOwnedBy(final AbstractQueuedLongSynchronizer abstractQueuedLongSynchronizer) {
