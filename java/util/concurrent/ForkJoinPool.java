...
public class ForkJoinPool extends AbstractExecutorService
{
    ...
    final void signalWork(final WorkQueue[] array, final WorkQueue workQueue) {
        ...
        while ((ctl = this.ctl) < 0L) {
            ...
            if ((n = (int)ctl) == 0) {
                ...
                final long n4 = (0xFFFFFFFF00000000L & ctl + 281474976710656L) | (0xFFFFFFFFL & (long)workQueue2.stackPred);
    ...
    private boolean tryRelease(final long n, final WorkQueue workQueue, final long n2) {
        ...
        if (workQueue != null && workQueue.scanState == n3 && ForkJoinPool.U.compareAndSwapLong(this, ForkJoinPool.CTL, n, (0xFFFFFFFF00000000L & n + n2) | (0xFFFFFFFFL & (long)workQueue.stackPred))) {
            ...
            while (true) {
                ...
                if ((n4 = (n4 + 1 & n2)) == n3) {
                    if ((scanState >= 0 || scanState == (scanState = workQueue.scanState)) && n5 == (n5 = n6)) {
                        ...
                        final long ctl2;
                        final long n10 = (0xFFFFFFFFL & (long)n9) | (0xFFFFFFFF00000000L & (ctl2 = this.ctl) - 281474976710656L);
        ...
        while ((scanState = workQueue.scanState) < 0) {
            if (n2 > 0) {
                ...
                if (n4 <= 0 && scanState == (int)ctl) {
                    n5 = ((0xFFFFFFFF00000000L & ctl + 281474976710656L) | (0xFFFFFFFFL & (long)stackPred));
    ...
    final int awaitJoin(final WorkQueue workQueue, final ForkJoinTask<?> forkJoinTask, final long n) {
        ...
        if (forkJoinTask != null && workQueue != null) {
            ...
            final CountedCompleter countedCompleter = (forkJoinTask instanceof CountedCompleter) ? ((CountedCompleter)forkJoinTask) : null;
            while (true) {
    ...
    public <T> ForkJoinTask<T> submit(final Callable<T> callable) {
        final ForkJoinTask.AdaptedCallable adaptedCallable = new ForkJoinTask.AdaptedCallable((Callable<? extends T>)callable);
    ...
    public <T> ForkJoinTask<T> submit(final Runnable runnable, final T t) {
        final ForkJoinTask.AdaptedRunnable adaptedRunnable = new ForkJoinTask.AdaptedRunnable(runnable, (T)t);
    ...
    public <T> List<Future<T>> invokeAll(final Collection<? extends Callable<T>> collection) {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                final ForkJoinTask.AdaptedCallable adaptedCallable = new ForkJoinTask.AdaptedCallable((Callable<?>)iterator.next());
                list.add(adaptedCallable);
                this.externalPush(adaptedCallable);
            }
            for (int i = 0; i < list.size(); ++i) {
                ((ForkJoinTask)list.get(i)).quietlyJoin();
            }
            b = true;
            return list;
        }
            ...
            if (!b) {
                for (int j = 0; j < list.size(); ++j) {
                    ((Future)list.get(j)).cancel(false);
                }
            }
        }
    }
    ...
    private static ForkJoinPool makeCommonPool() {
        ...
        if (forkJoinWorkerThreadFactory == null) {
            if (System.getSecurityManager() == null) {
                forkJoinWorkerThreadFactory = new DefaultCommonPoolForkJoinWorkerThreadFactory();
            }
            else {
        ...
        return new ForkJoinPool(int1, forkJoinWorkerThreadFactory, uncaughtExceptionHandler, 0, "ForkJoinPool.commonPool-worker-");
    }
    ...
    static final class WorkQueue
    {
        ...
        final boolean isEmpty() {
            ...
            final ForkJoinTask<?>[] array;
            final int n2;
            return (n = this.base - (top = this.top)) >= 0 || (n == -1 && ((array = this.array) == null || (n2 = array.length - 1) < 0 || WorkQueue.U.getObject(array, ((n2 & top - 1) << WorkQueue.ASHIFT) + (long)WorkQueue.ABASE) == null));
        }
