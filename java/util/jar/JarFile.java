...
{
    private static final ThreadLocal<Boolean> isInitializing;
    ...
    private byte[] getBytes(final ZipEntry zipEntry) throws IOException {
        try (final InputStream inputStream = super.getInputStream(zipEntry)) {
            ...
            if (n != -1 && allBytes.length != n) {
                throw new EOFException("Expected:" + n + ", read:" + allBytes.length);
            }
            return allBytes;
        }
    }
