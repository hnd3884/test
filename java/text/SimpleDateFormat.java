...
public class SimpleDateFormat extends DateFormat
{
    ...
    public StringBuffer format(final Date date, final StringBuffer sb, final FieldPosition fieldPosition) {
        final int n = 0;
        fieldPosition.endIndex = n;
        fieldPosition.beginIndex = n;
    ...
    private StringBuffer format(final Date time, final StringBuffer sb, final FieldDelegate fieldDelegate) {
        ...
        while (i < this.compiledPattern.length) {
            final int n = this.compiledPattern[i] >>> 8;
            int n2 = this.compiledPattern[i++] & '\u00ff';
            if (n2 == 255) {
                n2 = (this.compiledPattern[i++] << 16 | this.compiledPattern[i++]);
            }
            switch (n) {
                case 100: {
                    sb.append((char)n2);
                ...
                case 101: {
                    sb.append(this.compiledPattern, i, n2);
                    i += n2;
                    continue;
                }
                default: {
                    this.subFormat(n, n2, fieldDelegate, sb, useDateFormatSymbols);
                    continue;
                }
            }
        }
    ...
    public Date parse(final String s, final ParsePosition parsePosition) {
        ...
        while (i < this.compiledPattern.length) {
            ...
            switch (n) {
                case 100: {
                    if (subParse >= length || s.charAt(subParse) != (char)n2) {
                ...
                default: {
                    ...
                    if (i < this.compiledPattern.length) {
                        final int n3 = this.compiledPattern[i] >>> 8;
                        if (n3 != 100 && n3 != 101) {
                            b = true;
                        }
                        if (this.hasFollowingMinusSign && (n3 == 100 || n3 == 101)) {
                            int n4;
                            if (n3 == 100) {
                                n4 = (this.compiledPattern[i] & '\u00ff');
                            }
                            else {
                                n4 = this.compiledPattern[i + 1];
                            }
                            if (n4 == this.minusSign) {
                                b2 = true;
                            }
                        }
                    }
                    subParse = this.subParse(s, subParse, n, n2, b, array, parsePosition, b2, calendarBuilder);
                    if (subParse < 0) {
                        parsePosition.index = index;
                        return null;
                    }
