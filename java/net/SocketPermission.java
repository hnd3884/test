...
public final class SocketPermission extends Permission implements Serializable
{
    ...
    private boolean match(final String s, final String s2) {
        ...
        if (this.cdomain == null) {
            this.cdomain = RegisteredDomain.from(lowerCase).map((Function<? super RegisteredDomain, ?>)RegisteredDomain::name).orElse(lowerCase);
        }
        if (this.hdomain == null) {
            this.hdomain = RegisteredDomain.from(lowerCase2).map((Function<? super RegisteredDomain, ?>)RegisteredDomain::name).orElse(lowerCase2);
        }
