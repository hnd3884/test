...
public class Socket implements Closeable
{
    ...
    public Socket(final Proxy proxy) {
        ...
        if (type == Proxy.Type.SOCKS || type == Proxy.Type.HTTP) {
            ...
            if (((InetSocketAddress)socketAddress).getAddress() != null) {
                this.checkAddress(((InetSocketAddress)socketAddress).getAddress(), "Socket");
            }
            if (securityManager != null) {
                if (((InetSocketAddress)socketAddress).isUnresolved()) {
                    socketAddress = new InetSocketAddress(((InetSocketAddress)socketAddress).getHostName(), ((InetSocketAddress)socketAddress).getPort());
                }
                if (((InetSocketAddress)socketAddress).isUnresolved()) {
                    securityManager.checkConnect(((InetSocketAddress)socketAddress).getHostName(), ((InetSocketAddress)socketAddress).getPort());
                }
                else {
                    securityManager.checkConnect(((InetSocketAddress)socketAddress).getAddress().getHostAddress(), ((InetSocketAddress)socketAddress).getPort());
                }
            }
            ...
            if (Socket.factory == null) {
                (this.impl = new PlainSocketImpl()).setSocket(this);
            }
            else {
                this.setImpl();
            }
    ...
    private Socket(final SocketAddress socketAddress, final SocketAddress socketAddress2, final boolean b) throws IOException {
        ...
        try {
            ...
            try {
                ...
                ((Throwable)ex).addSuppressed(ex2);
            }
            throw ex;
        }
    }
    ...
    private void checkOldImpl() {
        ...
        this.oldImpl = AccessController.doPrivileged((PrivilegedAction<Boolean>)new PrivilegedAction<Boolean>() {
            @Override
            public Boolean run() {
                Class<? extends SocketImpl> clazz = Socket.this.impl.getClass();
                while (true) {
                    try {
                        ...
                        clazz = (Class<? extends SocketImpl>)clazz.getSuperclass();
