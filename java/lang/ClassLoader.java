...
public abstract class ClassLoader
{
    ...
    protected Package[] getPackages() {
        ...
        synchronized (this.packages) {
            hashMap = new HashMap((Map<? extends K, ? extends V>)this.packages);
        }
        Package[] array;
        if (this.parent != null) {
    ...
    static void loadLibrary(final Class<?> clazz, final String s, final boolean b) {
        ...
        assert ClassLoader.sys_paths != null : "should be initialized at this point";
        assert ClassLoader.usr_paths != null : "should be initialized at this point";
        if (!b) {
            if (classLoader != null) {
    ...
    private static boolean loadLibrary0(final Class<?> clazz, final File file) {
        ...
        synchronized (vector) {
            for (int size = vector.size(), i = 0; i < size; ++i) {
                if (s.equals(((NativeLibrary)vector.elementAt(i)).name)) {
                    return true;
                }
            }
