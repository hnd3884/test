...
public final class Security
{
    ...
    private static void initialize() {
        ...
        if ("true".equalsIgnoreCase(Security.props.getProperty("security.overridePropertiesFile"))) {
            ...
            final boolean equals = "true".equals(VM.getSavedProperty("org.openjsse.provider"));
            final boolean equals2 = "true".equals(VM.getSavedProperty("org.legacy8ujsse.provider"));
            if (equals && equals2) {
                throw new ProviderException("Unable to use OpenJSSE and Legacy8uJSSE providers simultaneously");
            }
            if (equals) {
                if (setJSSEProvider("OpenJSSE", "openjsse", "org.openjsse.net.www.protocol", Security.props)) {
                    b = true;
                }
            }
            else if (equals2 && setJSSEProvider("Legacy8uJSSE", "legacy8ujsse", null, Security.props)) {
                b = true;
            }
        }
        if (!b) {
    ...
    private static boolean setJSSEProvider(final String s, final String s2, final String s3, final Properties properties) {
        final File securityPropFile = securityPropFile(s2 + ".security");
        final String separator = File.separator;
        final File file = new File(securityPropFile.getParentFile().getParent(), separator + "ext" + separator + s2 + ".jar");
        boolean b = false;
        if (securityPropFile.exists() && file.exists()) {
            try {
                try (final FileInputStream fileInputStream = new FileInputStream(securityPropFile);
                    ...
                    if (s3 != null) {
                        ...
                        if (property == null || property.isEmpty()) {
                            System.setProperty("java.protocol.handler.pkgs", s3);
                        }
                        else if (!property.contains(s3)) {
                            System.setProperty("java.protocol.handler.pkgs", s3 + "|" + property);
                        }
                    }
                    if (Security.sdebug != null) {
                        Security.sdebug.println("reading " + s2 + " properties file: " + securityPropFile.getName());
                    }
                }
                return b;
            }
            catch (final IOException ex) {
                throw new ProviderException("Unable to load " + s + " provider properties");
            }
            throw new ProviderException(s + " provider requested but not found");
        }
        throw new ProviderException(s + " provider requested but not found");
    }
    ...
    public static Provider[] getProviders(final Map<String, String> map) {
        ...
        HashSet set = new LinkedHashSet(5);
    ...
    public static void setProperty(final String key, final String value) {
        ...
        ((Hashtable<String, String>)Security.props).put(key, value);
        invalidateSMCache(key);
        final SecurityPropertyModificationEvent securityPropertyModificationEvent = new SecurityPropertyModificationEvent();
        securityPropertyModificationEvent.key = key;
        securityPropertyModificationEvent.value = value;
        securityPropertyModificationEvent.commit();
        if (EventHelper.isLoggingSecurity()) {
            EventHelper.logSecurityPropertyEvent(key, value);
        }
    }
    ...
    private static LinkedHashSet<Provider> getProvidersNotUsingCache(final String s, final String s2, final String s3, final String s4, final Provider[] array) {
        final LinkedHashSet set = new LinkedHashSet(5);
