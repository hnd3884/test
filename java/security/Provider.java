...
public abstract class Provider extends Properties
{
    ...
    private void ensureLegacyParsed() {
        ...
        for (final Map.Entry entry : this.legacyStrings.entrySet()) {
    ...
    private void removeInvalidServices(final Map<ServiceKey, Service> map) {
        ...
        while (iterator.hasNext()) {
            if (!((Map.Entry<K, Service>)iterator.next()).getValue().isValid()) {
                iterator.remove();
            }
        }
    }
        ...
        if (s.toLowerCase(Locale.ENGLISH).startsWith("alg.alias.")) {
            ...
            if (index == -1) {
                ...
                if (engineName2.equals("SecureRandom")) {
                    this.updateSecureRandomEntries(true, service2.algorithm);
                }
            }
            else {
    ...
    protected void putService(final Service service) {
        ...
        final Iterator iterator = service.getAliases().iterator();
        while (iterator.hasNext()) {
        ...
        synchronized (this) {
            ...
            if (type.equals("SecureRandom")) {
                this.updateSecureRandomEntries(true, service.algorithm);
            }
        }
    }
    
    private void updateSecureRandomEntries(final boolean b, final String s) {
        Objects.requireNonNull(s);
        if (b) {
            if (this.prngAlgos == null) {
                this.prngAlgos = new LinkedHashSet<String>();
            }
            this.prngAlgos.add(s);
        }
        else {
            this.prngAlgos.remove(s);
        }
        if (Provider.debug != null) {
            Provider.debug.println((b ? "Add" : "Remove") + " SecureRandom algo " + s);
        }
    }
    ...
    synchronized Service getDefaultSecureRandomService() {
        ...
        if (this.prngAlgos != null && !this.prngAlgos.isEmpty()) {
            return this.getService("SecureRandom", this.prngAlgos.iterator().next());
        }
        return null;
    }
    ...
    private void putPropertyStrings(final Service service) {
        ...
        final Iterator iterator = service.getAliases().iterator();
        while (iterator.hasNext()) {
            super.put("Alg.Alias." + type + "." + (String)iterator.next(), algorithm);
        }
        for (final Map.Entry entry : service.attributes.entrySet()) {
    ...
    private void removePropertyStrings(final Service service) {
        ...
        final Iterator iterator = service.getAliases().iterator();
        while (iterator.hasNext()) {
            super.remove("Alg.Alias." + type + "." + (String)iterator.next());
        }
        final Iterator iterator2 = service.attributes.entrySet().iterator();
        while (iterator2.hasNext()) {
            super.remove(type + "." + algorithm + " " + ((Map.Entry<Object, V>)iterator2.next()).getKey());
        }
    }
    ...
    private void implRemoveService(final Service service) {
        ...
        synchronized (this) {
            ...
            if (type.equals("SecureRandom")) {
                this.updateSecureRandomEntries(false, service.algorithm);
            }
        }
    }
                ...
                synchronized (this) {
                    ...
                    if (attribute2 != null) {
                        ...
                        for (int length = array.length, i = 0; i < length; ++i) {
                            ...
                            if (keyClass != null) {
                                list.add((Object)keyClass);
                            }
                        }
                        this.supportedClasses = (Class[])list.toArray((Object[])Service.CLASS0);
                    }
