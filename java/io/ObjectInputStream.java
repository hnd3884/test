...
public class ObjectInputStream extends InputStream implements ObjectInput, ObjectStreamConstants
{
    ...
    private ObjectStreamClass readProxyDesc(final boolean b) throws IOException {
        ...
        if (int1 > 65535) {
            throw new InvalidObjectException("interface limit exceeded: " + int1 + ", limit: " + Caches.PROXY_INTERFACE_LIMIT);
        }
        final String[] array = new String[int1];
        for (int i = 0; i < int1; ++i) {
            array[i] = this.bin.readUTF();
        }
        if (int1 > Caches.PROXY_INTERFACE_LIMIT) {
            throw new InvalidObjectException("interface limit exceeded: " + int1 + ", limit: " + Caches.PROXY_INTERFACE_LIMIT + "; " + Arrays.toString(array));
        }
        ...
        try {
            ...
            final InvalidObjectException ex3 = new InvalidObjectException("Proxy interface limit exceeded: " + Arrays.toString(array));
            ex3.initCause(outOfMemoryError);
            throw ex3;
        }
        ...
        try {
            ...
            final InvalidObjectException ex4 = new InvalidObjectException("Proxy interface limit exceeded: " + Arrays.toString(array));
            ex4.initCause(outOfMemoryError2);
            throw ex4;
        }
        finally {
            --this.depth;
    ...
    private Object readArray(final boolean b) throws IOException {
        ...
        Class<Boolean> componentType = null;
    ...
    private static class Caches
    {
        ...
        private static int privilegedGetIntegerProperty(final String s, final int n) {
            if (System.getSecurityManager() == null) {
        ...
        static {
            ...
            PROXY_INTERFACE_LIMIT = Math.max(0, Math.min(65535, privilegedGetIntegerProperty("jdk.serialProxyInterfaceLimit", 65535)));
        }
    }
    ...
    private class BlockDataInputStream extends InputStream implements DataInput
    {
        ...
        private String readUTFBody(long n) throws IOException {
            ...
            while (n > 0L) {
                final int n3 = this.end - this.pos;
                if (n3 >= 3 || n3 == n) {
                    ...
                    if (n3 > 0) {
                        System.arraycopy(this.buf, this.pos, this.buf, 0, n3);
                    }
                    this.pos = 0;
                    this.end = (int)Math.min(1024L, n);
                    this.in.readFully(this.buf, n3, this.end - n3);
                }
            }
            return sb.toString();
        }
