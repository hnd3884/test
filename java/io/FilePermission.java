...
public final class FilePermission extends Permission implements Serializable
{
    ...
    private void init(final int mask) {
        ...
        if (FilePermission.builtInFS != null) {
            try {
                FilePermission.builtInFS.getPath(new File(this.cpath.endsWith("*") ? (this.cpath.substring(0, this.cpath.length() - 1) + "-") : this.cpath).getPath(), new String[0]);
            }
    ...
    static {
        builtInFS = DefaultFileSystemProvider.create().getFileSystem(URI.create("file:///"));
    }
}
