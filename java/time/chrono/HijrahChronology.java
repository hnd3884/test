...
public final class HijrahChronology extends AbstractChronology implements Serializable
{
    ...
    private static Properties readConfigProperties(final String s) throws Exception {
        try {
            return AccessController.doPrivileged(() -> {
                final File file = new File(System.getProperty("java.home") + File.separator + "lib", s2);
    ...
    private void loadCalendarData() {
        try {
            ...
            String s3 = null;
            int minEpochDay = 0;
            for (final Map.Entry entry : configProperties.entrySet()) {
                final String s5;
                final String s4 = s5 = (String)entry.getKey();
                switch (s5) {
                    case "id": {
                    ...
                    case "iso-start": {
                        final int[] ymd = this.parseYMD((String)entry.getValue());
                    ...
                    default: {
                        try {
                            ...
                            hashMap.put(intValue, this.parseMonths((String)entry.getValue()));
            ...
            PlatformLogger.getLogger("java.time.chrono").severe("Unable to initialize Hijrah calendar proxy: " + this.typeId, ex2);
            throw new DateTimeException("Unable to initialize HijrahCalendar: " + this.typeId, ex2);
        }
    }
