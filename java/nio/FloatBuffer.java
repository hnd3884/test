...
public abstract class FloatBuffer extends Buffer implements Comparable<FloatBuffer>
{
    ...
    public boolean equals(final Object o) {
        ...
        final int position2 = floatBuffer.position();
        ...
        for (int i = limit - 1, n3 = limit2 - 1; i >= position; --i, --n3) {
            if (!equals(this.get(i), floatBuffer.get(n3))) {
                return false;
            }
        }
        return true;
    ...
    public int compareTo(final FloatBuffer floatBuffer) {
        ...
        final int position2 = floatBuffer.position();
        final int n2 = floatBuffer.limit() - position2;
        if (Math.min(n, n2) < 0) {
            return -1;
        }
        for (int n3 = position + Math.min(n, n2), i = position, n4 = position2; i < n3; ++i, ++n4) {
            final int compare = compare(this.get(i), floatBuffer.get(n4));
            if (compare != 0) {
                return compare;
            }
        }
