...
class DirectCharBufferU extends CharBuffer implements DirectBuffer
{
    ...
    DirectCharBufferU(final DirectBuffer att, final int n, final int n2, final int n3, final int n4, final int n5) {
        super(n, n2, n3, n4);
    ...
    public CharBuffer slice() {
        ...
        assert n2 >= 0;
        return new DirectCharBufferU(this, -1, 0, n, n, n2);
    }
    ...
    @Override
    public char get() {
        return DirectCharBufferU.unsafe.getChar(this.ix(this.nextGetIndex()));
    }
    
    @Override
    public char get(final int n) {
        return DirectCharBufferU.unsafe.getChar(this.ix(this.checkIndex(n)));
    }
    
    @Override
    char getUnchecked(final int n) {
        return DirectCharBufferU.unsafe.getChar(this.ix(n));
    }
    ...
    public CharBuffer get(final char[] array, final int n, final int n2) {
        if ((long)n2 << 1 > 6L) {
            ...
            if (this.order() != ByteOrder.nativeOrder()) {
                ...
                Bits.copyToArray(this.ix(position), array, DirectCharBufferU.arrayBaseOffset, (long)n << 1, (long)n2 << 1);
            }
    ...
    public CharBuffer put(final char c) {
        DirectCharBufferU.unsafe.putChar(this.ix(this.nextPutIndex()), c);
        return this;
    }
    
    @Override
    public CharBuffer put(final int n, final char c) {
        DirectCharBufferU.unsafe.putChar(this.ix(this.checkIndex(n)), c);
        return this;
    }
    ...
    public CharBuffer put(final CharBuffer charBuffer) {
        if (charBuffer instanceof DirectCharBufferU) {
            ...
            final DirectCharBufferU directCharBufferU = (DirectCharBufferU)charBuffer;
            final int position = directCharBufferU.position();
            final int limit = directCharBufferU.limit();
            assert position <= limit;
            final int n = (position <= limit) ? (limit - position) : 0;
            ...
            DirectCharBufferU.unsafe.copyMemory(directCharBufferU.ix(position), this.ix(position2), (long)n << 1);
            directCharBufferU.position(position + n);
    ...
    public CharBuffer put(final char[] array, final int n, final int n2) {
        if ((long)n2 << 1 > 6L) {
            ...
            if (this.order() != ByteOrder.nativeOrder()) {
                ...
                Bits.copyFromArray(array, DirectCharBufferU.arrayBaseOffset, (long)n << 1, this.ix(position), (long)n2 << 1);
            }
            this.position(position + n2);
        }
        else {
    ...
    public CharBuffer compact() {
        ...
        assert position <= limit;
        final int n = (position <= limit) ? (limit - position) : 0;
        DirectCharBufferU.unsafe.copyMemory(this.ix(position), this.ix(0), (long)n << 1);
        this.position(n);
    ...
    public CharBuffer subSequence(final int n, final int n2) {
        ...
        return new DirectCharBufferU(this, -1, n3 + n, n3 + n2, this.capacity(), this.offset);
    }
    ...
    static {
        unsafe = Bits.unsafe();
        arrayBaseOffset = DirectCharBufferU.unsafe.arrayBaseOffset(char[].class);
