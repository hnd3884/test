...
class DirectLongBufferS extends LongBuffer implements DirectBuffer
{
    ...
    DirectLongBufferS(final DirectBuffer att, final int n, final int n2, final int n3, final int n4, final int n5) {
        super(n, n2, n3, n4);
    ...
    public LongBuffer slice() {
        ...
        assert n2 >= 0;
        return new DirectLongBufferS(this, -1, 0, n, n, n2);
    }
    ...
    @Override
    public long get() {
        return Bits.swap(DirectLongBufferS.unsafe.getLong(this.ix(this.nextGetIndex())));
    }
    
    @Override
    public long get(final int n) {
        return Bits.swap(DirectLongBufferS.unsafe.getLong(this.ix(this.checkIndex(n))));
    }
    ...
    public LongBuffer get(final long[] array, final int n, final int n2) {
        if ((long)n2 << 3 > 6L) {
            ...
            if (this.order() != ByteOrder.nativeOrder()) {
                ...
                Bits.copyToArray(this.ix(position), array, DirectLongBufferS.arrayBaseOffset, (long)n << 3, (long)n2 << 3);
            }
    ...
    public LongBuffer put(final long n) {
        DirectLongBufferS.unsafe.putLong(this.ix(this.nextPutIndex()), Bits.swap(n));
        return this;
    }
    
    @Override
    public LongBuffer put(final int n, final long n2) {
        DirectLongBufferS.unsafe.putLong(this.ix(this.checkIndex(n)), Bits.swap(n2));
        return this;
    }
    ...
    public LongBuffer put(final LongBuffer longBuffer) {
        if (longBuffer instanceof DirectLongBufferS) {
            ...
            final DirectLongBufferS directLongBufferS = (DirectLongBufferS)longBuffer;
            final int position = directLongBufferS.position();
            final int limit = directLongBufferS.limit();
            assert position <= limit;
            final int n = (position <= limit) ? (limit - position) : 0;
            ...
            DirectLongBufferS.unsafe.copyMemory(directLongBufferS.ix(position), this.ix(position2), (long)n << 3);
            directLongBufferS.position(position + n);
    ...
    public LongBuffer put(final long[] array, final int n, final int n2) {
        if ((long)n2 << 3 > 6L) {
            ...
            if (this.order() != ByteOrder.nativeOrder()) {
                ...
                Bits.copyFromArray(array, DirectLongBufferS.arrayBaseOffset, (long)n << 3, this.ix(position), (long)n2 << 3);
            }
            this.position(position + n2);
        }
        else {
    ...
    public LongBuffer compact() {
        ...
        assert position <= limit;
        final int n = (position <= limit) ? (limit - position) : 0;
        DirectLongBufferS.unsafe.copyMemory(this.ix(position), this.ix(0), (long)n << 3);
        this.position(n);
    ...
    static {
        unsafe = Bits.unsafe();
        arrayBaseOffset = DirectLongBufferS.unsafe.arrayBaseOffset(long[].class);
