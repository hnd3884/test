...
public abstract class AbstractSelectableChannel extends SelectableChannel
{
    ...
    @Override
    public final SelectableChannel configureBlocking(final boolean b) throws IOException {
        synchronized (this.regLock) {
            if (!this.isOpen()) {
                throw new ClosedChannelException();
            }
            if (b != !this.nonBlocking) {
                if (b && this.haveValidKeys()) {
                    throw new IllegalBlockingModeException();
                }
                this.implConfigureBlocking(b);
                this.nonBlocking = !b;
            }
        }
        return this;
