...
class DirectFloatBufferS extends FloatBuffer implements DirectBuffer
{
    ...
    DirectFloatBufferS(final DirectBuffer att, final int n, final int n2, final int n3, final int n4, final int n5) {
        super(n, n2, n3, n4);
    ...
    public FloatBuffer slice() {
        ...
        assert n2 >= 0;
        return new DirectFloatBufferS(this, -1, 0, n, n, n2);
    }
    ...
    @Override
    public float get() {
        return Float.intBitsToFloat(Bits.swap(DirectFloatBufferS.unsafe.getInt(this.ix(this.nextGetIndex()))));
    }
    
    @Override
    public float get(final int n) {
        return Float.intBitsToFloat(Bits.swap(DirectFloatBufferS.unsafe.getInt(this.ix(this.checkIndex(n)))));
    }
    ...
    public FloatBuffer get(final float[] array, final int n, final int n2) {
        if ((long)n2 << 2 > 6L) {
            ...
            if (this.order() != ByteOrder.nativeOrder()) {
                ...
                Bits.copyToArray(this.ix(position), array, DirectFloatBufferS.arrayBaseOffset, (long)n << 2, (long)n2 << 2);
            }
    ...
    public FloatBuffer put(final float n) {
        DirectFloatBufferS.unsafe.putInt(this.ix(this.nextPutIndex()), Bits.swap(Float.floatToRawIntBits(n)));
        return this;
    }
    
    @Override
    public FloatBuffer put(final int n, final float n2) {
        DirectFloatBufferS.unsafe.putInt(this.ix(this.checkIndex(n)), Bits.swap(Float.floatToRawIntBits(n2)));
        return this;
    }
    ...
    public FloatBuffer put(final FloatBuffer floatBuffer) {
        if (floatBuffer instanceof DirectFloatBufferS) {
            ...
            final DirectFloatBufferS directFloatBufferS = (DirectFloatBufferS)floatBuffer;
            final int position = directFloatBufferS.position();
            final int limit = directFloatBufferS.limit();
            assert position <= limit;
            final int n = (position <= limit) ? (limit - position) : 0;
            ...
            DirectFloatBufferS.unsafe.copyMemory(directFloatBufferS.ix(position), this.ix(position2), (long)n << 2);
            directFloatBufferS.position(position + n);
    ...
    public FloatBuffer put(final float[] array, final int n, final int n2) {
        if ((long)n2 << 2 > 6L) {
            ...
            if (this.order() != ByteOrder.nativeOrder()) {
                ...
                Bits.copyFromArray(array, DirectFloatBufferS.arrayBaseOffset, (long)n << 2, this.ix(position), (long)n2 << 2);
            }
            this.position(position + n2);
        }
        else {
    ...
    public FloatBuffer compact() {
        ...
        assert position <= limit;
        final int n = (position <= limit) ? (limit - position) : 0;
        DirectFloatBufferS.unsafe.copyMemory(this.ix(position), this.ix(0), (long)n << 2);
        this.position(n);
    ...
    static {
        unsafe = Bits.unsafe();
        arrayBaseOffset = DirectFloatBufferS.unsafe.arrayBaseOffset(float[].class);
