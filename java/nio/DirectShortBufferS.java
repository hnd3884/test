...
class DirectShortBufferS extends ShortBuffer implements DirectBuffer
{
    ...
    DirectShortBufferS(final DirectBuffer att, final int n, final int n2, final int n3, final int n4, final int n5) {
        super(n, n2, n3, n4);
    ...
    public ShortBuffer slice() {
        ...
        assert n2 >= 0;
        return new DirectShortBufferS(this, -1, 0, n, n, n2);
    }
    ...
    @Override
    public short get() {
        return Bits.swap(DirectShortBufferS.unsafe.getShort(this.ix(this.nextGetIndex())));
    }
    
    @Override
    public short get(final int n) {
        return Bits.swap(DirectShortBufferS.unsafe.getShort(this.ix(this.checkIndex(n))));
    }
    ...
    public ShortBuffer get(final short[] array, final int n, final int n2) {
        if ((long)n2 << 1 > 6L) {
            ...
            if (this.order() != ByteOrder.nativeOrder()) {
                ...
                Bits.copyToArray(this.ix(position), array, DirectShortBufferS.arrayBaseOffset, (long)n << 1, (long)n2 << 1);
            }
    ...
    public ShortBuffer put(final short n) {
        DirectShortBufferS.unsafe.putShort(this.ix(this.nextPutIndex()), Bits.swap(n));
        return this;
    }
    
    @Override
    public ShortBuffer put(final int n, final short n2) {
        DirectShortBufferS.unsafe.putShort(this.ix(this.checkIndex(n)), Bits.swap(n2));
        return this;
    }
    ...
    public ShortBuffer put(final ShortBuffer shortBuffer) {
        if (shortBuffer instanceof DirectShortBufferS) {
            ...
            final DirectShortBufferS directShortBufferS = (DirectShortBufferS)shortBuffer;
            final int position = directShortBufferS.position();
            final int limit = directShortBufferS.limit();
            assert position <= limit;
            final int n = (position <= limit) ? (limit - position) : 0;
            ...
            DirectShortBufferS.unsafe.copyMemory(directShortBufferS.ix(position), this.ix(position2), (long)n << 1);
            directShortBufferS.position(position + n);
    ...
    public ShortBuffer put(final short[] array, final int n, final int n2) {
        if ((long)n2 << 1 > 6L) {
            ...
            if (this.order() != ByteOrder.nativeOrder()) {
                ...
                Bits.copyFromArray(array, DirectShortBufferS.arrayBaseOffset, (long)n << 1, this.ix(position), (long)n2 << 1);
            }
            this.position(position + n2);
        }
        else {
    ...
    public ShortBuffer compact() {
        ...
        assert position <= limit;
        final int n = (position <= limit) ? (limit - position) : 0;
        DirectShortBufferS.unsafe.copyMemory(this.ix(position), this.ix(0), (long)n << 1);
        this.position(n);
    ...
    static {
        unsafe = Bits.unsafe();
        arrayBaseOffset = DirectShortBufferS.unsafe.arrayBaseOffset(short[].class);
