...
public class BinaryClient extends Connection
{
    ...
    public void setDb(final long db) {
        this.db = db;
    }
    ...
    public void spop(final byte[] key, final long count) {
        this.sendCommand(Protocol.Command.SPOP, new byte[][] { key, Protocol.toByteArray(count) });
    }
    ...
    public void zrevrangeByLex(final byte[] key, final byte[] max, final byte[] min) {
        this.sendCommand(Protocol.Command.ZREVRANGEBYLEX, new byte[][] { key, max, min });
    }
    
    public void zrevrangeByLex(final byte[] key, final byte[] max, final byte[] min, final int offset, final int count) {
        this.sendCommand(Protocol.Command.ZREVRANGEBYLEX, new byte[][] { key, max, min, Protocol.Keyword.LIMIT.raw, Protocol.toByteArray(offset), Protocol.toByteArray(count) });
    }
