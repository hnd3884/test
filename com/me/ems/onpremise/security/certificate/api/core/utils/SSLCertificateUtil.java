...
import com.me.ems.onpremise.security.certificate.api.model.CertificatePropertiesDAO;
...
public class SSLCertificateUtil
{
    ...
    public Set<String> getSSLCertificateHostNames(final X509Certificate certificate) throws Exception {
        ...
        Logger.getLogger(com.me.devicemanagement.framework.server.certificate.SSLCertificateUtil.class.getName()).log(Level.INFO, "MAP " + certificateInfo);
        return this.getSSLCertificateHostNamesFromCertDetails(certificateInfo);
    }
    ...
    public String getWebServerName() throws Exception {
        ...
        final Properties properties = WebServerUtil.getWebServerSettings();
        final String webservernameprops = "webserver.name";
        return properties.getProperty(webservernameprops);
    }
    ...
    public Map getCertificateDetails(final String certificateFilePath) {
        ...
        try {
            ...
            this.logger.log(Level.WARNING, "Exception while executing getCertificateDetails method... {0}", exp2);
            if (fis != null) {
                try {
                    fis.close();
                }
                catch (final IOException ex2) {
                    this.logger.log(Level.WARNING, "Exception while closing FileInputStream... {0}", ex2);
                }
            }
        }
        finally {
            if (fis != null) {
                try {
                    ...
                    this.logger.log(Level.WARNING, "Exception while closing FileInputStream... {0}", ex3);
                }
            }
        }
        return certificateDetails;
    ...
    private static byte[] readFileContentAsArray(final String fileName) throws Exception {
        ...
        try {
            ...
            Logger.getLogger("ImportCertificateLogger").log(Level.WARNING, "Exception occurred while reading file", e);
            throw e;
        }
        finally {
            if (fis != null) {
                try {
                    ...
                    Logger.getLogger("ImportCertificateLogger").log(Level.WARNING, "Exception occurred while closing file output stream", e2);
                }
            }
        }
        return content;
    ...
    public static X509Certificate getCertificate(final String certFilePath) throws CertificateParsingException, CertificateException {
        ...
        try {
            ...
            Logger.getLogger("ImportCertificateLogger").log(Level.WARNING, "Exception in BouncyCastlePayloadSigning:getCertificate : {0}", exp3);
        }
        return certificate;
    }
    ...
    private void writeFile(final String fileName, final byte[] content) throws IOException {
        ...
        try {
            ...
            Logger.getLogger("ImportCertificateLogger").log(Level.WARNING, "Exception occurred while writing file", e);
            throw e;
        }
        finally {
            if (fos != null) {
    ...
    private boolean copyFile(final File srcFile, final File destFile) throws Exception {
        ...
        Logger.getLogger("ImportCertificateLogger").log(Level.INFO, "Going to copy file.......");
        try {
            ...
            Logger.getLogger("ImportCertificateLogger").log(Level.WARNING, "Exception while copying file.......", e);
        }
        finally {
            if (inFile != null) {
                inFile.close();
    ...
    public void checkUnWantedSpaceinServerCertificate(final String path) throws FileNotFoundException, IOException {
        try {
            Logger.getLogger("ImportCertificateLogger").log(Level.INFO, "Going to check any unwanted space occurred in server certificate");
            ...
            try {
                ...
                Logger.getLogger("ImportCertificateLogger").log(Level.WARNING, "Exception occurred while closing connections for read server certificate", ex);
            }
            if (isSpaceinLine) {
                Logger.getLogger("ImportCertificateLogger").log(Level.INFO, "Going to remove unwanted space in server certificate");
                FileWriter fw = null;
                ...
                try {
                    ...
                    Logger.getLogger("ImportCertificateLogger").log(Level.WARNING, "Exception occurred while closing connection for writing server certificate", ex2);
                }
            }
            else {
                Logger.getLogger("ImportCertificateLogger").log(Level.INFO, "Server certificate was not having any extra space.");
            }
        }
        catch (final Exception ex3) {
            Logger.getLogger("ImportCertificateLogger").log(Level.WARNING, "Exception occurred while removing unwanted space from server certificate. ", ex3);
        }
    }
    ...
    public HashMap verifyCertificate(String givenNATAddress) throws Exception {
        ...
        if (givenNATAddress != null && !givenNATAddress.isEmpty()) {
            final Set sslCertificateHostName = com.me.devicemanagement.framework.server.certificate.SSLCertificateUtil.getInstance().getSSLCertificateHostNames();
        ...
        try {
            ...
            if (com.me.devicemanagement.framework.server.certificate.SSLCertificateUtil.getInstance().isThirdPartySSLInstalled()) {
                final Certificate serverCertificate = CertificateUtils.loadX509CertificateFromFile(new File(com.me.devicemanagement.framework.server.certificate.SSLCertificateUtil.getInstance().getServerCertificateFilePath()));
                final List<String> filePaths = new ArrayList<String>();
                final String intermediatePath = com.me.devicemanagement.framework.server.certificate.SSLCertificateUtil.getInstance().getIntermediateCertificateFilePath();
                if (intermediatePath != null && new File(intermediatePath).exists()) {
                    filePaths.add(intermediatePath);
                }
                final String caPath = com.me.devicemanagement.framework.server.certificate.SSLCertificateUtil.getInstance().getServerCACertificateFilePath();
                ...
                try {
                    ...
                    if (rootCause instanceof CertPathBuilderException) {
                        ...
                        Logger.getLogger("ImportCertificateLogger").log(Level.SEVERE, "Certificate Chain is not verified. Need to re upload certificate with complete chain.");
                    }
                    else {
                        Logger.getLogger("ImportCertificateLogger").log(Level.SEVERE, "Exception in verifyCertificateChain", (Throwable)exp);
                    }
                }
            }
            else {
            ...
            try {
                ...
                Logger.getLogger("ImportCertificateLogger").log(Level.INFO, "Exception in getting I18N properties" + e);
            }
        }
        catch (final Exception ex2) {
            map.put("CERT_CHAIN_NOT_VERIFIED", false);
            MessageProvider.getInstance().hideMessage("CERT_CHAIN_NOT_VERIFIED");
            Logger.getLogger("ImportCertificateLogger").log(Level.SEVERE, "Error in certificate verification", ex2);
        }
    ...
    public List<String> getServerInfoFromDB() throws DataAccessException {
    ...
    public boolean checkAndGenerateServerCertificate(final String natAddress) throws Exception {
        ...
        if (!this.isThirdPartySSLInstalled()) {
    ...
    public void doPostCertificateGeneration() throws Exception {
        ...
        com.me.devicemanagement.onpremise.server.util.SyMUtil.updateSyMParameter("SAN_CERTIFICATE_GENERATED", "true");
    ...
    private void copyCertsToClientDataDir() throws Exception {
    ...
    public Boolean checkServerFQDNWithSANList(final CertificateChainObject certificateResponseObject) throws Exception {
        Boolean isSANListMatchesWithServerFQDN = Boolean.FALSE;
        final String serverFQDN = com.me.devicemanagement.onpremise.server.util.SyMUtil.getServerFQDNName();
        this.logger.log(Level.INFO, "Server FQDN : " + serverFQDN);
        this.logger.log(Level.INFO, "Server certificate SAN :" + this.getSSLCertificateHostNames((X509Certificate)certificateResponseObject.getServerCertificate()));
        if (serverFQDN != null && !serverFQDN.trim().equals("")) {
            isSANListMatchesWithServerFQDN = this.checkHostNameValidWithCertificate(serverFQDN, (X509Certificate)certificateResponseObject.getServerCertificate());
        }
        return isSANListMatchesWithServerFQDN;
    }
    
    static {
        SSLCertificateUtil.sslCertificateUtil = null;
        try {
            SSLCertificateUtil.webServerName = WebServerUtil.getWebServerName();
        }
        catch (final Exception e) {
            Logger.getLogger("ImportCertificateLogger").log(Level.SEVERE, "Exception in getting Webserver Name.");
        }
