...
public class DBQueryExecutorHandler implements DBQueryExecutorAPI
{
    private static Logger logger;
    private static final String DB_CONF_FILE;
    
    public static TableModel executeUserQuery(final String query, final boolean isUpdate) throws Exception {
        TableModel tm = null;
        try {
            SyMLogger.info(DBQueryExecutorHandler.logger, "DBQueryExecutorHandler", "executeUserQuery", "Modified User Query is : \n" + query);
            tm = (TableModel)((DBQueryExecutorAPI)Class.forName(ProductClassLoader.getSingleImplProductClass("DM_DB_QUERY_EXECUTOR_IMPL_CLASS")).newInstance()).getTableModel(query, (String)null, isUpdate);
    ...
    public TableDatasetModel getTableModel(final String query, final String countSql, final boolean isUpdate) throws Exception {
        return getAsTableModel(query, countSql, null, true, null, null, isUpdate);
    }
    
    public Connection getConnection(final String connectionType) throws Exception {
        Properties dbProperties = null;
        final File propFile = new File(ApiFactoryProvider.getUtilAccessAPI().getServerHome() + File.separator + DBQueryExecutorHandler.DB_CONF_FILE);
        if (!DBUtil.getActiveDBName().trim().equalsIgnoreCase("postgres") || !connectionType.equals("READ_ONLY") || com.me.devicemanagement.onpremise.server.util.DBUtil.isRemoteDB()) {
            return RelationalAPI.getInstance().getConnection();
        }
        if (propFile.exists()) {
            dbProperties = FileUtils.readPropertyFile(propFile);
            Class.forName(dbProperties.getProperty("drivername")).newInstance();
            final String connURL = dbProperties.getProperty("url");
            final String userName = dbProperties.getProperty("r_username");
            final String readOnlyPass = dbProperties.getProperty("r_password", userName);
            final String password = PersistenceUtil.getDBPasswordProvider("postgres").getPassword((Object)readOnlyPass);
            return DriverManager.getConnection(connURL, userName, password);
        }
        return null;
    }
    
    public static boolean isCSRFSecure(final List<String> csrfSaltList, final String csrfSaltFromRequest) {
        if (csrfSaltList != null && csrfSaltList.contains(csrfSaltFromRequest)) {
            csrfSaltList.remove(csrfSaltFromRequest);
            return true;
        }
        return false;
    }
    
    public static String getNewCsrfValue(final List<String> csrfSaltList) {
        final String salt = RandomStringUtils.random(20, 0, 0, true, true, (char[])null, (Random)new SecureRandom());
        csrfSaltList.add(salt);
        return salt;
    }
    
    static {
        DBQueryExecutorHandler.logger = Logger.getLogger("QueryExecutorLogger");
        DB_CONF_FILE = "conf" + File.separator + "database_params.conf";
    }
}
