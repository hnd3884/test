...
public class ResetPassword extends StandAlonePersistence
{
    ...
    private static void showMsgInConsoleAndLog(final String msg) {
        ...
        ResetPassword.logger.log(Level.INFO, msg);
    }
    ...
    private static void initDBMigrationLog() {
        try {
            ...
            ResetPassword.logger.log(Level.INFO, "###########################################################");
        }
        catch (final Exception ex) {
            ResetPassword.logger.log(Level.WARNING, "Custom_Query_Executer_log.txt File not Found", ex);
        }
    }
    ...
    public static void main(final String[] args) throws Exception {
        try {
            final ResetPassword resetPwd = new ResetPassword();
            initializeCache();
    ...
    private static String generateTempPwdWithSpecialChar() {
    ...
    private static void changePassword() throws Exception {
        ...
        if (loginId != null) {
            final String tempPassword = generateTempPwdWithSpecialChar();
    ...
    private static ArrayList getDCUsers() {
        ...
        try {
            ...
            final Join accountJoin = new Join("AaaLogin", "AaaAccount", new String[] { "USER_ID" }, new String[] { "LOGIN_ID" }, 2);
            final Join accountStatusExtnJoin = new Join("AaaAccount", "AaaAccountStatusExtn", new String[] { "ACCOUNT_ID" }, new String[] { "ACCOUNT_ID" }, 2);
            Criteria activeCriteria = new Criteria(Column.getColumn("AaaUserStatus", "STATUS"), (Object)"ACTIVE", 0, (boolean)Boolean.FALSE);
            activeCriteria = activeCriteria.and(new Criteria(Column.getColumn("AaaAccountStatusExtn", "STATUS"), (Object)"0", 0));
            selectQuery.addJoin(userStatusJoin);
            selectQuery.addJoin(accountJoin);
            selectQuery.addJoin(accountStatusExtnJoin);
            ...
            ResetPassword.logger.log(Level.SEVERE, "Exception in getting User details", e);
        }
        return list;
    }
    ...
    private static Long getLoginIdForUser(final String userName, final String domainName) {
        ...
        try {
            ...
            Criteria activeCriteria = new Criteria(Column.getColumn("AaaUserStatus", "STATUS"), (Object)"ACTIVE", 0, (boolean)Boolean.FALSE);
            activeCriteria = activeCriteria.and(new Criteria(Column.getColumn("AaaAccountStatusExtn", "STATUS"), (Object)"0", 0));
            criteria = criteria.and(activeCriteria);
            final SelectQuery selectQuery = (SelectQuery)new SelectQueryImpl(new Table("AaaLogin"));
            final Join userStatusJoin = new Join("AaaLogin", "AaaUserStatus", new String[] { "USER_ID" }, new String[] { "USER_ID" }, 2);
            final Join accountJoin = new Join("AaaLogin", "AaaAccount", new String[] { "USER_ID" }, new String[] { "LOGIN_ID" }, 2);
            final Join accountStatusExtnJoin = new Join("AaaAccount", "AaaAccountStatusExtn", new String[] { "ACCOUNT_ID" }, new String[] { "ACCOUNT_ID" }, 2);
            ...
            ResetPassword.logger.log(Level.SEVERE, "Error occured while checking user account status", e);
        }
        return loginID;
    }
    ...
    private static void addOrUpdateAPIKeyForLoginId(final Long loginId) {
        ...
        try {
            ...
            ResetPassword.logger.log(Level.SEVERE, "Exception in addOrUpdateAPIKey for login Id " + loginId, ex);
        }
    }
    ...
    public static Long getCompatiblePassRuleId(final String[] services) throws DataAccessException {
        ...
        if (count == 0 || count > 1) {
            ResetPassword.logger.log(Level.FINEST, "more than one password rule obtained. so use password rule mapped to service System");
        ...
        ResetPassword.logger.log(Level.FINEST, "passwordrule row obtained matching services : {0} is : {1}", new Object[] { services, passRuleRow });
    ...
    static {
        ResetPassword.logger = Logger.getLogger(ResetPassword.class.getName());
    }
}
