...
public class ServerSSLCertificateChangeHandler
{
    Logger logger;
    String serverHome;
    ...
    public ServerSSLCertificateChangeHandler(final CertificateChainObject certificateResponseObject, final Properties userConfigProperties) {
    ...
    public void processRequest() throws SyMException {
        ...
        final ServerSSLCertificateChangeEvent event = this.certificateResponseObject.isSelfSignedCA ? new ServerSSLCertificateChangeEvent(3, this.certificateResponseObject) : new ServerSSLCertificateChangeEvent(2, this.certificateResponseObject);
        try {
            final Boolean allowCertificateUpload = this.allowEnterpriseCASignedCertificateUpload();
            final JSONObject remarkJSON = ServerSSLCertificateHandler.getInstance().canUploadCertificateListener(event);
            if (remarkJSON != null && !remarkJSON.optBoolean("status")) {
                ...
                if (errorCode == 80027 && !allowCertificateUpload) {
                    throw new SyMException(80019, remarkJSON.optString("errorKey"), (Throwable)new Exception());
                }
                if (errorCode == 80029) {
                    throw new SyMException(80024, remarkJSON.optString("errorKey"), (Throwable)new Exception());
                }
                if (errorCode == 80039) {
                    throw new SyMException(80039, remarkJSON.optString("errorKey"), (Throwable)new Exception());
                }
            }
            ...
            if (allowCertificateUpload || (remarkJSON != null && remarkJSON.optBoolean("status"))) {
                ...
                if (!isBackupSuccessful) {
                    throw new SyMException(80016, "Backup Failed - Failed to create backup of existing certificate", (Throwable)new Exception());
                }
                isFileMoveSuccessful = this.moveFilesToWebServerFolder(uploadFolder);
                if (!isFileMoveSuccessful) {
                    throw new SyMException(80017, "Writing to webserver/conf folder failed - Failed while moving files from upload folder to webserver/conf folder", (Throwable)new Exception());
                }
                isConfFileUpdationSuccessful = this.updateConfFiles();
                if (!isConfFileUpdationSuccessful) {
                    throw new SyMException(80018, "Websettings.conf file Updation Failed", (Throwable)new Exception());
                }
                SyMUtil.updateSyMParameter("IsEnterpriseCASignedServerCert", this.certificateResponseObject.isSelfSignedCA.toString());
                SyMUtil.updateSyMParameter("server_certificate_thumbprint", SSLCertificateUtil.getInstance().getThumbPrint((X509Certificate)this.certificateResponseObject.serverCertificate));
                if (this.certificateResponseObject.rootCACertificate != null) {
                    SyMUtil.updateSyMParameter("root_certificate_thumbprint", SSLCertificateUtil.getInstance().getThumbPrint((X509Certificate)this.certificateResponseObject.rootCACertificate));
                }
            ...
            throw new SyMException(80025, "Error in SSLCertificateChangeHandler", (Throwable)ex);
        }
        finally {
            try {
    ...
    private Boolean checkWritePermissionForFolders() throws Exception {
        Boolean isWriteAllowedForFolders = Boolean.TRUE;
        try {
            this.checkPermissionForFolderOrParentRecursively(this.webServerConfFolder);
        }
        catch (final Exception ex) {
            isWriteAllowedForFolders = Boolean.FALSE;
            throw new SyMException(80026, "Write permission not given for the following folder ", new File(this.webServerConfFolder).getCanonicalPath(), (Throwable)ex);
        }
        try {
            ...
            isWriteAllowedForFolders = Boolean.FALSE;
            throw new SyMException(80026, "Write permission not given for the following folder ", new File(this.certificateBackUpPath).getCanonicalPath(), (Throwable)ex);
        }
        return isWriteAllowedForFolders;
    }
    ...
    private void writeFilesToDestination(final String destinationPath, final CertificateChainObject certificateResponse) throws Exception {
        this.writePrivateKeyToFile(destinationPath + File.separator + "server.key", certificateResponse.serverKey);
        final StringBuilder allCertsValue = new StringBuilder();
        String allCertsFromServerCert = this.getBase64EncodedValueFromCert(certificateResponse.serverCertificate);
        ...
        final Certificate[] intermediateCerts = intermediateCertificate = certificateResponse.intermediateCertificate;
        ...
        this.writeCertificateListToFile(destinationPath + File.separator + "intermediate.crt", Arrays.asList(certificateResponse.intermediateCertificate));
        if (certificateResponse.rootCACertificate != null) {
            allCertsValue.append(this.getBase64EncodedValueFromCert(certificateResponse.rootCACertificate));
            this.writeCertificateListToFile(destinationPath + File.separator + "caCert.crt", Arrays.asList(certificateResponse.rootCACertificate));
        }
    ...
    private Boolean backUpServerCertificateAndDetails() throws Exception {
        ...
        if (fileAccessApi.isDirectory(this.certificateBackUpFolder) && fileAccessApi.isFileExists(this.certificateBackUpFolder)) {
        ...
        Boolean isSuccess = CertificateUtil.getInstance().copyFileListToDestinationFolder((List)filesToBackUp, this.certificateBackUpFolder);
        if (isSuccess) {
            filesToBackUp.remove(websettingsFile);
            isSuccess = CertificateUtil.getInstance().deleteFiles((ArrayList)filesToBackUp);
        }
        return isSuccess;
    }
    ...
    protected void cleanAllTempFolders() throws Exception {
        if (this.certificateResponseObject.tempFoldersListToDeleteFinally != null) {
            final FileAccessAPI fileAccessApi = ApiFactoryProvider.getFileAccessAPI();
