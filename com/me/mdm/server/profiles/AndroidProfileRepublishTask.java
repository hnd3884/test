...
public class AndroidProfileRepublishTask implements SchedulerExecutionInterface
{
    private static final Logger LOGGER;
    
    public void executeTask(final Properties props) {
        try {
            final String paramToExecute = ((Hashtable<K, String>)props).get("sysParam");
            final String isNeedToExecute = MDMUtil.getSyMParameter(paramToExecute);
            if (!MDMStringUtils.isEmpty(isNeedToExecute) && Boolean.valueOf(isNeedToExecute)) {
                final int configId = ((Hashtable<K, Integer>)props).get("configId");
                AndroidProfileRepublishTask.LOGGER.log(Level.INFO, "Starting the Android Profile : {0} regenerate task", new Object[] { configId });
                this.republishAndroidProfile(configId);
                MDMUtil.updateSyMParameter(paramToExecute, "false");
            }
        }
        catch (final Exception e) {
            AndroidProfileRepublishTask.LOGGER.log(Level.SEVERE, "Exception in Android Profile regenerate task", e);
        }
    }
    
    public void republishAndroidProfile(final int configId) throws Exception {
        try {
            final Long[] customerIds = CustomerInfoUtil.getInstance().getCustomerIdsFromDB();
            AndroidProfileRepublishTask.LOGGER.log(Level.INFO, "CustomerIDs for Android Profile : {1} Regeneration:{0}", new Object[] { customerIds, configId });
            if (customerIds != null) {
                    ...
                    if (!dataObject.isEmpty()) {
                        ...
                        while (profileCollectionIterator.hasNext()) {
                            ...
                            if (profileCollectionRow != null) {
                                ...
                                AndroidProfileRepublishTask.LOGGER.log(Level.INFO, "Going to generate json for android profile : {1} prop:{0}", new Object[] { publishProperties, configId });
                                new MDMConfigHandler().createAndroidJSON(publishProperties);
                            }
                        }
                    }
                    else {
                        AndroidProfileRepublishTask.LOGGER.log(Level.INFO, "There is no android profile with config id {1} for customer:{0}", new Object[] { customerId, configId });
                    }
                }
            }
        }
        catch (final SyMException e) {
            AndroidProfileRepublishTask.LOGGER.log(Level.SEVERE, "Exception in getting customerIds", (Throwable)e);
            throw e;
        }
        catch (final DataAccessException e2) {
            AndroidProfileRepublishTask.LOGGER.log(Level.SEVERE, "Exception in getting the dataObject", (Throwable)e2);
            throw e2;
        }
    }
