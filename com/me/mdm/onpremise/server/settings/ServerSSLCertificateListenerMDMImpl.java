...
import com.me.devicemanagement.onpremise.server.certificate.ServerSSLCertificateChangeEvent;
...
public class ServerSSLCertificateListenerMDMImpl implements ServerSSLCertificateListener
{
    private static final String SYNC_DATA_URL = "/syncDataServlet";
    Logger logger;
    
    public ServerSSLCertificateListenerMDMImpl() {
    ...
    public JSONObject canUploadCertificate(final ServerSSLCertificateChangeEvent changeEvent) {
        ...
        try {
            if (changeEvent.certificateType == 3) {
                ...
                if (managedDeviceCount > 0 && managedDeviceCount <= 5) {
                    remarkJSON.put("status", false);
