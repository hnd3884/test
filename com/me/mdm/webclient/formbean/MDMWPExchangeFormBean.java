...
public class MDMWPExchangeFormBean extends MDMDefaultFormBean
{
    @Override
    protected boolean getTransformedFormPropertyValue(final JSONObject multipleConfigForm, final JSONObject dynaFormData, final PayloadProperty payloadProperty) throws Exception {
        if (payloadProperty.name.equals("ACCOUNT_ICON_NAME")) {
            ...
            final boolean fileUploaded = ProfileUtil.getInstance().uploadProfileImageFile(accountIconPathSrc, ProfileUtil.getProfileExchangeActiveSyncAccountIconFolderPath(), fileName);
            if (fileUploaded) {
