...
import com.azul.crs.com.fasterxml.jackson.databind.util.LookupCache;
...
public class TypeFactory implements Serializable
{
    ...
    @Deprecated
    protected TypeFactory(final LRUMap<Object, JavaType> typeCache) {
    ...
    @Deprecated
    protected TypeFactory(final LRUMap<Object, JavaType> typeCache, final TypeParser p, final TypeModifier[] mods, final ClassLoader classLoader) {
        this((LookupCache<Object, JavaType>)typeCache, p, mods, classLoader);
    }
    
    protected TypeFactory(LookupCache<Object, JavaType> typeCache, final TypeParser p, final TypeModifier[] mods, final ClassLoader classLoader) {
        if (typeCache == null) {
            typeCache = new LRUMap<Object, JavaType>(16, 200);
        }
        this._typeCache = typeCache;
    ...
    public TypeFactory withCache(final LookupCache<Object, JavaType> cache) {
        return new TypeFactory(cache, this._parser, this._modifiers, this._classLoader);
    }
    ...
    public JavaType constructSpecializedType(final JavaType baseType, final Class<?> subclass, final boolean relaxedCompatibilityCheck) throws IllegalArgumentException {
        ...
        if (rawBase == Object.class) {
            ...
            if (baseType.getBindings().isEmpty()) {
                ...
                if (typeParamCount == 0) {
                    ...
                    final TypeBindings tb = this._bindingsForSubtype(baseType, typeParamCount, subclass, relaxedCompatibilityCheck);
    ...
    private TypeBindings _bindingsForSubtype(final JavaType baseType, final int typeParamCount, final Class<?> subclass, final boolean relaxedCompatibilityCheck) {
    ...
    private String _resolveTypePlaceholders(final JavaType sourceType, final JavaType actualType) throws IllegalArgumentException {
        ...
        for (int i = 0, expCount = expectedTypes.size(); i < expCount; ++i) {
            ...
            final JavaType act = (i < actCount) ? actualTypes.get(i) : unknownType();
            if (!this._verifyAndResolvePlaceholders(exp, act)) {
                if (!exp.hasRawClass(Object.class)) {
                    if (i != 0 || !sourceType.isMapLikeType() || !act.hasRawClass(Object.class)) {
    ...
    public JavaType resolveMemberType(final Type type, final TypeBindings contextBindings) {
    ...
    public JavaType constructType(final Type type, final TypeBindings bindings) {
        if (type instanceof Class) {
            final JavaType resultType = this._fromClass(null, (Class<?>)type, bindings);
    ...
    public JavaType constructParametricType(final Class<?> rawType, final TypeBindings parameterTypes) {
        final JavaType resultType = this._fromClass(null, rawType, parameterTypes);
    ...
    protected JavaType _findWellKnownSimple(final Class<?> clz) {
        if (clz.isPrimitive()) {
            ...
            if (clz == TypeFactory.CLS_JSON_NODE) {
    ...
    protected JavaType _fromAny(final ClassStack context, final Type srcType, final TypeBindings bindings) {
        JavaType resultType;
        if (srcType instanceof Class) {
            resultType = this._fromClass(context, (Class<?>)srcType, TypeFactory.EMPTY_BINDINGS);
        }
    ...
    protected JavaType _applyModifiers(final Type srcType, final JavaType resolvedType) {
        ...
        for (final TypeModifier mod : this._modifiers) {
            final JavaType t = mod.modifyType(resultType, srcType, b, this);
            if (t == null) {
    ...
    static {
        ...
        CORE_TYPE_JSON_NODE = new SimpleType(TypeFactory.CLS_JSON_NODE);
    }
}
