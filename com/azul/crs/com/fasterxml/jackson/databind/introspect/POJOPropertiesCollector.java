...
public class POJOPropertiesCollector
{
    ...
    protected POJOPropertiesCollector(final MapperConfig<?> config, final boolean forSerialization, final JavaType type, final AnnotatedClass classDef, final AccessorNamingStrategy accessorNaming) {
        this._config = config;
    ...
    protected void _addFields(final Map<String, POJOPropertyBuilder> props) {
        ...
        for (final AnnotatedField f : this._classDef.fields()) {
            if (Boolean.TRUE.equals(ai.hasAsKey(this._config, f))) {
                if (this._jsonKeyAccessors == null) {
                    this._jsonKeyAccessors = new LinkedList<AnnotatedMember>();
                }
            ...
            if (Boolean.TRUE.equals(ai.hasAsValue(f))) {
                ...
                final boolean anyGetter = Boolean.TRUE.equals(ai.hasAnyGetter(f));
                final boolean anySetter = Boolean.TRUE.equals(ai.hasAnySetter(f));
                if (anyGetter || anySetter) {
                    if (anyGetter) {
                        if (this._anyGetterField == null) {
                            this._anyGetterField = new LinkedList<AnnotatedMember>();
                        }
                    ...
                    final PropertyName implNameP = this._propNameFromSimple(implName);
                    final PropertyName rename = ai.findRenameByField(this._config, f, implNameP);
    ...
    private String _checkRenameByField(String implName) {
        if (this._fieldRenameMappings != null) {
            final PropertyName p = this._fieldRenameMappings.get(this._propNameFromSimple(implName));
            if (p != null) {
    ...
    protected void _collectIgnorals(final String name) {
    ...
    protected void _renameProperties(final Map<String, POJOPropertyBuilder> props) {
        ...
        if (renamed != null) {
            for (final POJOPropertyBuilder prop : renamed) {
                ...
                if (this._updateCreatorProperty(prop, this._creatorProperties) && this._ignoredPropertyNames != null) {
    ...
    protected void _sortProperties(final Map<String, POJOPropertyBuilder> props) {
        ...
        final boolean sortAlpha = (alpha == null) ? this._config.shouldSortPropertiesAlphabetically() : alpha;
        final boolean indexed = this._anyIndexed(props.values());
        ...
        if (indexed) {
            final Map<Integer, POJOPropertyBuilder> byIndex = new TreeMap<Integer, POJOPropertyBuilder>();
            final Iterator<Map.Entry<String, POJOPropertyBuilder>> it = all.entrySet().iterator();
            while (it.hasNext()) {
                final Map.Entry<String, POJOPropertyBuilder> entry = it.next();
                final POJOPropertyBuilder prop3 = entry.getValue();
                final Integer index = prop3.getMetadata().getIndex();
                if (index != null) {
                    byIndex.put(index, prop3);
                    it.remove();
                }
            }
            final Iterator<POJOPropertyBuilder> iterator3 = byIndex.values().iterator();
            while (iterator3.hasNext()) {
                final POJOPropertyBuilder prop3 = iterator3.next();
    ...
    private boolean _anyIndexed(final Collection<POJOPropertyBuilder> props) {
        for (final POJOPropertyBuilder prop : props) {
    ...
    protected boolean _updateCreatorProperty(final POJOPropertyBuilder prop, final List<POJOPropertyBuilder> creatorProperties) {
