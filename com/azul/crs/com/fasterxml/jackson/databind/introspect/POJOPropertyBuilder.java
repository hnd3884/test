...
public class POJOPropertyBuilder extends BeanPropertyDefinition implements Comparable<POJOPropertyBuilder>
{
    ...
    public PropertyMetadata getMetadata() {
        if (this._metadata == null) {
            final AnnotatedMember prim = this.getPrimaryMemberUnchecked();
            if (prim == null) {
                this._metadata = PropertyMetadata.STD_REQUIRED_OR_OPTIONAL;
            }
            else {
                final Boolean b = this._annotationIntrospector.hasRequiredMarker(prim);
                final String desc = this._annotationIntrospector.findPropertyDescription(prim);
                final Integer idx = this._annotationIntrospector.findPropertyIndex(prim);
                final String def = this._annotationIntrospector.findPropertyDefaultValue(prim);
    ...
    protected PropertyMetadata _getSetterInfo(PropertyMetadata metadata, final AnnotatedMember primary) {
        ...
        if (primary != null) {
            if (this._annotationIntrospector != null) {
                if (acc != null) {
                    final Boolean b = this._annotationIntrospector.findMergeInfo(primary);
                    if (b != null) {
                        needMerge = false;
                        if (b) {
                ...
                final JsonSetter.Value setterInfo = this._annotationIntrospector.findSetterInfo(primary);
                if (setterInfo != null) {
            ...
            if (needMerge || valueNulls == null || contentNulls == null) {
                final Class<?> rawType = this._rawTypeOf(primary);
    ...
    public Class<?>[] findViews() {
        return this.fromMemberAnnotations((WithMember<Class[]>)new WithMember<Class<?>[]>() {
            @Override
    ...
    public JsonProperty.Access removeNonVisible(final boolean inferMutators, final POJOPropertiesCollector parent) {
        ...
        switch (acc) {
            case READ_ONLY: {
                if (parent != null) {
                    ...
                    for (final PropertyName pn : this.findExplicitNames()) {
    ...
    protected Class<?> _rawTypeOf(final AnnotatedMember m) {
        if (m instanceof AnnotatedMethod) {
            final AnnotatedMethod meh = (AnnotatedMethod)m;
