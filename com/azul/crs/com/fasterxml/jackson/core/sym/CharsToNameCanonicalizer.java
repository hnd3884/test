package com.azul.crs.com.fasterxml.jackson.core.sym;

import java.util.Arrays;
import com.azul.crs.com.fasterxml.jackson.core.util.InternCache;
...
public final class CharsToNameCanonicalizer
{
    ...
    private void _handleSpillOverflow(final int bucketIndex, final Bucket newBucket, final int mainIndex) {
        if (this._overflows == null) {
            (this._overflows = new BitSet()).set(bucketIndex);
        }
    ...
    private void rehash() {
        final int size = this._symbols.length;
        ...
        for (int bucketSize = size >> 1, j = 0; j < bucketSize; ++j) {
            for (Bucket b = oldBuckets[j]; b != null; b = b.next) {
                ++count;
    ...
    protected void verifyInternalConsistency() {
        ...
        for (int i = 0; i < size; ++i) {
            final String symbol = this._symbols[i];
            if (symbol != null) {
                ++count;
            }
        }
