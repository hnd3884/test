...
public abstract class StdDeserializer<T> extends JsonDeserializer<T> implements Serializable
{
    ...
    protected final boolean _parseBooleanPrimitive(final DeserializationContext ctxt, final JsonParser p, final Class<?> targetType) throws IOException {
        ...
        if (t == JsonToken.VALUE_STRING) {
            ...
            final Boolean b = (Boolean)ctxt.handleWeirdStringValue(targetType, text, "only \"true\" or \"false\" recognized", new Object[0]);
            return Boolean.TRUE.equals(b);
        }
        else {
            if (t == JsonToken.START_ARRAY && ctxt.isEnabled(DeserializationFeature.UNWRAP_SINGLE_VALUE_ARRAYS)) {
                p.nextToken();
                final boolean parsed = this._parseBooleanPrimitive(ctxt, p, targetType);
    ...
    protected T _deserializeFromArray(final JsonParser p, final DeserializationContext ctxt) throws IOException {
        ...
        final T result = (T)ctxt.handleUnexpectedToken(this.getValueType(ctxt), p.getCurrentToken(), p, null, new Object[0]);
        return result;
    }
    ...
    protected T _deserializeWrappedValue(final JsonParser p, final DeserializationContext ctxt) throws IOException {
        if (p.hasToken(JsonToken.START_ARRAY)) {
            ...
            final T result = (T)ctxt.handleUnexpectedToken(this.getValueType(ctxt), p.getCurrentToken(), p, msg, new Object[0]);
            return result;
        }
