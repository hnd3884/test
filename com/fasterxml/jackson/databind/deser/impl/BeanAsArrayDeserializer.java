...
public class BeanAsArrayDeserializer extends BeanDeserializerBase
{
    ...
    protected Object _deserializeFromNonArray(final JsonParser p, final DeserializationContext ctxt) throws IOException {
        final String message = "Cannot deserialize a POJO (of type %s) from non-Array representation (token: %s): type/property designed to be serialized as JSON Array";
        return ctxt.handleUnexpectedToken(this.getValueType(ctxt), p.getCurrentToken(), p, message, this._beanType.getRawClass().getName(), p.getCurrentToken());
    }
}
