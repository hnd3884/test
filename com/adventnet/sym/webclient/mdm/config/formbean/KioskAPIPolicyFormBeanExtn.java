...
public class KioskAPIPolicyFormBeanExtn extends KioskAPIPolicyFormBean
{
    private Logger logger;
    HashMap<Long, Object> webClipsParentChildMap;
    
    public KioskAPIPolicyFormBeanExtn() {
    ...
    private void cloneAppLockPolicyApps(final Integer configID, final DataObject configDOFromDB, final DataObject cloneConfigDO) throws Exception {
        if (cloneConfigDO.containsTable("AndroidKioskPolicyApps")) {
            ...
            HashMap<Long, Object> parentChildUVHMap = CloneGlobalConfigHandler.getInstance().getParentChildAppsUVHMap(cloneConfigDO, kioskConfiguredApps);
            final Object configDataItem = CloneGlobalConfigHandler.getInstance().getConfigDataItemId(configID, cloneConfigDO);
            final Iterator<Row> iterator = cloneConfigDO.getRows("AndroidKioskPolicyApps", new Criteria(Column.getColumn("AndroidKioskPolicyApps", "CONFIG_DATA_ITEM_ID"), configDataItem, 0));
            ...
            if (cloneConfigDO.containsTable("AndroidKioskPolicyBackgroundApps")) {
                final List backgroundApps = CloneGlobalConfigHandler.getInstance().getKioskConfiguredApps(configDOFromDB, "MDBACKGROUNDAPPGROUPDETAILS", "APP_GROUP_ID");
                parentChildUVHMap = CloneGlobalConfigHandler.getInstance().getParentChildAppsUVHMap(cloneConfigDO, backgroundApps);
                final Iterator<Row> iterator2 = cloneConfigDO.getRows("AndroidKioskPolicyBackgroundApps", new Criteria(Column.getColumn("AndroidKioskPolicyBackgroundApps", "CONFIG_DATA_ITEM_ID"), configDataItem, 0));
                while (iterator2.hasNext()) {
                    final Row appDetailsRow2 = iterator.next();
                    final Long parentAppGroupId2 = (Long)appDetailsRow2.get("APP_GROUP_ID");
                    final Object childAppGroupId2 = parentChildUVHMap.get(parentAppGroupId2);
                    if (childAppGroupId2 != null) {
                        appDetailsRow2.set("APP_GROUP_ID", childAppGroupId2);
                        cloneConfigDO.updateRow(appDetailsRow2);
                    }
                }
            }
        }
    ...
    private void cloneHomeScreenLayoutPolicy(final Integer configID, final DataObject configDOFromDB, final DataObject cloneConfigDO) throws Exception {
        if (cloneConfigDO.containsTable("ScreenLayoutSettings")) {
            ...
            final String payloadName = "androidkioskhomescreenpolicy";
    ...
    public void cloneConfigDO(final Integer configID, final DataObject configDOFromDB, final DataObject cloneConfigDO) throws DataAccessException {
        ...
        if (!parentCustomerId.equals(childCustomerId)) {
            try {
                ...
                this.logger.log(Level.SEVERE, "Exception in cloneConfigDO of KioskAPIPolicyFormBeanExtn", ex);
            }
        }
    }
}
