...
public class ceil extends FunctionCalls
{
    ...
    public void toInformix(final SelectQueryStatement to_sqs, final SelectQueryStatement from_sqs) throws ConvertException {
        ...
        final SelectColumn ceilArgument = new SelectColumn();
        final Vector ceilArgs = new Vector();
        final TableColumn innerFunction = new TableColumn();
        innerFunction.setColumnName("CAST");
        final FunctionCalls cast = new FunctionCalls();
        final String castArgument = new String(arg + "AS INT");
        final Vector castArg = new Vector();
        final Vector ceilArg = new Vector();
        castArg.add(castArgument);
        cast.setFunctionName(innerFunction);
        cast.setFunctionArguments(castArg);
        ceilArg.addElement(cast);
        ceilArg.addElement("+ 1");
        ceilArgument.setColumnExpression(ceilArg);
        ceilArgs.add(ceilArgument);
        this.setFunctionArguments(ceilArgs);
    }
