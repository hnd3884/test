...
public class fiscalQuarterYear extends FunctionCalls
{
    ...
    public void toVectorWise(final SelectQueryStatement to_sqs, final SelectQueryStatement from_sqs) throws ConvertException {
        if (this.functionName.getColumnName().equalsIgnoreCase("ZR_fquarteryear")) {
            ...
            qry = " (YEAR(timestamp(" + colName + ") + INTERVAL '1' MONTH * " + addMonth + ") * 10) + quarter(timestamp(" + colName + ") + INTERVAL  '1'  MONTH * " + addMonth + ")";
            this.functionName.setColumnName(qry);
            this.setOpenBracesForFunctionNameRequired(false);
            this.functionArguments = new Vector();
        }
        else if (this.functionName.getColumnName().equalsIgnoreCase("ZR_fquarteryearintrval")) {
            ...
            qry2 = "CAST((((((MONTH((FROM_UNIXTIME(0) + INTERVAL  '1'  second * (" + colNameInt + " / 1000))) + " + (Object)argu[1] + " -1) / 12) + YEAR((FROM_UNIXTIME(0) + INTERVAL  '1'  second * (" + colNameInt + " / 1000)))) * 10) + ((  MOD((MONTH((FROM_UNIXTIME(0) + INTERVAL  '1'  second * (" + colNameInt + " / 1000))) + " + (Object)argu[1] + " -1), 12)  ) / 3) + 1) AS INTEGER)";
            ...
            qry2 = "CAST((((((MONTH(" + colNameDate + ") + " + (Object)argu[1] + " -1) / 12) + YEAR(" + colNameDate + ")) * 10) + ((  MOD((MONTH(" + colNameDate + ") + " + (Object)argu[1] + " -1), 12)  ) / 3) + 1) AS INTEGER)";
            this.functionName.setColumnName(qry2);
            this.setOpenBracesForFunctionNameRequired(false);
            this.functionArguments = new Vector();
        }
