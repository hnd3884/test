...
public class IPHandler
{
    ...
    protected static boolean addIP(final FOSConfig config) throws Exception {
        ...
        if (consecutiveFailureCount >= config.ipCheckRetryCount()) {
            if (!isInterfaceUp(config.publicIPIfName())) {
                config.errorHandler().handleError(new FOSException(FOSErrorCode.ERROR_IF_DOWN, "Network Interface Card is NOT working"));
            }
            else {
                config.errorHandler().handleError(new FOSException(FOSErrorCode.ERROR_IP_BINDING, "Not able to bind the 'public_ip' to this node"));
            }
        }
        return false;
    }
    ...
    protected static void deleteIP(final FOSConfig config) throws Exception {
        final int status = FOSUtil.getInst().deleteIP(config.publicIP());
        if (status != 0) {
            config.errorHandler().handleError(new FOSException(FOSErrorCode.ERROR_IP_UNBINDING, "Problem while starting FOS. Unbinding of public_ip failed with status [" + status + "]"));
        }
    }
    
    private static boolean isOtherNodePresentWithIP(final FOSConfig config) throws Exception {
        deleteIP(config);
        for (int i = 0; i < 4; ++i) {
    ...
    private static void ipPrecheck(final FOSConfig config) throws Exception {
        if (isOtherNodePresentWithIP(config)) {
            config.errorHandler().handleError(new FOSException(FOSErrorCode.ERROR_IP_CLASH, "Another node present in network with 'public_ip' binded to it"));
        }
    }
