...
public final class GenerateSourceFromSchema extends LDAPCommandLineTool implements Serializable
{
    ...
    private static final TreeSet<String> EMPTY_TREE_SET;
    ...
    public void addNonLDAPArguments(final ArgumentParser parser) throws ArgumentException {
        (this.outputDirectoryArg = new FileArgument('d', "outputDirectory", false, 1, PersistMessages.INFO_GEN_SOURCE_VALUE_PLACEHOLDER_PATH.get(), PersistMessages.INFO_GEN_SOURCE_ARG_DESCRIPTION_OUTPUT_DIRECTORY.get(), true, true, false, true)).addLongIdentifier("output-directory", true);
        parser.addArgument(this.outputDirectoryArg);
        (this.structuralClassArg = new StringArgument('s', "structuralClass", true, 1, PersistMessages.INFO_GEN_SOURCE_VALUE_PLACEHOLDER_NAME.get(), PersistMessages.INFO_GEN_SOURCE_ARG_DESCRIPTION_STRUCTURAL_CLASS.get())).addLongIdentifier("structural-class", true);
        parser.addArgument(this.structuralClassArg);
        (this.auxiliaryClassArg = new StringArgument('a', "auxiliaryClass", false, 0, PersistMessages.INFO_GEN_SOURCE_VALUE_PLACEHOLDER_NAME.get(), PersistMessages.INFO_GEN_SOURCE_ARG_DESCRIPTION_AUXILIARY_CLASS.get())).addLongIdentifier("auxiliary-class", true);
        parser.addArgument(this.auxiliaryClassArg);
        (this.rdnAttributeArg = new StringArgument('r', "rdnAttribute", true, 0, PersistMessages.INFO_GEN_SOURCE_VALUE_PLACEHOLDER_NAME.get(), PersistMessages.INFO_GEN_SOURCE_ARG_DESCRIPTION_RDN_ATTRIBUTE.get())).addLongIdentifier("rdn-attribute", true);
        parser.addArgument(this.rdnAttributeArg);
        (this.lazyAttributeArg = new StringArgument('l', "lazyAttribute", false, 0, PersistMessages.INFO_GEN_SOURCE_VALUE_PLACEHOLDER_NAME.get(), PersistMessages.INFO_GEN_SOURCE_ARG_DESCRIPTION_LAZY_ATTRIBUTE.get())).addLongIdentifier("lazy-attribute", true);
        parser.addArgument(this.lazyAttributeArg);
        (this.operationalAttributeArg = new StringArgument('O', "operationalAttribute", false, 0, PersistMessages.INFO_GEN_SOURCE_VALUE_PLACEHOLDER_NAME.get(), PersistMessages.INFO_GEN_SOURCE_ARG_DESCRIPTION_OPERATIONAL_ATTRIBUTE.get())).addLongIdentifier("operational-attribute", true);
        parser.addArgument(this.operationalAttributeArg);
        (this.defaultParentDNArg = new DNArgument('b', "defaultParentDN", false, 1, PersistMessages.INFO_GEN_SOURCE_VALUE_PLACEHOLDER_DN.get(), PersistMessages.INFO_GEN_SOURCE_ARG_DESCRIPTION_DEFAULT_PARENT_DN.get())).addLongIdentifier("default-parent-dn", true);
        parser.addArgument(this.defaultParentDNArg);
        (this.packageNameArg = new StringArgument('n', "packageName", false, 1, PersistMessages.INFO_GEN_SOURCE_VALUE_PLACEHOLDER_NAME.get(), PersistMessages.INFO_GEN_SOURCE_ARG_DESCRIPTION_PACKAGE_NAME.get())).addLongIdentifier("package-name", true);
        parser.addArgument(this.packageNameArg);
        (this.classNameArg = new StringArgument('c', "className", false, 1, PersistMessages.INFO_GEN_SOURCE_VALUE_PLACEHOLDER_NAME.get(), PersistMessages.INFO_GEN_SOURCE_ARG_DESCRIPTION_CLASS_NAME.get())).addLongIdentifier("class-name", true);
        parser.addArgument(this.classNameArg);
        parser.addArgument(this.terseArg = new BooleanArgument('t', "terse", 1, PersistMessages.INFO_GEN_SOURCE_ARG_DESCRIPTION_TERSE.get()));
    }
    ...
    public ResultCode doToolProcessing() {
        ...
        try {
            ...
            this.err(PersistMessages.ERR_GEN_SOURCE_CANNOT_READ_SCHEMA.get(StaticUtils.getExceptionMessage(le2)));
    ...
    private ResultCode generateSourceFile(final Schema schema, final boolean terse) {
        ...
        final String structuralClassName = this.structuralClassArg.getValue();
        ...
        if (this.auxiliaryClassArg.isPresent()) {
            for (final String s : this.auxiliaryClassArg.getValues()) {
                ...
                this.processObjectClass(oc, schema, requiredAttrs, requiredAttrOCs, optionalAttrs, optionalAttrOCs, types);
            }
        }
        final TreeMap<String, ObjectClassDefinition> superiorOCs = new TreeMap<String, ObjectClassDefinition>();
        for (final ObjectClassDefinition s2 : structuralOC.getSuperiorClasses(schema, true)) {
            superiorOCs.put(StaticUtils.toLowerCase(s2.getNameOrOID()), s2);
        }
        for (final ObjectClassDefinition d : auxiliaryOCs.values()) {
            for (final ObjectClassDefinition s3 : d.getSuperiorClasses(schema, true)) {
                superiorOCs.put(StaticUtils.toLowerCase(s3.getNameOrOID()), s3);
            }
        }
        superiorOCs.remove(StaticUtils.toLowerCase(structuralClassName));
        for (final String s4 : auxiliaryOCs.keySet()) {
            superiorOCs.remove(s4);
        }
        ...
        if (this.operationalAttributeArg.isPresent()) {
            for (final String s5 : this.operationalAttributeArg.getValues()) {
                final AttributeTypeDefinition d2 = schema.getAttributeType(s5);
                if (d2 == null) {
                    this.err(PersistMessages.ERR_GEN_SOURCE_OPERATIONAL_ATTRIBUTE_NOT_DEFINED.get(s5));
                    return ResultCode.PARAM_ERROR;
                }
                if (!d2.isOperational()) {
                    this.err(PersistMessages.ERR_GEN_SOURCE_OPERATIONAL_ATTRIBUTE_NOT_OPERATIONAL.get(s5));
                    return ResultCode.PARAM_ERROR;
                }
                final String lowerName = StaticUtils.toLowerCase(s5);
                operationalAttrs.put(lowerName, d2);
                types.put(lowerName, this.getJavaType(schema, d2));
            }
        }
        final TreeSet<String> rdnAttrs = new TreeSet<String>();
        for (final String s6 : this.rdnAttributeArg.getValues()) {
            final AttributeTypeDefinition d3 = schema.getAttributeType(s6);
            if (d3 == null) {
                this.err(PersistMessages.ERR_GEN_SOURCE_RDN_ATTRIBUTE_NOT_DEFINED.get(s6));
                return ResultCode.PARAM_ERROR;
            }
            final String lowerName2 = StaticUtils.toLowerCase(d3.getNameOrOID());
            rdnAttrs.add(lowerName2);
            ...
            requiredAttrs.put(lowerName2, optionalAttrs.remove(lowerName2));
            requiredAttrOCs.put(lowerName2, optionalAttrOCs.remove(lowerName2));
        }
        final TreeSet<String> lazyAttrs = new TreeSet<String>();
        for (final String s7 : this.lazyAttributeArg.getValues()) {
            final AttributeTypeDefinition d4 = schema.getAttributeType(s7);
            if (d4 == null) {
                this.err(PersistMessages.ERR_GEN_SOURCE_LAZY_ATTRIBUTE_NOT_DEFINED.get(s7));
                return ResultCode.PARAM_ERROR;
            }
            final String lowerName3 = StaticUtils.toLowerCase(d4.getNameOrOID());
            lazyAttrs.add(lowerName3);
            if (!requiredAttrs.containsKey(lowerName3) && !optionalAttrs.containsKey(lowerName3) && !operationalAttrs.containsKey(lowerName3)) {
                this.err(PersistMessages.ERR_GEN_SOURCE_LAZY_ATTRIBUTE_NOT_ALLOWED.get(s7));
                return ResultCode.PARAM_ERROR;
            }
        }
        String className;
        if (this.classNameArg.isPresent()) {
            className = this.classNameArg.getValue();
            final StringBuilder invalidReason = new StringBuilder();
            if (!PersistUtils.isValidJavaIdentifier(className, invalidReason)) {
                this.err(PersistMessages.ERR_GEN_SOURCE_INVALID_CLASS_NAME.get(className, invalidReason.toString()));
                return ResultCode.PARAM_ERROR;
            }
        }
        else {
        ...
        final File sourceFile = new File(this.outputDirectoryArg.getValue(), className + ".java");
        PrintWriter writer;
        try {
        ...
        if (this.packageNameArg.isPresent()) {
            final String packageName = this.packageNameArg.getValue();
        ...
        for (final String lowerName4 : rdnAttrs) {
            final AttributeTypeDefinition d5 = requiredAttrs.get(lowerName4);
            final TreeSet<String> ocNames = requiredAttrOCs.get(lowerName4);
            writeField(writer, d5, types.get(lowerName4), ocNames, true, true, structuralClassName, false, terse);
        }
        for (final String lowerName4 : requiredAttrs.keySet()) {
            if (rdnAttrs.contains(lowerName4)) {
                continue;
            }
            final AttributeTypeDefinition d5 = requiredAttrs.get(lowerName4);
            final TreeSet<String> ocNames = requiredAttrOCs.get(lowerName4);
            writeField(writer, d5, types.get(lowerName4), ocNames, false, true, structuralClassName, lazyAttrs.contains(lowerName4), terse);
        }
        for (final String lowerName4 : optionalAttrs.keySet()) {
            final AttributeTypeDefinition d5 = optionalAttrs.get(lowerName4);
            final TreeSet<String> ocNames = optionalAttrOCs.get(lowerName4);
            writeField(writer, d5, types.get(lowerName4), ocNames, false, false, structuralClassName, lazyAttrs.contains(lowerName4), terse);
        }
        for (final String lowerName4 : operationalAttrs.keySet()) {
            final AttributeTypeDefinition d5 = operationalAttrs.get(lowerName4);
            final TreeSet<String> ocNames = GenerateSourceFromSchema.EMPTY_TREE_SET;
            writeField(writer, d5, types.get(lowerName4), ocNames, false, false, structuralClassName, lazyAttrs.contains(lowerName4), terse);
        }
        writer.println();
        writer.println();
        ...
        for (final String lowerName4 : rdnAttrs) {
            final AttributeTypeDefinition d5 = requiredAttrs.get(lowerName4);
            writeFieldMethods(writer, d5, types.get(lowerName4), true);
        }
        for (final String lowerName4 : requiredAttrs.keySet()) {
            if (rdnAttrs.contains(lowerName4)) {
                continue;
        ...
        for (final String lowerName4 : operationalAttrs.keySet()) {
            final AttributeTypeDefinition d5 = operationalAttrs.get(lowerName4);
            writeFieldMethods(writer, d5, types.get(lowerName4), false);
        }
    ...
    private void processObjectClass(final ObjectClassDefinition oc, final Schema s, final TreeMap<String, AttributeTypeDefinition> ra, final TreeMap<String, TreeSet<String>> rac, final TreeMap<String, AttributeTypeDefinition> oa, final TreeMap<String, TreeSet<String>> oac, final TreeMap<String, String> t) {
    ...
    private static void writeField(final PrintWriter writer, final AttributeTypeDefinition d, final String type, final TreeSet<String> ocNames, final boolean inRDN, final boolean required, final String sc, final boolean lazy, final boolean terse) {
    ...
    private static void writeFieldMethods(final PrintWriter writer, final AttributeTypeDefinition d, final String type, final boolean addSetter) {
        writer.println();
        writer.println();
        writer.println();
        final String attrName = d.getNameOrOID();
    ...
    private static void writeToString(final PrintWriter writer, final String className, final Collection<AttributeTypeDefinition> requiredAttrs, final Collection<AttributeTypeDefinition> optionalAttrs, final Collection<AttributeTypeDefinition> operationalAttrs) {
        writer.println();
        writer.println();
        writer.println();
        writer.println("  /**");
    ...
    private String getJavaType(final Schema schema, final AttributeTypeDefinition d) {
        if (!d.isSingleValued()) {
        ...
        final int bracePos = syntaxOID.indexOf(123);
        String oid;
        if (bracePos > 0) {
    ...
    static {
        EMPTY_TREE_SET = new TreeSet<String>();
    }
}
