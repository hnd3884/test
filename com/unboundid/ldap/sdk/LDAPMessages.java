...
enum LDAPMessages
{
    ...
    public String get() {
        String s = LDAPMessages.MESSAGE_STRINGS.get(this);
        if (s == null) {
            if (LDAPMessages.RESOURCE_BUNDLE == null) {
                s = this.defaultText;
            }
            else {
                try {
                    s = LDAPMessages.RESOURCE_BUNDLE.getString(this.name());
                }
                catch (final Exception e) {
                    s = this.defaultText;
                }
                LDAPMessages.MESSAGE_STRINGS.putIfAbsent(this, s);
            }
        }
        if (LDAPMessages.IS_WITHIN_UNIT_TESTS && (s.contains("{0}") || s.contains("{0,number,0}") || s.contains("{1}") || s.contains("{1,number,0}") || s.contains("{2}") || s.contains("{2,number,0}") || s.contains("{3}") || s.contains("{3,number,0}") || s.contains("{4}") || s.contains("{4,number,0}") || s.contains("{5}") || s.contains("{5,number,0}") || s.contains("{6}") || s.contains("{6,number,0}") || s.contains("{7}") || s.contains("{7,number,0}") || s.contains("{8}") || s.contains("{8,number,0}") || s.contains("{9}") || s.contains("{9,number,0}") || s.contains("{10}") || s.contains("{10,number,0}"))) {
    ...
    public String get(final Object... args) {
        MessageFormat f = LDAPMessages.MESSAGES.get(this);
        if (f == null) {
            if (LDAPMessages.RESOURCE_BUNDLE == null) {
    ...
    static {
        ...
        RESOURCE_BUNDLE = rb;
        MESSAGE_STRINGS = new ConcurrentHashMap<LDAPMessages, String>(100);
        MESSAGES = new ConcurrentHashMap<LDAPMessages, MessageFormat>(100);
    }
}
