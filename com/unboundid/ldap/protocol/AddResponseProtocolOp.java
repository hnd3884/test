...
public final class AddResponseProtocolOp extends GenericResponseProtocolOp
{
    ...
    public AddResponseProtocolOp(final int resultCode, final String matchedDN, final String diagnosticMessage, final List<String> referralURLs) {
        super((byte)105, resultCode, matchedDN, diagnosticMessage, referralURLs);
    }
    
    public AddResponseProtocolOp(final LDAPResult result) {
        super((byte)105, result.getResultCode().intValue(), result.getMatchedDN(), result.getDiagnosticMessage(), StaticUtils.toList(result.getReferralURLs()));
    }
    
    AddResponseProtocolOp(final ASN1StreamReader reader) throws LDAPException {
        super(reader);
    }
    ...
    public ASN1Element encodeProtocolOp() {
        ...
        return new ASN1Sequence((byte)105, elements);
    }
    
    public static AddResponseProtocolOp decodeProtocolOp(final ASN1Element element) throws LDAPException {
        try {
            ...
            return new AddResponseProtocolOp(resultCode, matchedDN, diagnosticMessage, referralURLs);
        }
        catch (final Exception e2) {
            Debug.debugException(e2);
            throw new LDAPException(ResultCode.DECODING_ERROR, ProtocolMessages.ERR_ADD_RESPONSE_CANNOT_DECODE.get(StaticUtils.getExceptionMessage(e2)), e2);
        }
    }
}
