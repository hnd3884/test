...
public final class DeleteResponseProtocolOp extends GenericResponseProtocolOp
{
    ...
    public DeleteResponseProtocolOp(final int resultCode, final String matchedDN, final String diagnosticMessage, final List<String> referralURLs) {
        super((byte)107, resultCode, matchedDN, diagnosticMessage, referralURLs);
    }
    
    public DeleteResponseProtocolOp(final LDAPResult result) {
        super((byte)107, result.getResultCode().intValue(), result.getMatchedDN(), result.getDiagnosticMessage(), StaticUtils.toList(result.getReferralURLs()));
    }
    
    DeleteResponseProtocolOp(final ASN1StreamReader reader) throws LDAPException {
        super(reader);
    }
    ...
    public ASN1Element encodeProtocolOp() {
        ...
        return new ASN1Sequence((byte)107, elements);
    }
    
    public static DeleteResponseProtocolOp decodeProtocolOp(final ASN1Element element) throws LDAPException {
        try {
            ...
            return new DeleteResponseProtocolOp(resultCode, matchedDN, diagnosticMessage, referralURLs);
        }
        catch (final Exception e2) {
            Debug.debugException(e2);
            throw new LDAPException(ResultCode.DECODING_ERROR, ProtocolMessages.ERR_DELETE_RESPONSE_CANNOT_DECODE.get(StaticUtils.getExceptionMessage(e2)), e2);
        }
    }
}
