...
public final class ModifyDNResponseProtocolOp extends GenericResponseProtocolOp
{
    ...
    public ModifyDNResponseProtocolOp(final int resultCode, final String matchedDN, final String diagnosticMessage, final List<String> referralURLs) {
        super((byte)109, resultCode, matchedDN, diagnosticMessage, referralURLs);
    }
    
    public ModifyDNResponseProtocolOp(final LDAPResult result) {
        super((byte)109, result.getResultCode().intValue(), result.getMatchedDN(), result.getDiagnosticMessage(), StaticUtils.toList(result.getReferralURLs()));
    }
    
    ModifyDNResponseProtocolOp(final ASN1StreamReader reader) throws LDAPException {
        super(reader);
    }
    ...
    public ASN1Element encodeProtocolOp() {
        ...
        return new ASN1Sequence((byte)109, elements);
    }
    
    public static ModifyDNResponseProtocolOp decodeProtocolOp(final ASN1Element element) throws LDAPException {
        try {
            ...
            return new ModifyDNResponseProtocolOp(resultCode, matchedDN, diagnosticMessage, referralURLs);
        }
        catch (final Exception e2) {
            Debug.debugException(e2);
            throw new LDAPException(ResultCode.DECODING_ERROR, ProtocolMessages.ERR_MODIFY_DN_RESPONSE_CANNOT_DECODE.get(StaticUtils.getExceptionMessage(e2)), e2);
        }
    }
}
