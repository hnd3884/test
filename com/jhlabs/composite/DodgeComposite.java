...
{
    public DodgeComposite(final float alpha) {
        super(alpha);
    }
    ...
    static class Context extends RGBCompositeContext
    {
        ...
        public void composeRGB(final int[] src, final int[] dst, final float alpha) {
            for (int w = src.length, i = 0; i < w; i += 4) {
                ...
                final int dor = RGBCompositeContext.clamp((sr << 8) / (256 - dir));
                final int dog = RGBCompositeContext.clamp((sg << 8) / (256 - dig));
                final int dob = RGBCompositeContext.clamp((sb << 8) / (256 - dib));
