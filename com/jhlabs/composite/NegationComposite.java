...
{
    public NegationComposite(final float alpha) {
        super(alpha);
    }
    ...
    static class Context extends RGBCompositeContext
    {
        ...
        public void composeRGB(final int[] src, final int[] dst, final float alpha) {
            for (int w = src.length, i = 0; i < w; i += 4) {
                ...
                final int dor = 255 - Math.abs(255 - sr - dir);
                final int dog = 255 - Math.abs(255 - sg - dig);
                final int dob = 255 - Math.abs(255 - sb - dib);
