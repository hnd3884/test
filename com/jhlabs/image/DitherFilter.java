...
public class DitherFilter extends PointFilter
{
    ...
    public DitherFilter() {
        ...
        this.rows = 2;
        this.cols = 2;
        this.matrix = DitherFilter.ditherMagic4x4Matrix;
        this.levels = 6;
    ...
    protected void initialize() {
        final int n = (int)Math.sqrt(this.matrix.length);
        this.cols = n;
        this.rows = n;
        ...
        final int rc = this.rows * this.cols + 1;
        for (int j = 0; j < 256; ++j) {
    ...
    public int filterRGB(final int x, final int y, final int rgb) {
        ...
        int g = rgb >> 8 & 0xFF;
        int b = rgb & 0xFF;
        final int col = x % this.cols;
        final int row = y % this.rows;
        final int v = this.matrix[row * this.cols + col];
        if (this.colorDither) {
            ...
            g = (r = (b = this.map[(this.mod[value] > v) ? (this.div[value] + 1) : this.div[value]]));
        }
        return a | r << 16 | g << 8 | b;
    }
    ...
    static {
        ditherMagic2x2Matrix = new int[] { 0, 2, 3, 1 };
