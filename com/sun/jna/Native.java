...
public final class Native
{
    ...
    private static boolean deleteNativeLibrary() {
        ...
        try {
            ...
            final Iterator i = libs.iterator();
            while (i.hasNext()) {
            ...
            try {
                ...
                for (int i = 0; i < fields.length; ++i) {
                    ...
                    if (field.getType() == cls && Modifier.isStatic(field.getModifiers())) {
                        Native.libraries.put(cls, new WeakReference(field.get(null)));
                        break;
                    }
                }
            }
        ...
        synchronized (Native.libraries) {
            ...
            if (!Native.options.containsKey(interfaceClass)) {
                try {
                    ...
                    throw new IllegalArgumentException("OPTIONS must be a public field of type java.util.Map (" + e2 + "): " + interfaceClass);
                }
            }
        ...
        synchronized (Native.libraries) {
            ...
            if (!Native.typeMappers.containsKey(interfaceClass)) {
                try {
                    ...
                    throw new IllegalArgumentException("TYPE_MAPPER must be a public field of type " + TypeMapper.class.getName() + " (" + e2 + "): " + interfaceClass);
                }
            }
        ...
        synchronized (Native.libraries) {
            ...
            if (!Native.alignments.containsKey(interfaceClass)) {
                try {
                    ...
                    throw new IllegalArgumentException("STRUCTURE_ALIGNMENT must be a public field of type int (" + e2 + "): " + interfaceClass);
                }
            }
    ...
    private static void loadNativeLibraryFromJar() {
        ...
        if (url.getProtocol().toLowerCase().equals("file")) {
            ...
            try {
                ...
                throw new Error("Failed to create temporary file for jnidispatch library: " + e2);
            }
            finally {
                try {
                    is.close();
    ...
    private static int getConversion(Class type, final TypeMapper mapper) {
        ...
        if (Buffer.class.isAssignableFrom((Class)type)) {
            return 5;
        }
    ...
    private static void cacheOptions(final Class cls, final Map libOptions, final Object proxy) {
        synchronized (Native.libraries) {
            ...
            if (proxy != null) {
                Native.libraries.put(cls, new WeakReference(proxy));
            }
