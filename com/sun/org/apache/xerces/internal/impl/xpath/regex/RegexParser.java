...
class RegexParser
{
    ...
    protected final boolean isSet(final int flag) {
    ...
    synchronized Token parse(final String regex, final int options) throws ParseException {
        ...
        if (this.parenCount < 0) {
            throw this.ex("parser.factor.0", this.offset);
        }
    ...
    Token processCondition() throws ParseException {
        ...
        if (49 <= ch && ch <= 57) {
            refno = ch - 48;
            int finalRefno;
            if (this.parennumber <= (finalRefno = refno)) {
    ...
    Token processBackreference() throws ParseException {
        int finalRefnum;
        int refnum = finalRefnum = this.chardata - 48;
        ...
        final Token tok = Token.createBackReference(finalRefnum);
        this.hasBackReferences = true;
        if (this.references == null) {
            this.references = new Vector();
        }
    ...
    protected static final void addCaseInsensitiveChar(final RangeToken tok, final int c) {
    ...
    protected static final void addCaseInsensitiveCharRange(final RangeToken tok, final int start, final int end) {
        int r1;
        int r2;
        if (start <= end) {
            r1 = start;
        ...
        for (int ch = r1; ch <= r2; ++ch) {
            final int[] caseMap = CaseInsensitiveMap.get(ch);
            if (caseMap != null) {
                for (int i = 0; i < caseMap.length; i += 2) {
                    tok.addRange(caseMap[i], caseMap[i]);
                }
