...
public final class XalanConstants
{
    ...
    public static final boolean IS_JDK8_OR_ABOVE;
    
    public static boolean isJavaVersionAtLeast(final int compareTo) {
        final String javaVersion = SecuritySupport.getSystemProperty("java.version");
        final String[] versions = javaVersion.split("\\.", 3);
        return Integer.parseInt(versions[0]) >= compareTo || Integer.parseInt(versions[1]) >= compareTo;
    }
    
    static {
        IS_JDK8_OR_ABOVE = isJavaVersionAtLeast(8);
    }
}
