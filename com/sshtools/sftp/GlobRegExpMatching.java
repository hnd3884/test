...
public class GlobRegExpMatching implements RegularExpressionMatching
{
    public String[] matchFileNamesWithPattern(final File[] array, final String s) throws SshException {
        ...
        Pattern compile;
        try {
            compile = ((PatternCompiler)globCompiler).compile(s);
        }
        ...
        for (int i = 0; i < array.length; ++i) {
            if (!array[i].getName().equals(".") && !array[i].getName().equals("..") && ((PatternMatcher)perl5Matcher).matches(array[i].getName(), compile)) {
                vector.addElement(array[i].getAbsolutePath());
            }
        }
