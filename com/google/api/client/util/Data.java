...
public class Data
{
    ...
    public static <T> T nullOf(final Class<T> objClass) {
        Object result = Data.NULL_CACHE.get(objClass);
        if (result == null) {
            final Object newValue = createNullInstance(objClass);
            result = Data.NULL_CACHE.putIfAbsent(objClass, newValue);
            if (result == null) {
                result = newValue;
            }
        }
    ...
    private static Object createNullInstance(final Class<?> objClass) {
        if (objClass.isArray()) {
            int dims = 0;
        ...
        if (objClass.isEnum()) {
            ...
            final Enum e = fieldInfo.enumValue();
            return e;
        }
    ...
    public static Object parsePrimitiveValue(final Type type, final String stringValue) {
        final Class<?> primitiveClass = (type instanceof Class) ? ((Class)type) : null;
    ...
    public static Collection<Object> newCollectionInstance(Type type) {
        ...
        final Class<?> collectionClass = (type instanceof Class) ? ((Class)type) : null;
