...
class DefaultCredentialProvider extends SystemEnvironmentProvider
{
    ...
    private boolean runningUsingEnvironmentVariable() throws IOException {
        ...
        catch (final AccessControlException expected) {
            return false;
        }
    }
    ...
    private boolean useGAEStandardAPI() {
        ...
        catch (final IllegalArgumentException exception3) {
            cause = exception3;
        }
        catch (final IllegalAccessException exception4) {
            cause = exception4;
        }
        catch (final NoSuchMethodException exception5) {
            cause = exception5;
        }
        catch (final InvocationTargetException exception6) {
            cause = exception6;
        }
    ...
    private static class ComputeGoogleCredential extends GoogleCredential
    {
        ...
        protected TokenResponse executeRefreshToken() throws IOException {
            ...
            if (statusCode == 200) {
                ...
                return (TokenResponse)parser.parseAndClose(content, response.getContentCharset(), (Class)TokenResponse.class);
            }
            else {
                if (statusCode == 404) {
