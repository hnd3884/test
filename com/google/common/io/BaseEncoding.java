    ...
    private static final class Alphabet
    {
        ...
        Alphabet(final String name, final char[] chars) {
            ...
            try {
                ...
                final String s = "Illegal alphabet ";
                final String value = String.valueOf(new String(chars));
                throw new IllegalArgumentException((value.length() != 0) ? s.concat(value) : new String(s), e2);
            }
        ...
        int decode(final char ch) throws DecodingException {
            if (ch > '\u007f') {
                final String s = "Unrecognized character: 0x";
                final String value = String.valueOf(Integer.toHexString(ch));
                throw new DecodingException((value.length() != 0) ? s.concat(value) : new String(s));
            }
            ...
            if (ch <= ' ' || ch == '\u007f') {
                final String s2 = "Unrecognized character: 0x";
                final String value2 = String.valueOf(Integer.toHexString(ch));
                throw new DecodingException((value2.length() != 0) ? s2.concat(value2) : new String(s2));
            }
