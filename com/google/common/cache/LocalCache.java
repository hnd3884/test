...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>
{
    ...
    LocalCache(final CacheBuilder<? super K, ? super V> builder, final CacheLoader<? super K, V> loader) {
            ...
            if (!keysToLoad.isEmpty()) {
                try {
                    ...
                    for (final K key2 : keysToLoad) {
                        ...
                        if (value2 == null) {
                            final String value3 = String.valueOf(key2);
    ...
    Map<K, V> loadAll(final Set<? extends K> keys, final CacheLoader<? super K, V> loader) throws ExecutionException {
        ...
        if (result == null) {
            this.globalStatsCounter.recordLoadException(stopwatch.elapsed(TimeUnit.NANOSECONDS));
            final String value2 = String.valueOf(loader);
            throw new CacheLoader.InvalidCacheLoadException(new StringBuilder(31 + String.valueOf(value2).length()).append(value2).append(" returned null map from loadAll").toString());
        }
        stopwatch.stop();
        ...
        if (nullsPresent) {
            this.globalStatsCounter.recordLoadException(stopwatch.elapsed(TimeUnit.NANOSECONDS));
            final String value3 = String.valueOf(loader);
    ...
    ReferenceEntry<K, V> getEntry(final Object key) {
        if (key == null) {
            return null;
        }
        final int hash = this.hash(key);
    ...
    boolean removeIf(final BiPredicate<? super K, ? super V> filter) {
        ...
        for (final K key : this.keySet()) {
            while (true) {
                ...
                if (!filter.test((Object)key, (Object)value)) {
                    break;
                }
    ...
    static class Segment<K, V> extends ReentrantLock
    {
        ...
        V waitForLoadingValue(final ReferenceEntry<K, V> e, final K key, final ValueReference<K, V> valueReference) throws ExecutionException {
            ...
            try {
                ...
                if (value == null) {
                    ...
                    throw new CacheLoader.InvalidCacheLoadException(new StringBuilder(35 + String.valueOf(value2).length()).append("CacheLoader returned null for key ").append(value2).append(".").toString());
                }
                final long now = this.map.ticker.read();
                this.recordRead(e, now);
                return value;
        ...
        V compute(final K key, final int hash, final BiFunction<? super K, ? super V, ? extends V> function) {
            ...
            try {
                ...
                for (first = (e = table.get(index)); e != null; e = e.getNext()) {
                    final K entryKey = e.getKey();
                    if (e.getHash() == hash && entryKey != null && this.map.keyEquivalence.equivalent(key, entryKey)) {
                        valueReference = e.getValueReference();
                        if (this.map.isExpired(e, now)) {
                ...
                computingValueReference = new ComputingValueReference<K, V>(valueReference);
                if (e == null) {
                    createNewEntry = true;
                    e = this.newEntry(key, hash, first);
                ...
                final V newValue = computingValueReference.compute(key, function);
                if (newValue != null) {
                    ...
                    try {
                        return (V)this.getAndRecordStats(key, hash, (LoadingValueReference<K, Object>)computingValueReference, (ListenableFuture<Object>)Futures.immediateFuture(newValue));
                    }
        ...
        @GuardedBy("this")
        void enqueueNotification(final K key, final int hash, final V value, final int weight, final RemovalCause cause) {
            this.totalWeight -= weight;
            if (cause.wasEvicted()) {
                this.statsCounter.recordEviction();
            }
            if (this.map.removalNotificationQueue != LocalCache.DISCARDING_QUEUE) {
                final RemovalNotification<K, V> notification = RemovalNotification.create(key, value, cause);
        ...
        @GuardedBy("this")
        ReferenceEntry<K, V> removeValueFromChain(final ReferenceEntry<K, V> first, final ReferenceEntry<K, V> entry, final K key, final int hash, final V value, final ValueReference<K, V> valueReference, final RemovalCause cause) {
    ...
    static class LoadingValueReference<K, V> implements ValueReference<K, V>
    {
        ...
        public ListenableFuture<V> loadFuture(final K key, final CacheLoader<? super K, V> loader) {
            try {
                ...
                return Futures.transform(newValue2, (com.google.common.base.Function<? super V, ? extends V>)new com.google.common.base.Function<V, V>() {
                    @Override
        ...
        public V compute(final K key, final BiFunction<? super K, ? super V, ? extends V> function) {
            ...
            V newValue;
            try {
                newValue = (V)function.apply((Object)key, (Object)previousValue);
            }
    ...
    {
        ComputingValueReference(final ValueReference<K, V> oldValue) {
            super(oldValue);
        }
