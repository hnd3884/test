...
public abstract class UnicodeEscaper extends Escaper
{
    ...
    protected static int codePointAt(final CharSequence seq, int index, final int end) {
        ...
        if (c1 > '\udbff') {
            ...
            throw new IllegalArgumentException(new StringBuilder(88 + String.valueOf(value).length()).append("Unexpected low surrogate character '").append(c1).append("' with value ").append((int)c3).append(" at index ").append(n).append(" in '").append(value).append("'").toString());
        }
        if (index == end) {
            return -c1;
        }
        ...
        throw new IllegalArgumentException(new StringBuilder(89 + String.valueOf(value2).length()).append("Expected low surrogate but got char '").append(c2).append("' with value ").append((int)c4).append(" at index ").append(n2).append(" in '").append(value2).append("'").toString());
    }
