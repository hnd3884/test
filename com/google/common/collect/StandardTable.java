...
class StandardTable<R, C, V> extends AbstractTable<R, C, V> implements Serializable
{
    ...
    private Map<R, V> removeColumn(@CheckForNull final Object column) {
        ...
        while (iterator.hasNext()) {
            ...
            final V value = (V)entry.getValue().remove(column);
            if (value != null) {
    ...
    Spliterator<Table.Cell<R, C, V>> cellSpliterator() {
        return CollectSpliterators.flatMap(this.backingMap.entrySet().spliterator(), rowEntry -> CollectSpliterators.map((Spliterator<Object>)rowEntry.getValue().entrySet().spliterator(), columnEntry -> Tables.immutableCell(rowEntry.getKey(), columnEntry.getKey(), columnEntry.getValue())), 65, this.size());
    }
