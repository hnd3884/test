...
public final class HostAndPort implements Serializable
{
    ...
    public static HostAndPort fromString(final String hostPortString) {
        ...
        if (!Strings.isNullOrEmpty(portString)) {
            Preconditions.checkArgument(!portString.startsWith("+") && CharMatcher.ascii().matchesAllOf(portString), "Unparseable port number: %s", hostPortString);
            try {
    ...
    private static String[] getHostAndPortFromBracketedHost(final String hostPortString) {
        ...
        final int colonIndex = hostPortString.indexOf(58);
        final int closeBracketIndex = hostPortString.lastIndexOf(93);
