...
final class BitMatrixParser
{
    ...
    BitMatrixParser(final BitMatrix bitMatrix) {
        this.bitMatrix = bitMatrix;
    }
    
    byte[] readCodewords() {
        final byte[] result = new byte[144];
        final int height = this.bitMatrix.getHeight();
        final int width = this.bitMatrix.getWidth();
        for (int y = 0; y < height; ++y) {
            final int[] bitnrRow = BitMatrixParser.BITNR[y];
            for (int x = 0; x < width; ++x) {
                final int bit = bitnrRow[x];
                if (bit >= 0 && this.bitMatrix.get(x, y)) {
                    final byte[] array = result;
                    final int n = bit / 6;
                    array[n] |= (byte)(1 << 5 - bit % 6);
                }
            }
        }
        return result;
