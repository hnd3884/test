...
public final class ExpandedProductResultParser extends ResultParser
{
    ...
    public ExpandedProductParsedResult parse(final Result result) {
        ...
        final Map<String, String> uncommonAIs = new HashMap<String, String>();
        int i = 0;
        while (i < rawText.length()) {
            final String ai = findAIvalue(i, rawText);
            if (ai == null) {
                return null;
            }
            i += ai.length() + 2;
            final String value = findValue(i, rawText);
            i += value.length();
            if ("00".equals(ai)) {
                sscc = value;
            }
    ...
    private static String findAIvalue(final int i, final String rawText) {
        final StringBuilder buf = new StringBuilder();
        final char c = rawText.charAt(i);
        if (c != '(') {
            return null;
        }
        final String rawTextAux = rawText.substring(i + 1);
        for (int index = 0; index < rawTextAux.length(); ++index) {
            final char currentChar = rawTextAux.charAt(index);
            if (currentChar == ')') {
                return buf.toString();
            }
    ...
    private static String findValue(final int i, final String rawText) {
        final StringBuilder buf = new StringBuilder();
        final String rawTextAux = rawText.substring(i);
        for (int index = 0; index < rawTextAux.length(); ++index) {
            final char c = rawTextAux.charAt(index);
            if (c == '(') {
