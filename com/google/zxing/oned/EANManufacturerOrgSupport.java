...
final class EANManufacturerOrgSupport
{
    ...
    String lookupCountryIdentifier(final String productCode) {
        this.initIfNeeded();
        final int prefix = Integer.parseInt(productCode.substring(0, 3));
        for (int max = this.ranges.size(), i = 0; i < max; ++i) {
            final int[] range = this.ranges.get(i);
            final int start = range[0];
            if (prefix < start) {
                return null;
            }
            final int end = (range.length == 1) ? start : range[1];
            if (prefix <= end) {
    ...
    private void add(final int[] range, final String id) {
