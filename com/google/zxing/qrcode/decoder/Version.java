...
public final class Version
{
    ...
    private Version(final int versionNumber, final int[] alignmentPatternCenters, final ECBlocks... ecBlocks) {
        this.versionNumber = versionNumber;
        this.alignmentPatternCenters = alignmentPatternCenters;
        this.ecBlocks = ecBlocks;
        int total = 0;
        final int ecCodewords = ecBlocks[0].getECCodewordsPerBlock();
        final ECB[] arr$;
        final ECB[] ecbArray = arr$ = ecBlocks[0].getECBlocks();
    ...
    public ECBlocks getECBlocksForLevel(final ErrorCorrectionLevel ecLevel) {
        return this.ecBlocks[ecLevel.ordinal()];
    }
    
    public static Version getProvisionalVersionForDimension(final int dimension) throws FormatException {
        if (dimension % 4 != 1) {
    ...
    public static Version getVersionForNumber(final int versionNumber) {
    ...
    static Version decodeVersionInformation(final int versionBits) {
        int bestDifference = Integer.MAX_VALUE;
        int bestVersion = 0;
        for (int i = 0; i < Version.VERSION_DECODE_INFO.length; ++i) {
            final int targetVersion = Version.VERSION_DECODE_INFO[i];
            if (targetVersion == versionBits) {
                return getVersionForNumber(i + 7);
            }
            final int bitsDifference = FormatInformation.numBitsDiffering(versionBits, targetVersion);
    ...
    BitMatrix buildFunctionPattern() {
        final int dimension = this.getDimensionForVersion();
        final BitMatrix bitMatrix = new BitMatrix(dimension);
        bitMatrix.setRegion(0, 0, 9, 9);
        bitMatrix.setRegion(dimension - 8, 0, 8, 9);
        bitMatrix.setRegion(0, dimension - 8, 9, 8);
        for (int max = this.alignmentPatternCenters.length, x = 0; x < max; ++x) {
            final int i = this.alignmentPatternCenters[x] - 2;
            for (int y = 0; y < max; ++y) {
                if (x != 0 || (y != 0 && y != max - 1)) {
                    if (x != max - 1 || y != 0) {
    ...
    public static final class ECBlocks
    {
        ...
        ECBlocks(final int ecCodewordsPerBlock, final ECB... ecBlocks) {
        ...
        public int getNumBlocks() {
            int total = 0;
            for (final ECB ecBlock : this.ecBlocks) {
    ...
    public static final class ECB
    {
        ...
        ECB(final int count, final int dataCodewords) {
            this.count = count;
