...
public class DefaultHttp2FrameReader implements Http2FrameReader, Http2FrameSizePolicy, Configuration
{
    ...
    private void processPayloadState(final ChannelHandlerContext ctx, final ByteBuf in, final Http2FrameListener listener) throws Http2Exception {
        ...
        final int payloadEndIndex = in.readerIndex() + this.payloadLength;
        this.readingHeaders = true;
        switch (this.frameType) {
            case 0: {
    ...
    private void readDataFrame(final ChannelHandlerContext ctx, final ByteBuf payload, final int payloadEndIndex, final Http2FrameListener listener) throws Http2Exception {
        final int padding = this.readPadding(payload);
        this.verifyPadding(padding);
        final int dataLength = lengthWithoutTrailingPadding(payloadEndIndex - payload.readerIndex(), padding);
        final ByteBuf data = payload.readSlice(dataLength);
        listener.onDataRead(ctx, this.streamId, data, padding, this.flags.endOfStream());
    }
    
    private void readHeadersFrame(final ChannelHandlerContext ctx, final ByteBuf payload, final int payloadEndIndex, final Http2FrameListener listener) throws Http2Exception {
        ...
        if (!this.flags.priorityPresent()) {
            this.headersContinuation = new HeadersContinuation() {
                ...
                public void processFragment(final boolean endOfHeaders, final ByteBuf fragment, final int len, final Http2FrameListener listener) throws Http2Exception {
        ...
        }).processFragment(this.flags.endOfHeaders(), payload, lenToRead, listener);
        this.resetHeadersContinuationIfEnd(this.flags.endOfHeaders());
    }
    ...
    private void readPushPromiseFrame(final ChannelHandlerContext ctx, final ByteBuf payload, final int payloadEndIndex, final Http2FrameListener listener) throws Http2Exception {
    ...
    private static void readGoAwayFrame(final ChannelHandlerContext ctx, final ByteBuf payload, final int payloadEndIndex, final Http2FrameListener listener) throws Http2Exception {
    ...
    private void readContinuationFrame(final ByteBuf payload, final int payloadEndIndex, final Http2FrameListener listener) throws Http2Exception {
        this.headersContinuation.processFragment(this.flags.endOfHeaders(), payload, payloadEndIndex - payload.readerIndex(), listener);
        this.resetHeadersContinuationIfEnd(this.flags.endOfHeaders());
    }
    
    private void readUnknownFrame(final ChannelHandlerContext ctx, ByteBuf payload, final int payloadEndIndex, final Http2FrameListener listener) throws Http2Exception {
    ...
    private abstract class HeadersContinuation
    {
        ...
        abstract void processFragment(final boolean p0, final ByteBuf p1, final int p2, final Http2FrameListener p3) throws Http2Exception;
    ...
    protected class HeadersBlockBuilder
    {
        ...
        final void addFragment(final ByteBuf fragment, final int len, final ByteBufAllocator alloc, final boolean endOfHeaders) throws Http2Exception {
            ...
            if (this.headerBlock.isWritable(len)) {
                ...
                final ByteBuf buf = alloc.buffer(this.headerBlock.readableBytes() + len);
