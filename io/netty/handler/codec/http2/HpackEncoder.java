...
final class HpackEncoder
{
    ...
    HpackEncoder(final boolean ignoreMaxHeaderListSize, final int arraySizeHint, final int huffCodeThreshold) {
    ...
    private void encodeHeader(final ByteBuf out, final CharSequence name, final CharSequence value, final boolean sensitive, final long headerSize) {
        ...
        if (this.maxHeaderTableSize == 0L) {
            final int staticTableIndex = HpackStaticTable.getIndexInsensitive(name, value);
        ...
        final HeaderEntry headerField = this.getEntryInsensitive(name, value);
        if (headerField != null) {
            final int index = this.getIndex(headerField.index) + HpackStaticTable.length;
            encodeInteger(out, 128, 7, index);
        }
        else {
            final int staticTableIndex2 = HpackStaticTable.getIndexInsensitive(name, value);
    ...
    private HeaderEntry getEntryInsensitive(final CharSequence name, final CharSequence value) {
