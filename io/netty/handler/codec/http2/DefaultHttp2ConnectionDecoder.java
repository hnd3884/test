...
public class DefaultHttp2ConnectionDecoder implements Http2ConnectionDecoder
{
    ...
    private final class FrameReadListener implements Http2FrameListener
    {
        ...
        public void onHeadersRead(final ChannelHandlerContext ctx, final int streamId, final Http2Headers headers, final int streamDependency, final short weight, final boolean exclusive, final int padding, final boolean endOfStream) throws Http2Exception {
            ...
            if (!isTrailers) {
                final List<? extends CharSequence> contentLength = ((Headers<AsciiString, ? extends CharSequence, T>)headers).getAll(HttpHeaderNames.CONTENT_LENGTH);
                if (contentLength != null && !contentLength.isEmpty()) {
                    try {
                        final long cLength = HttpUtil.normalizeAndGetContentLength(contentLength, false, true);
                        if (cLength != -1L) {
                            ((Headers<AsciiString, Object, Headers>)headers).setLong(HttpHeaderNames.CONTENT_LENGTH, cLength);
