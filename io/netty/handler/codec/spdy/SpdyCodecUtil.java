...
final class SpdyCodecUtil
{
    ...
    static void validateHeaderName(final CharSequence name) {
        ...
        if (name.length() > 65535) {
            throw new IllegalArgumentException("name exceeds allowable length: " + (Object)name);
        }
        for (int i = 0; i < name.length(); ++i) {
            final char c = name.charAt(i);
            if (c == '\0') {
                throw new IllegalArgumentException("name contains null character: " + (Object)name);
            }
            if (c >= 'A' && c <= 'Z') {
                throw new IllegalArgumentException("name must be all lower case.");
            }
            if (c > '\u007f') {
                throw new IllegalArgumentException("name contains non-ascii character: " + (Object)name);
            }
        }
    }
    ...
    static void validateHeaderValue(final CharSequence value) {
        ...
        for (int i = 0; i < value.length(); ++i) {
            ...
            if (c == '\0') {
                throw new IllegalArgumentException("value contains null character: " + (Object)value);
            }
        }
    }
