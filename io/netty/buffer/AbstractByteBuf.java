...
public abstract class AbstractByteBuf extends ByteBuf
{
    ...
    protected final void trimIndicesToCapacity(final int newCapacity) {
    ...
    final void ensureWritable0(final int minWritableBytes) {
        ...
        if (AbstractByteBuf.checkBounds && (targetCapacity < 0 || targetCapacity > this.maxCapacity)) {
            this.ensureAccessible();
        ...
        final int fastWritable = this.maxFastWritableBytes();
        final int newCapacity = (fastWritable >= minWritableBytes) ? (writerIndex + fastWritable) : this.alloc().calculateNewCapacity(targetCapacity, this.maxCapacity);
    ...
    public int ensureWritable(final int minWritableBytes, final boolean force) {
        ...
        if (minWritableBytes <= maxCapacity - writerIndex) {
            final int fastWritable = this.maxFastWritableBytes();
            final int newCapacity = (fastWritable >= minWritableBytes) ? (writerIndex + fastWritable) : this.alloc().calculateNewCapacity(writerIndex + minWritableBytes, maxCapacity);
            this.capacity(newCapacity);
            return 2;
        }
    ...
    public ByteBuf readBytes(final ByteBuf dst, final int length) {
        if (AbstractByteBuf.checkBounds && length > dst.writableBytes()) {
    ...
    int forEachByteAsc0(int start, final int end, final ByteProcessor processor) throws Exception {
        while (start < end) {
    ...
    int forEachByteDesc0(int rStart, final int rEnd, final ByteProcessor processor) throws Exception {
        while (rStart >= rEnd) {
    ...
    protected final void checkDstIndex(final int length, final int dstIndex, final int dstCapacity) {
        this.checkReadableBytes(length);
        if (AbstractByteBuf.checkBounds) {
            checkRangeBounds("dstIndex", dstIndex, length, dstCapacity);
        }
    ...
    protected final void checkNewCapacity(final int newCapacity) {
        this.ensureAccessible();
        if (AbstractByteBuf.checkBounds && (newCapacity < 0 || newCapacity > this.maxCapacity())) {
    ...
    private void checkReadableBytes0(final int minimumReadableBytes) {
        this.ensureAccessible();
        if (AbstractByteBuf.checkBounds && this.readerIndex > this.writerIndex - minimumReadableBytes) {
    ...
    static {
        ...
        if (AbstractByteBuf.logger.isDebugEnabled()) {
            AbstractByteBuf.logger.debug("-D{}: {}", "io.netty.buffer.checkAccessible", AbstractByteBuf.checkAccessible);
            AbstractByteBuf.logger.debug("-D{}: {}", "io.netty.buffer.checkBounds", AbstractByteBuf.checkBounds);
        }
