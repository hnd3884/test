...
final class PooledUnsafeHeapByteBuf extends PooledHeapByteBuf
{
    ...
    private PooledUnsafeHeapByteBuf(final ObjectPool.Handle<PooledUnsafeHeapByteBuf> recyclerHandle, final int maxCapacity) {
    ...
    public ByteBuf writeZero(final int length) {
        if (PlatformDependent.javaVersion() >= 7) {
            ...
            UnsafeByteBufUtil.setZero((byte[])(Object)this.memory, this.idx(wIndex), length);
    ...
    static {
        RECYCLER = ObjectPool.newPool((ObjectPool.ObjectCreator<PooledUnsafeHeapByteBuf>)new ObjectPool.ObjectCreator<PooledUnsafeHeapByteBuf>() {
            @Override
            public PooledUnsafeHeapByteBuf newObject(final ObjectPool.Handle<PooledUnsafeHeapByteBuf> handle) {
