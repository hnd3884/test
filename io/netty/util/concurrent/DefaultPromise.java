...
public class DefaultPromise<V> extends AbstractFuture<V> implements Promise<V>
{
    ...
    private Throwable cause0(Object result) {
        ...
        if (result == DefaultPromise.CANCELLATION_CAUSE_HOLDER) {
            final CancellationException ce = new LeanCancellationException();
            if (DefaultPromise.RESULT_UPDATER.compareAndSet(this, DefaultPromise.CANCELLATION_CAUSE_HOLDER, new CauseHolder(ce))) {
                return ce;
            }
            result = this.result;
        }
    ...
    public V getNow() {
        final Object result = this.result;
        if (result instanceof CauseHolder || result == DefaultPromise.SUCCESS || result == DefaultPromise.UNCANCELLABLE) {
            return null;
        }
        return (V)result;
    }
    ...
    public V get() throws InterruptedException, ExecutionException {
        ...
        if (result == DefaultPromise.SUCCESS || result == DefaultPromise.UNCANCELLABLE) {
            return null;
        }
        final Throwable cause = this.cause0(result);
        if (cause == null) {
            return (V)result;
        }
    ...
    protected static void notifyListener(final EventExecutor eventExecutor, final Future<?> future, final GenericFutureListener<?> listener) {
        notifyListenerWithStackOverFlowProtection(ObjectUtil.checkNotNull(eventExecutor, "eventExecutor"), ObjectUtil.checkNotNull(future, "future"), ObjectUtil.checkNotNull(listener, "listener"));
    }
    ...
    static {
        ...
        CANCELLATION_CAUSE_HOLDER = new CauseHolder(StacklessCancellationException.newInstance(DefaultPromise.class, "cancel(...)"));
        CANCELLATION_STACK = DefaultPromise.CANCELLATION_CAUSE_HOLDER.cause.getStackTrace();
    }
    ...
    private static final class StacklessCancellationException extends CancellationException
    {
        ...
        static StacklessCancellationException newInstance(final Class<?> clazz, final String method) {
