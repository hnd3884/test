...
public final class AsciiString implements CharSequence, Comparable<CharSequence>
{
    ...
    public AsciiString concat(final CharSequence string) {
        ...
        if (string instanceof AsciiString) {
            ...
            final byte[] newValue = PlatformDependent.allocateUninitializedArray(thisLen + thatLen);
            ...
            final byte[] newValue2 = PlatformDependent.allocateUninitializedArray(thisLen + thatLen);
    ...
    public boolean contentEqualsIgnoreCase(final CharSequence string) {
        ...
        if (string instanceof AsciiString) {
            ...
            for (int i = this.arrayOffset(), j = rhs.arrayOffset(), end = i + this.length(); i < end; ++i, ++j) {
        ...
        int k = this.arrayOffset();
        for (int l = 0, end2 = this.length(); l < end2; ++l) {
    ...
    public int indexOf(final CharSequence subString, int start) {
        ...
        for (int len = this.offset + this.length - subCount, i = start + this.offset; i <= len; ++i) {
    ...
    public int indexOf(final char ch, int start) {
        ...
        for (int len = this.offset + this.length, i = start + this.offset; i < len; ++i) {
    ...
    public AsciiString replace(final char oldChar, final char newChar) {
        ...
        for (int len = this.offset + this.length, i = this.offset; i < len; ++i) {
            if (this.value[i] == oldCharAsByte) {
                final byte[] buffer = PlatformDependent.allocateUninitializedArray(this.length());
                System.arraycopy(this.value, this.offset, buffer, 0, i - this.offset);
                buffer[i - this.offset] = newCharAsByte;
                ++i;
                while (i < len) {
                    final byte oldValue = this.value[i];
    ...
    public AsciiString toLowerCase() {
        ...
        final byte[] newValue = PlatformDependent.allocateUninitializedArray(this.length());
    ...
    public static CharSequence trim(final CharSequence c) {
        ...
        int end;
        int last;
        for (start = 0, last = (end = c.length() - 1); start <= end && c.charAt(start) <= ' '; ++start) {}
    ...
    public static int indexOf(final CharSequence cs, final char searchChar, final int start) {
        ...
        for (int sz = cs.length(), i = (start < 0) ? 0 : start; i < sz; ++i) {
    ...
    private static byte c2b0(final char c) {
        return (byte)c;
    }
