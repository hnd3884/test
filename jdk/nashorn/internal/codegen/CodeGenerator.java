...
final class CodeGenerator extends NodeOperatorVisitor<CodeGeneratorLexicalContext> implements Loggable
{
    ...
    int getCallSiteFlags() {
        return this.lc.getCurrentFunction().getCallSiteFlags() | this.callSiteFlags;
    }
    ...
    private boolean isFastScope(final Symbol symbol) {
        ...
        if (!((CodeGeneratorLexicalContext)this.lc).inDynamicScope()) {
            assert symbol.isGlobal() || this.lc.getDefiningBlock(symbol).needsScope() : symbol.getName();
            return true;
        }
        else {
            if (symbol.isGlobal()) {
    ...
    private MethodEmitter loadSharedScopeVar(final Type valueType, final Symbol symbol, final int flags) {
        ...
        this.method.load(this.getScopeProtoDepth(this.lc.getCurrentBlock(), symbol));
    ...
    private void loadFastScopeProto(final Symbol symbol, final boolean swap) {
        final int depth = this.getScopeProtoDepth(this.lc.getCurrentBlock(), symbol);
        assert depth != -1 : "Couldn't find scope depth for symbol " + symbol.getName() + " in " + this.lc.getCurrentFunction();
        if (depth > 0) {
            if (swap) {
                this.method.swap();
            }
    ...
    public boolean enterBlock(final Block block) {
        ...
        if (this.lc.isFunctionBody() && this.emittedMethods.contains(this.lc.getCurrentFunction().getName())) {
            return false;
        }
    ...
    public Node leaveBlock(final Block block) {
        ...
        assert (this.lc.isFunctionBody() ? 0 : ((CodeGeneratorLexicalContext)this.lc).getUsedSlotCount()) == this.method.getFirstTemp() : "reachable=" + this.method.isReachable() + " isFunctionBody=" + this.lc.isFunctionBody() + " usedSlotCount=" + ((CodeGeneratorLexicalContext)this.lc).getUsedSlotCount() + " firstTemp=" + this.method.getFirstTemp();
        return block;
    }
    
    private void popBlockScope(final Block block) {
        final Label breakLabel = block.getBreakLabel();
        if (!block.needsScope() || this.lc.isFunctionBody()) {
    ...
    private void popScopesUntil(final LexicalContextNode until) {
        this.popScopes(this.lc.getScopeNestingLevelTo(until));
    }
    ...
    private boolean loadCallNode(final CallNode callNode, final TypeBounds resultBounds) {
        ...
        final Block currentBlock = this.lc.getCurrentBlock();
            ...
            private void evalCall(final IdentNode ident, final int flags) {
                ...
                new OptimisticOperation(callNode, resultBounds) {
                    ...
                    void consumeStack() {
                        ...
                        CodeGenerator.this.method.load(CodeGenerator.this.lc.getCurrentFunction().isStrict());
    ...
    private void initLocals(final Block block) {
        ...
        final boolean isFunctionBody = this.lc.isFunctionBody();
                ...
                if (!symbol.isInternal()) {
                    ...
                    if (symbol.isVar()) {
                        ...
                        if (varsInScope || symbol.isScope()) {
                            ...
                            assert symbol.slotCount() == 0 : symbol + " should have a slot only, no scope";
                            continue;
                        }
                    }
                    else {
    ...
    private boolean skipFunction(final FunctionNode functionNode) {
        ...
        return ((onDemand || lazy) && this.lc.getOutermostFunction() != functionNode) || (!onDemand && lazy && env._optimistic_types && functionNode.isProgram());
    }
    ...
    private MethodEmitter loadRegex(final Lexer.RegexToken regexToken) {
        ...
        final String regexName = this.lc.getCurrentFunction().uniqueName(CompilerConstants.REGEX_PREFIX.symbolName());
        final ClassEmitter classEmitter = this.unit.getClassEmitter();
    ...
    public boolean enterReturnNode(final ReturnNode returnNode) {
        ...
        final Type returnType = this.lc.getCurrentFunction().getReturnType();
    ...
    private boolean undefinedCheck(final RuntimeNode runtimeNode, final List<Expression> args) {
        ...
        if (!this.compiler.isGlobalSymbol(this.lc.getCurrentFunction(), "undefined")) {
            return false;
        }
    ...
    public boolean enterSplitReturn(final SplitReturn splitReturn) {
        if (this.method.isReachable()) {
            this.method.loadUndefined(this.lc.getCurrentFunction().getReturnType())._return();
        }
        return false;
    }
    ...
    private Source getCurrentSource() {
        return this.lc.getCurrentFunction().getSource();
    }
    ...
    private void enterForOrWhile(final LoopNode loopNode, final JoinPredecessorExpression modify) {
        ...
        if (loopNode.hasPerIterationScope() && this.lc.getCurrentBlock().needsScope()) {
            this.method.loadCompilerConstant(CompilerConstants.SCOPE);
    ...
    private void loadTernaryNode(final TernaryNode ternaryNode, final TypeBounds resultBounds) {
        ...
        assert Type.generic(this.method.peekType()) == outBounds.narrowest;
    ...
    private void newFunctionObject(final FunctionNode functionNode, final boolean addInitializer) {
        assert this.lc.peek() == functionNode;
        ...
        if (this.lc.getOutermostFunction() == functionNode) {
            return;
        }
    ...
    private abstract class Store<T extends Expression>
    {
        ...
        private IdentNode quickLocalVariable(final Type type) {
            final String name = CodeGenerator.this.lc.getCurrentFunction().uniqueName(CompilerConstants.QUICK_PREFIX.symbolName());
        ...
        MethodEmitter emit(final int ignoredArgCount) {
            ...
            if (this.isOptimistic || currentContinuationEntryPoint) {
                ...
                if (currentContinuationEntryPoint) {
                    ...
                    assert ci != null : "no continuation info found for " + CodeGenerator.this.lc.getCurrentFunction();
