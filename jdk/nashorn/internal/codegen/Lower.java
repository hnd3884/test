...
final class Lower extends NodeOperatorVisitor<BlockLexicalContext> implements Loggable
{
    ...
    private Block catchAllBlock(final TryNode tryNode) {
        ...
        final IdentNode exception = new IdentNode(token, finish, this.lc.getCurrentFunction().uniqueName(CompilerConstants.EXCEPTION_PREFIX.symbolName()));
    ...
    public Node leaveTryNode(final TryNode tryNode) {
        ...
        if (finallyBody == null || finallyBody.getStatementCount() == 0) {
            ...
            return (TryNode)this.lc.replace(tryNode, this.spliceFinally(newTryNode, rethrows.get(0), finallyBody));
        }
    }
    ...
    public Node leaveVarNode(final VarNode varNode) {
        ...
        if (varNode.getFlag(4) && this.lc.getCurrentFunction().isProgram()) {
    ...
    public Node leaveWhileNode(final WhileNode whileNode) {
        ...
        if (Expression.isAlwaysTrue(test)) {
            ...
            this.lc.replace(whileNode, forNode);
            return forNode;
        }
    ...
    private String evalLocation(final IdentNode node) {
        final Source source = this.lc.getCurrentFunction().getSource();
