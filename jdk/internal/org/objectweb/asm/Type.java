...
public class Type
{
    ...
    public static Type getReturnType(final String s) {
        return getType(s.toCharArray(), s.indexOf(41) + 1);
    }
    ...
    private void getDescriptor(final StringBuffer sb) {
        if (this.buf == null) {
    ...
    private static void getDescriptor(final StringBuffer sb, final Class<?> clazz) {
        Class<Integer> componentType;
        for (componentType = (Class<Integer>)clazz; !componentType.isPrimitive(); componentType = componentType.getComponentType()) {
