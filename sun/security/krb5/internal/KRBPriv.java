...
public class KRBPriv
{
    ...
    public KRBPriv(final EncryptedData encPart) {
        this.pvno = 5;
        this.msgType = 21;
        this.encPart = encPart;
    }
    
    public KRBPriv(final byte[] array) throws Asn1Exception, KrbApErrException, IOException {
        this.init(new DerValue(array));
    }
    
    public KRBPriv(final DerValue derValue) throws Asn1Exception, KrbApErrException, IOException {
        this.init(derValue);
    }
    
    private void init(final DerValue derValue) throws Asn1Exception, KrbApErrException, IOException {
        if ((derValue.getTag() & 0x1F) != 0x15 || !derValue.isApplication() || !derValue.isConstructed()) {
