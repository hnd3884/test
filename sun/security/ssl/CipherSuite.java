...
enum CipherSuite
{
    ...
    private CipherSuite(final String s2, final int n2) {
        this(n2, false, s2, "", ProtocolVersion.PROTOCOLS_EMPTY, null, null, null, null);
    }
    
    private CipherSuite(final int n2, final boolean b, final String s2, final ProtocolVersion[] array, final SSLCipher sslCipher, final HashAlg hashAlg) {
        this(n2, b, s2, "", array, null, sslCipher, MacAlg.M_NULL, hashAlg);
    }
    
    private CipherSuite(final int id, final boolean isDefaultEnabled, final String name, final String s2, final ProtocolVersion[] array, final KeyExchange keyExchange, final SSLCipher bulkCipher, final MacAlg macAlg, final HashAlg hashAlg) {
        this.id = id;
        this.isDefaultEnabled = isDefaultEnabled;
        this.name = name;
        if (!s2.isEmpty()) {
            this.aliases = Arrays.asList(s2.split(","));
        }
        else {
        ...
        this.exportable = (bulkCipher != null && bulkCipher.exportable);
    }
    
    static CipherSuite nameOf(final String s) {
        for (final CipherSuite cipherSuite : values()) {
            if (cipherSuite.name.equals(s) || cipherSuite.aliases.contains(s)) {
                return cipherSuite;
            }
        }
        return null;
    ...
    static CipherSuite valueOf(final int n) {
        for (final CipherSuite cipherSuite : values()) {
            if (cipherSuite.id == n) {
                return cipherSuite;
            }
    ...
    static String nameOf(final int n) {
        ...
        return "UNKNOWN-CIPHER-SUITE(" + Utilities.byte16HexString(n) + ")";
    }
    ...
    static List<CipherSuite> validValuesOf(final String[] array) {
        if (array == null) {
    ...
    static String[] namesOf(final List<CipherSuite> list) {
        final String[] array = new String[list.size()];
        int n = 0;
        final Iterator iterator = list.iterator();
        while (iterator.hasNext()) {
            array[n++] = ((CipherSuite)iterator.next()).name;
        }
        return array;
    }
    ...
    int calculatePacketSize(final int n, final ProtocolVersion protocolVersion) {
        int n2 = n;
        if (this.bulkCipher != null && this.bulkCipher != SSLCipher.B_NULL) {
            final int ivSize = this.bulkCipher.ivSize;
            switch (this.bulkCipher.cipherType) {
                case BLOCK_CIPHER: {
                    int n3 = n2 + this.macAlg.size;
                    n2 = ++n3 + (ivSize - n3 % ivSize) % ivSize;
                ...
                case AEAD_CIPHER: {
                    if (protocolVersion == ProtocolVersion.TLS12) {
                        n2 += this.bulkCipher.ivSize - this.bulkCipher.fixedIvSize;
                    }
                    final int n4 = n2;
    ...
    int calculateFragSize(final int n, final ProtocolVersion protocolVersion) {
        int n2 = n - 5;
        if (this.bulkCipher != null && this.bulkCipher != SSLCipher.B_NULL) {
            final int ivSize = this.bulkCipher.ivSize;
            switch (this.bulkCipher.cipherType) {
                case BLOCK_CIPHER: {
                    ...
                    int n3 = n2 - n2 % ivSize;
                    n2 = --n3 - this.macAlg.size;
                    break;
                }
                case AEAD_CIPHER: {
                    final int n4 = n2;
                    this.bulkCipher.getClass();
                    n2 = n4 - 16 - (this.bulkCipher.ivSize - this.bulkCipher.fixedIvSize);
                    break;
                }
                default: {
                    n2 -= this.macAlg.size;
    ...
    enum HashAlg
    {
        ...
        private HashAlg(final String name, final int hashLength, final int blockSize) {
            this.name = name;
