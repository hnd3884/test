...
final class PredefinedDHParameterSpecs
{
    ...
    static {
        ...
        supportedPrimes = new BigInteger[] { PredefinedDHParameterSpecs.p512, PredefinedDHParameterSpecs.p768, PredefinedDHParameterSpecs.p1024, PredefinedDHParameterSpecs.p1536, PredefinedDHParameterSpecs.p2048, PredefinedDHParameterSpecs.p3072, PredefinedDHParameterSpecs.p4096, PredefinedDHParameterSpecs.p6144, PredefinedDHParameterSpecs.p8192 };
        ffdhePrimes = new BigInteger[] { PredefinedDHParameterSpecs.p2048, PredefinedDHParameterSpecs.p3072, PredefinedDHParameterSpecs.p4096, PredefinedDHParameterSpecs.p6144, PredefinedDHParameterSpecs.p8192 };
        ...
        if (s != null && !s.isEmpty()) {
            s = PredefinedDHParameterSpecs.spacesPattern.matcher(s).replaceAll("");
            if (SSLLogger.isOn && SSLLogger.isOn("sslctx")) {
                SSLLogger.fine("The Security Property jdk.tls.server.defaultDHEParameters: " + s, new Object[0]);
            }
        }
        final HashMap hashMap = new HashMap();
        if (s != null && !s.isEmpty()) {
            if (PredefinedDHParameterSpecs.syntaxPattern.matcher(s).matches()) {
                final Matcher matcher = PredefinedDHParameterSpecs.paramsPattern.matcher(s);
                while (matcher.find()) {
                    final String group = matcher.group(1);
                    final BigInteger bigInteger = new BigInteger(group, 16);
                    if (!bigInteger.isProbablePrime(120)) {
                        if (!SSLLogger.isOn || !SSLLogger.isOn("sslctx")) {
                            continue;
                        }
                        SSLLogger.fine("Prime modulus p in Security Property, jdk.tls.server.defaultDHEParameters, is not a prime: " + group, new Object[0]);
                    }
                    else {
                        hashMap.put(bigInteger.bitLength(), new DHParameterSpec(bigInteger, new BigInteger(matcher.group(2), 16)));
                    }
                }
            }
        ...
        for (final BigInteger bigInteger2 : PredefinedDHParameterSpecs.ffdhePrimes) {
            final int bitLength = bigInteger2.bitLength();
            final DHParameterSpec dhParameterSpec = new DHParameterSpec(bigInteger2, value);
            hashMap2.put(bitLength, dhParameterSpec);
            hashMap.putIfAbsent(bitLength, dhParameterSpec);
        }
        for (final BigInteger bigInteger3 : PredefinedDHParameterSpecs.supportedPrimes) {
            final int bitLength2 = bigInteger3.bitLength();
            if (hashMap.get(bitLength2) == null) {
                hashMap.put(bitLength2, new DHParameterSpec(bigInteger3, value));
            }
        }
        ffdheParams = Collections.unmodifiableMap((Map<?, ?>)hashMap2);
        definedParams = Collections.unmodifiableMap((Map<?, ?>)hashMap);
    }
}
