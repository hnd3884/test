...
{
    static final String[] INITIAL_CHARSETS;
    static final String[] LAZY_CHARSETS;
    private static final Charset DUMMY_CHARSET;
    private ConcurrentMap<String, Charset> cache;
    
    public CharsetCache() {
        this.cache = new ConcurrentHashMap<String, Charset>();
        for (final String charsetName : CharsetCache.INITIAL_CHARSETS) {
            final Charset charset = Charset.forName(charsetName);
            this.addToCache(charsetName, charset);
        }
        for (final String charsetName : CharsetCache.LAZY_CHARSETS) {
            this.addToCache(charsetName, CharsetCache.DUMMY_CHARSET);
        }
    }
    
    private void addToCache(final String name, final Charset charset) {
    ...
    public Charset getCharset(final String charsetName) {
        final String lcCharsetName = charsetName.toLowerCase(Locale.ENGLISH);
        Charset result = this.cache.get(lcCharsetName);
        if (result == CharsetCache.DUMMY_CHARSET) {
            final Charset charset = Charset.forName(lcCharsetName);
            if (charset == null) {
    ...
    static {
        INITIAL_CHARSETS = new String[] { "iso-8859-1", "utf-8" };
        ...
        DUMMY_CHARSET = new DummyCharset("Dummy", null);
    }
    
    private static class DummyCharset extends Charset
    {
        protected DummyCharset(final String canonicalName, final String[] aliases) {
