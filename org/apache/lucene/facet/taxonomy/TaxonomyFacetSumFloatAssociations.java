...
{
    public TaxonomyFacetSumFloatAssociations(final TaxonomyReader taxoReader, final FacetsConfig config, final FacetsCollector fc) throws IOException {
        this("$facets", taxoReader, config, fc);
    }
    
    public TaxonomyFacetSumFloatAssociations(final String indexFieldName, final TaxonomyReader taxoReader, final FacetsConfig config, final FacetsCollector fc) throws IOException {
    ...
    private final void sumValues(final List<FacetsCollector.MatchingDocs> matchingDocs) throws IOException {
        for (final FacetsCollector.MatchingDocs hits : matchingDocs) {
            ...
            while ((doc = docs.nextDoc()) != Integer.MAX_VALUE) {
                ...
                float[] values;
                int n;
                for (int end = bytesRef.offset + bytesRef.length, offset = bytesRef.offset; offset < end; offset += 4, value = ((bytes[offset] & 0xFF) << 24 | (bytes[offset + 1] & 0xFF) << 16 | (bytes[offset + 2] & 0xFF) << 8 | (bytes[offset + 3] & 0xFF)), offset += 4, values = this.values, n = ord, values[n] += Float.intBitsToFloat(value)) {
