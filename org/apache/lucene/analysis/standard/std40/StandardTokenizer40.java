...
public final class StandardTokenizer40 extends Tokenizer
{
    ...
    public StandardTokenizer40(final AttributeFactory factory) {
        super(factory);
        this.maxTokenLength = 255;
        this.termAtt = (CharTermAttribute)this.addAttribute((Class)CharTermAttribute.class);
        this.offsetAtt = (OffsetAttribute)this.addAttribute((Class)OffsetAttribute.class);
    ...
    private final void init() {
        this.scanner = new StandardTokenizerImpl40(this.input);
    }
    ...
    public final boolean incrementToken() throws IOException {
        ...
        while (true) {
            ...
            if (this.scanner.yylength() <= this.maxTokenLength) {
                ...
                if (tokenType == 8) {
                    this.typeAtt.setType(StandardTokenizer40.TOKEN_TYPES[5]);
                    this.termAtt.setLength(this.termAtt.length() - 1);
                }
                else {
                    this.typeAtt.setType(StandardTokenizer40.TOKEN_TYPES[tokenType]);
                }
                return true;
            }
