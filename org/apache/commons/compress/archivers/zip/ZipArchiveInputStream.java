...
public class ZipArchiveInputStream extends ArchiveInputStream implements InputStreamStatistics
{
    ...
    public ZipArchiveEntry getNextZipEntry() throws IOException {
        ...
        if (sig.equals(ZipLong.LFH_SIG)) {
            ...
            try {
                ...
                final ZipException z = new ZipException("Invalid extra data in entry " + this.current.entry.getName());
                z.initCause(ex);
                throw z;
            }
    ...
    private void processZip64Extra(final ZipLong size, final ZipLong cSize) throws ZipException {
        final ZipExtraField extra = this.current.entry.getExtraField(Zip64ExtendedInformationExtraField.HEADER_ID);
    ...
    private boolean isApkSigningBlock(final byte[] suspectLocalFileHeader) throws IOException {
        ...
        BigInteger toSkip = len.add(BigInteger.valueOf(8 - suspectLocalFileHeader.length - (long)ZipArchiveInputStream.APK_SIGNING_BLOCK_MAGIC.length));
