...
public class CpioArchiveInputStream extends ArchiveInputStream implements CpioConstants
{
    ...
    private final byte[] readRange(final int len) throws IOException {
        final byte[] b = IOUtils.readRange(this.in, len);
        this.count(b.length);
        if (b.length < len) {
            throw new EOFException();
        }
    ...
    private String readCString(final int length) throws IOException {
        final byte[] tmpBuffer = this.readRange(length - 1);
        if (this.in.read() == -1) {
            throw new EOFException();
        }
