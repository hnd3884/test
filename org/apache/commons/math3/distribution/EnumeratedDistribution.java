...
public class EnumeratedDistribution<T> implements Serializable
{
    ...
    public EnumeratedDistribution(final RandomGenerator rng, final List<Pair<T, Double>> pmf) throws NotPositiveException, MathArithmeticException, NotFiniteNumberException, NotANumberException {
        ...
        for (int j = 0; j < this.probabilities.length; ++j) {
            sum += this.probabilities[j];
            this.cumulativeProbabilities[j] = sum;
        }
    }
    ...
    public List<Pair<T, Double>> getPmf() {
        final List<Pair<T, Double>> samples = new ArrayList<Pair<T, Double>>(this.probabilities.length);
        for (int i = 0; i < this.probabilities.length; ++i) {
    ...
    public Object[] sample(final int sampleSize) throws NotStrictlyPositiveException {
        if (sampleSize <= 0) {
    ...
    public T[] sample(final int sampleSize, final T[] array) throws NotStrictlyPositiveException {
        if (sampleSize <= 0) {
            throw new NotStrictlyPositiveException(LocalizedFormats.NUMBER_OF_SAMPLES, sampleSize);
        }
        if (array == null) {
        ...
        if (array.length < sampleSize) {
            final T[] unchecked = out = (T[])Array.newInstance(array.getClass().getComponentType(), sampleSize);
        }
        else {
            out = array;
        }
