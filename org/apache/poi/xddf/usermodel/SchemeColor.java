...
public enum SchemeColor
{
    ...
    private SchemeColor(final STSchemeColorVal.Enum color) {
        this.underlying = color;
    }
    
    static SchemeColor valueOf(final STSchemeColorVal.Enum color) {
        return SchemeColor.reverse.get(color);
    }
    
    static {
        reverse = new HashMap<STSchemeColorVal.Enum, SchemeColor>();
        for (final SchemeColor value : values()) {
            SchemeColor.reverse.put(value.underlying, value);
        }
    }
}
