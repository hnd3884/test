...
public final class HSSFPatriarch implements HSSFShapeContainer, Drawing<HSSFShape>
{
    ...
    HSSFPatriarch(final HSSFSheet sheet, final EscherAggregate boundAggregate) {
        ...
        this._spgrRecord = spContainer.getChildById(EscherSpgrRecord.RECORD_ID);
        this.buildShapeTree();
    }
    ...
    protected void preSerialize() {
        ...
        for (final NoteRecord rec : tailRecords.values()) {
            final String noteRef = new CellReference(rec.getRow(), rec.getColumn(), true, true).formatAsString();
    ...
    @Override
    public HSSFObjectData createObjectData(final ClientAnchor anchor, final int storageId, final int pictureIndex) {
        ...
        try {
            final DirectoryNode dn = this._sheet.getWorkbook().getDirectory();
            if (dn == null) {
        ...
        final HSSFPicture shape = new HSSFPicture(null, (HSSFAnchor)anchor);
        shape.setPictureIndex(pictureIndex);
        final EscherContainerRecord spContainer = shape.getEscherContainer();
        final EscherSpRecord spRecord = spContainer.getChildById(EscherSpRecord.RECORD_ID);
    ...
    int newShapeId() {
        ...
        final EscherDgRecord dg = this._boundAggregate.getEscherContainer().getChildById(EscherDgRecord.RECORD_ID);
        return dm.allocateShapeId(dg);
    }
    
    public boolean containsChart() {
        final EscherOptRecord optRecord = (EscherOptRecord)this._boundAggregate.findFirstWithId(EscherOptRecord.RECORD_ID);
        if (optRecord == null) {
            return false;
        }
