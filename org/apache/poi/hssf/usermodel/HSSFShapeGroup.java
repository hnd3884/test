...
public class HSSFShapeGroup extends HSSFShape implements HSSFShapeContainer
{
    ...
    public HSSFShapeGroup(final HSSFShape parent, final HSSFAnchor anchor) {
        super(parent, anchor);
        this.shapes = new ArrayList<HSSFShape>();
        this._spgrRecord = ((EscherContainerRecord)this.getEscherContainer().getChild(0)).getChildById(EscherSpgrRecord.RECORD_ID);
    }
    ...
    protected void afterRemove(final HSSFPatriarch patriarch) {
        patriarch.getBoundAggregate().removeShapeToObjRecord(this.getEscherContainer().getChildContainers().get(0).getChildById(EscherClientDataRecord.RECORD_ID));
    ...
    public HSSFSimpleShape createShape(final HSSFChildAnchor anchor) {
        ...
        this.shapes.add(shape);
        this.onCreate(shape);
        final EscherSpRecord sp = shape.getEscherContainer().getChildById(EscherSpRecord.RECORD_ID);
    ...
    void afterInsert(final HSSFPatriarch patriarch) {
        ...
        final EscherContainerRecord containerRecord = this.getEscherContainer().getChildById(EscherContainerRecord.SP_CONTAINER);
        agg.associateShapeToObjRecord(containerRecord.getChildById(EscherClientDataRecord.RECORD_ID), this.getObjRecord());
    }
    
    @Override
    void setShapeId(final int shapeId) {
        final EscherContainerRecord containerRecord = this.getEscherContainer().getChildById(EscherContainerRecord.SP_CONTAINER);
        final EscherSpRecord spRecord = containerRecord.getChildById(EscherSpRecord.RECORD_ID);
    ...
    int getShapeId() {
        final EscherContainerRecord containerRecord = this.getEscherContainer().getChildById(EscherContainerRecord.SP_CONTAINER);
        return containerRecord.getChildById(EscherSpRecord.RECORD_ID).getShapeId();
    }
    ...
    protected HSSFShape cloneShape(final HSSFPatriarch patriarch) {
        ...
        spgrContainer.setOptions((short)15);
        final EscherContainerRecord spContainer = new EscherContainerRecord();
        final EscherContainerRecord cont = this.getEscherContainer().getChildById(EscherContainerRecord.SP_CONTAINER);
