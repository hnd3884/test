...
public class StandardDecryptor extends Decryptor
{
    ...
    protected StandardDecryptor(final StandardDecryptor other) {
        super(other);
        this._length = -1L;
    ...
    private Cipher getCipher(final SecretKey key) {
        final EncryptionHeader em = this.getEncryptionInfo().getHeader();
    ...
    public InputStream getDataStream(final DirectoryNode dir) throws IOException {
        ...
        final int blockSize = this.getEncryptionInfo().getHeader().getCipherAlgorithm().blockSize;
