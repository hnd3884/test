...
public class Ole10Native
{
    ...
    public static Ole10Native createFromEmbeddedOleObject(final DirectoryNode directory) throws IOException, Ole10NativeException {
        ...
        try (final DocumentInputStream dis = directory.createDocumentInputStream(nativeEntry)) {
            final byte[] data = IOUtils.toByteArray(dis, nativeEntry.getSize(), 100000000);
    ...
    public Ole10Native(final byte[] data, final int offset) throws Ole10NativeException {
        ...
        if (dataSize + (long)ofs > data.length) {
            throw new Ole10NativeException("Invalid Ole10Native: declared data length > available data");
        }
        System.arraycopy(data, ofs, this.dataBuffer = IOUtils.safelyAllocate(dataSize, 100000000), 0, dataSize);
        ofs += dataSize;
    }
    
    public static void createOleMarkerEntry(final DirectoryEntry parent) throws IOException {
        if (!parent.hasEntry("\u0001Ole")) {
            parent.createDocument("\u0001Ole", new ByteArrayInputStream(Ole10Native.OLE_MARKER_BYTES));
        }
    }
