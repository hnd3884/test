...
public class LittleEndianCP950Reader extends Reader
{
    ...
    public LittleEndianCP950Reader(final byte[] data) {
        this(data, 0, data.length);
    }
    
    public LittleEndianCP950Reader(final byte[] data, final int offset, final int length) {
    ...
    public int read() {
        ...
        final int ch = (this.leading << 8) + this.trailing;
        ...
        if (this.charBuffer.length() == 0) {
            LittleEndianCP950Reader.LOGGER.log(5, "couldn't create char for: " + Integer.toString(this.leading & 0xFF, 16) + " " + Integer.toString(this.trailing & 0xFF, 16));
            return 63;
        }
    ...
    private int handleRange1(final int leading, final int trailing) {
        return 61112 + 157 * (leading - 129) + ((trailing < 128) ? (trailing - 64) : (trailing - 98));
    }
    
    private int handleRange2(final int leading, final int trailing) {
        return 58129 + 157 * (leading - 142) + ((trailing < 128) ? (trailing - 64) : (trailing - 98));
    }
    
    private int handleRange3(final int leading, final int trailing) {
        return 63090 + 157 * (leading - 198) + ((trailing < 128) ? (trailing - 64) : (trailing - 98));
    }
    
    private int handleRange4(final int leading, final int trailing) {
        return 57344 + 157 * (leading - 250) + ((trailing < 128) ? (trailing - 64) : (trailing - 98));
    }
    
    private int handleF9(final int trailing) {
        switch (trailing) {
            case 64: {
                return 32408;
            }
            ...
            default: {
                LittleEndianCP950Reader.LOGGER.log(5, "couldn't create char for: f9 " + Integer.toString(trailing & 0xFF, 16));
                return 63;
            }
        }
    }
    ...
    static {
        LOGGER = POILogFactory.getLogger(LittleEndianCP950Reader.class);
    }
}
