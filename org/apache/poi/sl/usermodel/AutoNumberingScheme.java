...
public enum AutoNumberingScheme
{
    ...
    private AutoNumberingScheme(final int nativeId, final int ooxmlId) {
        this.nativeId = nativeId;
        this.ooxmlId = ooxmlId;
    }
    
    public static AutoNumberingScheme forNativeID(final int nativeId) {
        for (final AutoNumberingScheme ans : values()) {
    ...
    public static AutoNumberingScheme forOoxmlID(final int ooxmlId) {
        for (final AutoNumberingScheme ans : values()) {
    ...
    public String format(final int value) {
        final String index = this.formatIndex(value);
        final String cased = this.formatCase(index);
        return this.formatSeperator(cased);
    }
    
    private String formatSeperator(final String cased) {
    ...
    private String formatCase(final String index) {
        final String name = this.name().toLowerCase(Locale.ROOT);
        if (name.contains("lc")) {
    ...
    private String formatIndex(final int value) {
        final String name = this.name().toLowerCase(Locale.ROOT);
        if (name.startsWith("roman")) {
            return this.formatRomanIndex(value);
        }
        if (name.startsWith("arabic") && !name.contains("db")) {
            return getIndexedList(value, "0123456789", false);
        }
        if (name.startsWith("alpha")) {
            return getIndexedList(value, "abcdefghijklmnopqrstuvwxyz", true);
        }
        if (name.contains("WdWhite")) {
            return (value == 10) ? "\u008a" : getIndexedList(value, "\u0080\u0081\u0082\u0083\u0084\u0085\u0086\u0087\u0088\u0089", false);
        }
        if (name.contains("WdBlack")) {
            return (value == 10) ? "\u0095" : getIndexedList(value, "\u008b\u008c\u008d\u008e\u008f\u0090\u0091\u0092\u0093\u0094", false);
        }
        if (name.contains("NumDb")) {
            return (value == 10) ? "\u277f" : getIndexedList(value, "\u2776\u2777\u2778\u2779\u277a\u277b\u277c\u277d\u277e", true);
        }
        return "?";
    }
    
    private static String getIndexedList(final int val, final String list, final boolean oneBased) {
        final StringBuilder sb = new StringBuilder();
        addIndexedChar(val, list, oneBased, sb);
        return sb.toString();
    }
    
    private static void addIndexedChar(int val, final String list, final boolean oneBased, final StringBuilder sb) {
        if (oneBased) {
            --val;
        }
    ...
    private String formatRomanIndex(int value) {
        ...
        final String[][] conciseList = { { "XLV", "VL" }, { "XCV", "VC" }, { "CDL", "LD" }, { "CML", "LM" }, { "CMVC", "LMVL" }, { "CDXC", "LDXL" }, { "CDVC", "LDVL" }, { "CMXC", "LMXL" }, { "XCIX", "VCIV" }, { "XLIX", "VLIV" }, { "XLIX", "IL" }, { "XCIX", "IC" }, { "CDXC", "XD" }, { "CDVC", "XDV" }, { "CDIC", "XDIX" }, { "LMVL", "XMV" }, { "CMIC", "XMIX" }, { "CMXC", "XM" }, { "XDV", "VD" }, { "XDIX", "VDIV" }, { "XMV", "VM" }, { "XMIX", "VMIV" }, { "VDIV", "ID" }, { "VMIV", "IM" } };
        final StringBuilder sb = new StringBuilder();
        for (int i = 0; i < 13; ++i) {
        ...
        for (final String[] cc : conciseList) {
