...
public enum XSSFBRecordType
{
    ...
    private XSSFBRecordType(final int id) {
        this.id = id;
    }
    ...
    public static XSSFBRecordType lookup(final int id) {
        final XSSFBRecordType type = XSSFBRecordType.TYPE_MAP.get(id);
        if (type == null) {
            return XSSFBRecordType.Unimplemented;
        }
        return type;
    }
    ...
    static {
        ...
        for (final XSSFBRecordType type : values()) {
            XSSFBRecordType.TYPE_MAP.put(type.getId(), type);
        }
    }
}
