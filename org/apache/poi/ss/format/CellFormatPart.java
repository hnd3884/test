...
public class CellFormatPart
{
    ...
    public CellFormatPart(final Locale locale, final String desc) {
    ...
    private CellFormatter getFormatter(final Locale locale, final Matcher matcher) {
        String fdesc = matcher.group(CellFormatPart.SPECIFICATION_GROUP);
        final Matcher currencyM = CellFormatPart.CURRENCY_PAT.matcher(fdesc);
        if (currencyM.find()) {
            final String currencyPart = currencyM.group(1);
            String currencyRepl;
    ...
    private CellFormatType formatType(String fdesc) {
        ...
        while (m.find()) {
            ...
            final Iterator<String> codePoints = new StringCodepointsIterable(repl).iterator();
            if (codePoints.hasNext()) {
                final String c1 = codePoints.next();
                String c2 = null;
                if (codePoints.hasNext()) {
    ...
    static String quoteSpecial(final String repl, final CellFormatType type) {
        ...
        for (final String ch : new StringCodepointsIterable(repl)) {
            if ("'".equals(ch) && type.isSpecial('\'')) {
                sb.append('\0');
            }
            else {
                final boolean special = type.isSpecial(ch.charAt(0));
                if (special) {
                    sb.append("'");
                }
                sb.append(ch);
    ...
    static String expandChar(final String part) {
        final List<String> codePoints = new ArrayList<String>();
        ...
        final String ch = codePoints.get(1);
        return ch + ch + ch;
    }
    ...
    static {
        ...
        for (final HSSFColor.HSSFColorPredefined color : HSSFColor.HSSFColorPredefined.values()) {
        ...
        final String currency = "(\\[\\$.{0,3}-[0-9a-f]{3}\\])";
