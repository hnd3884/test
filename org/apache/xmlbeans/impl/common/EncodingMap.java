...
public class EncodingMap
{
    ...
    public static String getJava2IANAMapping(final String java) {
        String iana = EncodingMap._java_to_iana.get(java.toUpperCase());
        if (iana != null) {
            return iana;
        }
    ...
    public static String getIANA2JavaMapping(final String iana) {
        final String java = EncodingMap._iana_to_java.get(iana.toUpperCase());
        if (java != null) {
            return java;
        }
    ...
    private static final void addMapping(final String java, final String iana, final boolean isDefault) {
        assert !EncodingMap._iana_to_java.containsKey(iana);
        assert java.toUpperCase().equals(java);
        assert iana.toUpperCase().equals(iana);
        EncodingMap._iana_to_java.put(iana, java);
        if (isDefault) {
            assert !EncodingMap._java_to_iana.containsKey(java);
            EncodingMap._java_to_iana.put(java, iana);
        }
    }
    
    private static final boolean completeMappings() {
        final HashMap m = new HashMap();
        Iterator i = EncodingMap._iana_to_java.keySet().iterator();
        while (i.hasNext()) {
            m.put(EncodingMap._iana_to_java.get(i.next()), null);
        }
        i = m.keySet().iterator();
        while (i.hasNext()) {
            final Object k = i.next();
            assert EncodingMap._java_to_iana.containsKey(k) : k;
        }
        return true;
    }
