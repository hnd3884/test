...
public final class CharUtil
{
    ...
    public CharUtil(final int charBufSize) {
        this._charIter = new CharIterator();
        this._charBufSize = charBufSize;
    }
    
    public CharIterator getCharIterator(final Object src, final int off, final int cch) {
        this._charIter.init(src, off, cch);
        return this._charIter;
    }
    
    public CharIterator getCharIterator(final Object src, final int off, final int cch, final int start) {
        this._charIter.init(src, off, cch, start);
        return this._charIter;
    }
    
    public static CharUtil getThreadLocalCharUtil() {
        final SoftReference softRef = CharUtil.tl_charUtil.get();
        CharUtil charUtil = softRef.get();
        if (charUtil == null) {
            charUtil = new CharUtil(CharUtil.CHARUTIL_INITIAL_BUFSIZE);
            CharUtil.tl_charUtil.set(new SoftReference(charUtil));
        }
        return charUtil;
    }
    
    public static void getString(final StringBuffer sb, final Object src, final int off, final int cch) {
    ...
    public static void getChars(final char[] chars, final int start, final Object src, final int off, final int cch) {
        assert isValid(src, off, cch);
        ...
        if (src instanceof char[]) {
            System.arraycopy(src, off, chars, start, cch);
        }
        else if (src instanceof String) {
    ...
    public static String getString(final Object src, final int off, final int cch) {
        assert isValid(src, off, cch);
        if (cch == 0) {
            return "";
        }
    ...
    public final boolean isWhiteSpace(final Object src, int off, int cch) {
        assert isValid(src, off, cch);
        if (cch <= 0) {
            return true;
        }
        if (src instanceof char[]) {
            final char[] chars = (char[])src;
            while (cch > 0) {
    ...
    public Object stripLeft(final Object src, int off, int cch) {
        assert isValid(src, off, cch);
        if (cch > 0) {
            if (src instanceof char[]) {
                for (char[] chars = (char[])src; cch > 0 && isWhiteSpace(chars[off]); --cch, ++off) {}
            }
            else if (src instanceof String) {
                for (String s = (String)src; cch > 0 && isWhiteSpace(s.charAt(off)); --cch, ++off) {}
            }
            else {
                int count = 0;
                this._charIter.init(src, off, cch);
    ...
    public Object stripRight(final Object src, final int off, int cch) {
        assert isValid(src, off, cch);
        if (cch > 0) {
    ...
    public Object insertChars(final int posInsert, final Object src, final int off, final int cch, final Object srcInsert, final int offInsert, final int cchInsert) {
        assert isValid(src, off, cch);
        ...
        if (this._cchSrc <= 64 && this.canAllocate(this._cchSrc)) {
            final char[] c = this.allocate(this._cchSrc);
            ...
            if (posInsert == 0) {
                ...
                final CharJoin j = new CharJoin(src, off, posInsert, srcInsert, offInsert);
    ...
    public Object removeChars(final int posRemove, final int cchRemove, final Object src, final int off, final int cch) {
        assert isValid(src, off, cch);
        ...
        if (this._cchSrc == 0) {
            ...
            final int cchAfter = cch - cchRemove;
            if (cchAfter <= 64 && this.canAllocate(cchAfter)) {
                final char[] chars = this.allocate(cchAfter);
                ...
                final CharJoin j = new CharJoin(src, off, posRemove, src, off + posRemove + cchRemove);
                if (j._depth > 64) {
    ...
    private static int sizeof(final Object src) {
        assert !(!(src instanceof char[]));
        if (src instanceof char[]) {
    ...
    private boolean canAllocate(final int cch) {
        return this._currentBuffer == null || this._currentBuffer.length - this._currentOffset >= cch;
    }
    
    private char[] allocate(final int cch) {
        assert this._currentBuffer.length - this._currentOffset > 0;
        ...
        final char[] retBuf = this._currentBuffer;
    ...
    public Object saveChars(final Object srcSave, final int offSave, final int cchSave) {
        return this.saveChars(srcSave, offSave, cchSave, null, 0, 0);
    }
    
    public Object saveChars(final Object srcSave, final int offSave, final int cchSave, final Object srcPrev, final int offPrev, final int cchPrev) {
        ...
        if (cchPrev == 0) {
            ...
            CharJoin j;
            if (srcPrev instanceof CharJoin && (j = (CharJoin)srcPrev)._srcRight == srcAlloc && offPrev + cchPrev - j._cchLeft + j._offRight == offAlloc) {
                assert j._srcRight instanceof char[];
                srcNew = srcPrev;
                offNew = offPrev;
            }
            else {
                j = (CharJoin)(srcNew = new CharJoin(srcPrev, offPrev, cchPrev, srcAlloc, offAlloc));
                offNew = 0;
                srcNew = ((j._depth > 64) ? this.saveChars(j, 0, cchNew) : j);
            }
        }
        final int cchMore = cchSave - cchAlloc;
        if (cchMore > 0) {
            ...
            final CharJoin j = new CharJoin(srcNew, offNew, cchNew, srcAlloc, offAlloc);
            offNew = 0;
            cchNew += cchMore;
            srcNew = ((j._depth > 64) ? this.saveChars(j, 0, cchNew) : j);
        }
    ...
    private static void dumpText(final PrintStream o, final String s) {
        o.print("\"");
    ...
    public static void dump(final Object src, final int off, final int cch) {
        dumpChars(System.out, src, off, cch);
        System.out.println();
    }
    
    public static void dumpChars(final PrintStream p, final Object src, final int off, final int cch) {
    ...
    public static boolean isValid(final Object src, final int off, final int cch) {
        if (cch < 0 || off < 0) {
            return false;
        }
        if (src == null) {
        ...
        return src instanceof CharJoin && ((CharJoin)src).isValid(off, cch);
    }
    
    public static void clearThreadLocals() {
        CharUtil.tl_charUtil.remove();
    }
    
    static {
        CharUtil.CHARUTIL_INITIAL_BUFSIZE = 32768;
        CharUtil.tl_charUtil = new ThreadLocal() {
            @Override
    ...
    public static final class CharJoin
    {
        ...
        public CharJoin(final Object srcLeft, final int offLeft, final int cchLeft, final Object srcRight, final int offRight) {
            ...
            if (srcRight instanceof CharJoin) {
                final int rightDepth = ((CharJoin)srcRight)._depth;
                if (rightDepth > depth) {
                    depth = rightDepth;
                }
            }
        ...
        private int cchRight(final int off, final int cch) {
        ...
        public boolean isValid(final int off, final int cch) {
            if (this._depth > 2) {
                return true;
            }
            assert this._depth == this.depth();
            return off >= 0 && cch >= 0 && CharUtil.isValid(this._srcLeft, this._offLeft, this._cchLeft) && CharUtil.isValid(this._srcRight, this._offRight, this.cchRight(off, cch));
        }
        
        private void getString(final StringBuffer sb, final int off, final int cch) {
            assert cch > 0;
            if (off < this._cchLeft) {
                final int cchL = Math.min(this._cchLeft - off, cch);
        ...
        private void getChars(final char[] chars, final int start, final int off, final int cch) {
            assert cch > 0;
            if (off < this._cchLeft) {
                final int cchL = Math.min(this._cchLeft - off, cch);
        ...
        private void dumpChars(final int off, final int cch) {
            this.dumpChars(System.out, off, cch);
        }
        
        private void dumpChars(final PrintStream p, final int off, final int cch) {
            p.print("( ");
    ...
    public static final class CharIterator
    {
        ...
        public void init(final Object src, final int off, final int cch) {
            this.init(src, off, cch, 0);
        }
        
        public void init(final Object src, final int off, final int cch, final int startPos) {
        ...
        public void movePos(final int newPos) {
        ...
        private char currentChar() {
            final int i = this._offLeaf + this._pos - this._minPos;
