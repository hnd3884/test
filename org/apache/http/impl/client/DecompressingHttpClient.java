...
public class DecompressingHttpClient implements HttpClient
{
    ...
    public HttpResponse execute(final HttpHost target, final HttpRequest request, final HttpContext context) throws IOException, ClientProtocolException {
        try {
            final HttpContext localContext = (HttpContext)((context != null) ? context : new BasicHttpContext());
            HttpRequest wrapped;
            if (request instanceof HttpEntityEnclosingRequest) {
                wrapped = (HttpRequest)new EntityEnclosingRequestWrapper((HttpEntityEnclosingRequest)request);
            }
            else {
                wrapped = (HttpRequest)new RequestWrapper(request);
            }
