...
public class ConnPoolByRoute extends AbstractConnPool
{
    ...
    public ConnPoolByRoute(final ClientConnectionOperator operator, final ConnPerRoute connPerRoute, final int maxTotalConnections, final long connTTL, final TimeUnit connTTLTimeUnit) {
        ...
        Args.notNull((Object)connPerRoute, "Connections per route");
    ...
    public PoolEntryRequest requestPoolEntry(final HttpRoute route, final Object state) {
        ...
        return new PoolEntryRequest() {
            ...
            @Override
            public BasicPoolEntry getPoolEntry(final long timeout, final TimeUnit timeUnit) throws InterruptedException, ConnectionPoolTimeoutException {
                return ConnPoolByRoute.this.getEntryBlocking(route, state, timeout, timeUnit, aborter);
            }
        };
    }
    
    protected BasicPoolEntry getEntryBlocking(final HttpRoute route, final Object state, final long timeout, final TimeUnit timeUnit, final WaitingThreadAborter aborter) throws ConnectionPoolTimeoutException, InterruptedException {
        Date deadline = null;
        if (timeout > 0L) {
    ...
    public void closeIdleConnections(final long idletime, final TimeUnit timeUnit) {
        ...
        if (this.log.isDebugEnabled()) {
            this.log.debug((Object)("Closing connections idle longer than " + t + " " + timeUnit));
        }
    ...
    public void shutdown() {
        ...
        try {
            ...
            while (iter2.hasNext()) {
                ...
                iter2.remove();
                if (this.log.isDebugEnabled()) {
                    this.log.debug((Object)("Closing connection [" + entry2.getPlannedRoute() + "][" + entry2.getState() + "]"));
                }
