...
public class RequestAddCookies implements HttpRequestInterceptor
{
    ...
    public void process(final HttpRequest request, final HttpContext context) throws HttpException, IOException {
        ...
        final CookieOrigin cookieOrigin = new CookieOrigin(hostName, (port >= 0) ? port : 0, TextUtils.isEmpty((CharSequence)path) ? "/" : path, route.isSecure());
        final CookieSpecProvider provider = (CookieSpecProvider)registry.lookup(policy);
        if (provider == null) {
            if (this.log.isDebugEnabled()) {
        ...
        final CookieSpec cookieSpec = provider.create((HttpContext)clientContext);
