...
public class TypeInfoCache implements TypeInfo
{
    ...
    private String getSQLTypeQuery(final boolean typnameParam) {
    ...
    public int getSQLType(final int oid) throws SQLException {
        return this.getSQLType(Nullness.castNonNull(this.getPGType(oid)));
    }
    ...
    public synchronized int getSQLType(final String pgTypeName) throws SQLException {
        ...
        final Integer i = this.pgNameToSQLType.get(pgTypeName);
        if (i != null) {
            return i;
        }
        TypeInfoCache.LOGGER.log(Level.FINEST, "querying SQL typecode for pg type '{0}'", pgTypeName);
