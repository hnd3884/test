...
public class Base64
{
    ...
    public static final String ENABLE_UNSAFE_SERIALIZATION = "org.owasp.esapi.enableUnsafeSerialization";
    ...
    public static String encodeObject(final Serializable serializableObject, final int options) {
        ...
        try {
            ...
            Base64.logger.error(Logger.SECURITY_FAILURE, "Problem writing object", e);
            return null;
        }
        finally {
            try {
    ...
    public static String encodeBytes(final byte[] source, final int off, final int len, final int options) {
        ...
        if (gzip == 2) {
            ...
            try {
                ...
                Base64.logger.error(Logger.SECURITY_FAILURE, "Problem writing gzip stream", e);
                return null;
            }
            finally {
                try {
        ...
        catch (final UnsupportedEncodingException uue2) {
    ...
    private static int decode4to3(final byte[] source, final int srcOffset, final byte[] destination, final int destOffset, final int options) {
        ...
        try {
            ...
            final StringBuilder sb = new StringBuilder("Problem writing object:");
            sb.append(Base64.EOL);
            sb.append(source[srcOffset]).append(": ").append(DECODABET[source[srcOffset]]).append(Base64.EOL);
            sb.append(source[srcOffset + 1]).append(": ").append(DECODABET[source[srcOffset + 1]]).append(Base64.EOL);
            sb.append(source[srcOffset + 2]).append(": ").append(DECODABET[source[srcOffset + 2]]).append(Base64.EOL);
            sb.append(source[srcOffset + 3]).append(": ").append(DECODABET[source[srcOffset + 3]]).append(Base64.EOL);
            Base64.logger.error(Logger.SECURITY_FAILURE, sb.toString(), e);
            return -1;
        }
    }
    ...
    public static byte[] decode(final byte[] source, final int off, final int len, final int options) {
        ...
        for (i = off; i < off + len; ++i) {
            ...
            if (sbiDecode < -5) {
                Base64.logger.error(Logger.SECURITY_FAILURE, "Bad Base64 input character at " + i + ": " + source[i] + "(decimal)");
                return null;
            }
            if (sbiDecode >= -1) {
                b4[b4Posn++] = sbiCrop;
    ...
    public static byte[] decode(final String s, final int options) {
        ...
        try {
            ...
            bytes = s.getBytes();
            Base64.logger.warning(Logger.SECURITY_FAILURE, "Problem decoding string using UTF-8; substituting native platform encoding instead", uee);
        }
    ...
    public static Object decodeToObject(final String encodedObject) {
        if (!"true".equalsIgnoreCase(System.getProperty("org.owasp.esapi.enableUnsafeSerialization"))) {
            throw new UnsupportedOperationException("Deserialization by Base64.decodeToObject(String) is disabled for security reasons. To re-enable it, set the system property 'org.owasp.esapi.enableUnsafeSerialization' to 'true'.For details, see: https://github.com/ESAPI/esapi-java-legacy/issues/354");
        }
        ...
        try {
            ...
            Base64.logger.error(Logger.SECURITY_FAILURE, "Problem reading object", e);
            obj = null;
        }
        catch (final ClassNotFoundException e2) {
            Base64.logger.error(Logger.SECURITY_FAILURE, "Problem reading object", e2);
            obj = null;
        }
        finally {
            try {
    ...
    public static byte[] decodeFromFile(final String filename) {
        ...
        try {
            ...
            if (file.length() > 2147483647L) {
                Base64.logger.error(Logger.SECURITY_FAILURE, "File is too big for this convenience method (" + file.length() + " bytes).");
                return null;
            }
            ...
            Base64.logger.error(Logger.SECURITY_FAILURE, "Error decoding from file " + filename, e);
        }
        finally {
            try {
                if (bis != null) {
    ...
    public static String encodeFromFile(final String filename) {
        ...
        try {
            ...
            Base64.logger.error(Logger.SECURITY_FAILURE, "Error encoding from file " + filename, e);
        }
        finally {
            try {
                bis.close();
    ...
    public static boolean encodeFileToFile(final String infile, final String outfile) {
        ...
        try {
            ...
            Base64.logger.error(Logger.SECURITY_FAILURE, "Problem encoding file to file", exc);
        }
        finally {
            try {
                in.close();
    ...
    public static boolean decodeFileToFile(final String infile, final String outfile) {
        ...
        try {
            ...
            Base64.logger.error(Logger.SECURITY_FAILURE, "Problem decoding file to file", exc);
        }
        finally {
            try {
                in.close();
