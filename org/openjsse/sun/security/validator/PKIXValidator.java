...
public final class PKIXValidator extends Validator
{
    ...
    PKIXValidator(final String variant, final Collection<X509Certificate> trustedCerts) {
        ...
        this.trustedCerts = ((trustedCerts instanceof Set) ? ((Set)trustedCerts) : new HashSet<X509Certificate>(trustedCerts));
    ...
    private Map<X500Principal, List<PublicKey>> setTrustedSubjects() {
        final Map<X500Principal, List<PublicKey>> subjectMap = new HashMap<X500Principal, List<PublicKey>>();
    ...
    @Override
    X509Certificate[] engineValidate(final X509Certificate[] chain, final Collection<X509Certificate> otherCerts, final List<byte[]> responseList, final AlgorithmConstraints constraints, final Object parameter) throws CertificateException {
    ...
    private X509Certificate[] doValidate(final X509Certificate[] chain, final PKIXBuilderParameters params) throws CertificateException {
        try {
            ...
            final CertPath path = this.factory.generateCertPath(Arrays.asList(newChain));
    ...
    private static void addResponses(final PKIXBuilderParameters pkixParams, final X509Certificate[] chain, final List<byte[]> responseList) {
        if (pkixParams.isRevocationEnabled()) {
            try {
                PKIXRevocationChecker revChecker = null;
                final List<PKIXCertPathChecker> checkerList = new ArrayList<PKIXCertPathChecker>(pkixParams.getCertPathCheckers());
                for (final PKIXCertPathChecker checker : checkerList) {
                ...
                final Map<X509Certificate, byte[]> responseMap = revChecker.getOcspResponses();
                for (int limit = Integer.min(chain.length, responseList.size()), idx = 0; idx < limit; ++idx) {
                    final byte[] respBytes = responseList.get(idx);
