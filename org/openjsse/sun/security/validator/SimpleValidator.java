...
public final class SimpleValidator extends Validator
{
    ...
    @Override
    X509Certificate[] engineValidate(X509Certificate[] chain, final Collection<X509Certificate> otherCerts, final List<byte[]> responseList, final AlgorithmConstraints constraints, final Object parameter) throws CertificateException {
    ...
    private X509Certificate[] buildTrustedChain(final X509Certificate[] chain) throws CertificateException {
        ...
        if (list != null) {
            ...
            final X509CertImpl certImpl = X509CertImpl.toImpl(cert2);
            final KeyIdentifier akid = certImpl.getAuthKeyId();
            if (akid != null) {
                for (final X509Certificate sup : list) {
                    final X509CertImpl supCert = X509CertImpl.toImpl(sup);
