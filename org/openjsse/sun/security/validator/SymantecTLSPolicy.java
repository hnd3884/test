...
final class SymantecTLSPolicy
{
    ...
    static void checkDistrust(final X509Certificate[] chain) throws ValidatorException {
        final X509Certificate anchor = chain[chain.length - 1];
        if (SymantecTLSPolicy.FINGERPRINTS.contains(fingerprint(anchor))) {
            final Date notBefore = chain[0].getNotBefore();
            final LocalDate ldNotBefore = notBefore.toInstant().atZone(ZoneOffset.UTC).toLocalDate();
            if (chain.length > 2) {
                final X509Certificate subCA = chain[chain.length - 2];
                final LocalDate distrustDate = SymantecTLSPolicy.EXEMPT_SUBCAS.get(fingerprint(subCA));
    ...
    private static String fingerprint(final X509Certificate cert) {
        return (cert instanceof X509CertImpl) ? ((X509CertImpl)cert).getFingerprint("SHA-256") : X509CertImpl.getFingerprint("SHA-256", cert);
    }
    
    private static void checkNotBefore(final LocalDate notBeforeDate, final LocalDate distrustDate, final X509Certificate anchor) throws ValidatorException {
