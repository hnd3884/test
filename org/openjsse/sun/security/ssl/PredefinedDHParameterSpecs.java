package org.openjsse.sun.security.ssl;

import java.util.regex.Matcher;
import java.util.Collections;
import java.util.HashMap;
import java.security.AccessController;
import java.security.Security;
import java.security.PrivilegedAction;
import javax.crypto.spec.DHParameterSpec;
import java.util.Map;
import java.util.regex.Pattern;
import java.math.BigInteger;

final class PredefinedDHParameterSpecs
{
    private static final BigInteger p512;
    private static final BigInteger p768;
    private static final BigInteger p1024;
    private static final BigInteger p1536;
    private static final BigInteger p2048;
    private static final BigInteger p3072;
    private static final BigInteger p4096;
    private static final BigInteger p6144;
    private static final BigInteger p8192;
    private static final BigInteger[] supportedPrimes;
    private static final BigInteger[] ffdhePrimes;
    private static final int PRIME_CERTAINTY = 120;
    private static final String PROPERTY_NAME = "jdk.tls.server.defaultDHEParameters";
    private static final Pattern spacesPattern;
    private static final Pattern syntaxPattern;
    private static final Pattern paramsPattern;
    static final Map<Integer, DHParameterSpec> definedParams;
    static final Map<Integer, DHParameterSpec> ffdheParams;
    
    static {
        p512 = new BigInteger("D87780E15FF50B4ABBE89870188B049406B5BEA98AB23A0241D88EA75B7755E669C08093D3F0CA7FC3A5A25CF067DCB9A43DD89D1D90921C6328884461E0B6D3", 16);
        p768 = new BigInteger("FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD129024E088A67CC74020BBEA63B139B22514A08798E3404DDEF9519B3CD3A431B302B0A6DF25F14374FE1356D6D51C245E485B576625E7EC6F44C42E9A63A3620FFFFFFFFFFFFFFFF", 16);
        p1024 = new BigInteger("", 16);
        p1536 = new BigInteger("", 16);
        p2048 = new BigInteger("", 16);
        p3072 = new BigInteger("", 16);
        p4096 = new BigInteger("", 16);
        p6144 = new BigInteger("", 16);
        p8192 = new BigInteger("", 16);
        supportedPrimes = new BigInteger[] { PredefinedDHParameterSpecs.p512, PredefinedDHParameterSpecs.p768, PredefinedDHParameterSpecs.p1024, PredefinedDHParameterSpecs.p1536, PredefinedDHParameterSpecs.p2048, PredefinedDHParameterSpecs.p3072, PredefinedDHParameterSpecs.p4096, PredefinedDHParameterSpecs.p6144, PredefinedDHParameterSpecs.p8192 };
        ffdhePrimes = new BigInteger[] { PredefinedDHParameterSpecs.p2048, PredefinedDHParameterSpecs.p3072, PredefinedDHParameterSpecs.p4096, PredefinedDHParameterSpecs.p6144, PredefinedDHParameterSpecs.p8192 };
        spacesPattern = Pattern.compile("\\s+");
        syntaxPattern = Pattern.compile("(\\{[0-9A-Fa-f]+,[0-9A-Fa-f]+\\})(,\\{[0-9A-Fa-f]+,[0-9A-Fa-f]+\\})*");
        paramsPattern = Pattern.compile("\\{([0-9A-Fa-f]+),([0-9A-Fa-f]+)\\}");
        String property = AccessController.doPrivileged((PrivilegedAction<String>)new PrivilegedAction<String>() {
            @Override
            public String run() {
                return Security.getProperty("jdk.tls.server.defaultDHEParameters");
            }
        });
        if (property != null && !property.isEmpty()) {
            if (property.length() >= 2 && property.charAt(0) == '\"' && property.charAt(property.length() - 1) == '\"') {
                property = property.substring(1, property.length() - 1);
            }
            property = property.trim();
        }
        if (property != null && !property.isEmpty()) {
            final Matcher spacesMatcher = PredefinedDHParameterSpecs.spacesPattern.matcher(property);
            property = spacesMatcher.replaceAll("");
            if (SSLLogger.isOn && SSLLogger.isOn("sslctx")) {
                SSLLogger.fine("The Security Property jdk.tls.server.defaultDHEParameters: " + property, new Object[0]);
            }
        }
        final Map<Integer, DHParameterSpec> defaultParams = new HashMap<Integer, DHParameterSpec>();
        if (property != null && !property.isEmpty()) {
            final Matcher syntaxMatcher = PredefinedDHParameterSpecs.syntaxPattern.matcher(property);
            if (syntaxMatcher.matches()) {
                final Matcher paramsFinder = PredefinedDHParameterSpecs.paramsPattern.matcher(property);
                while (paramsFinder.find()) {
                    final String primeModulus = paramsFinder.group(1);
                    final BigInteger p8193 = new BigInteger(primeModulus, 16);
                    if (!p8193.isProbablePrime(120)) {
                        if (!SSLLogger.isOn || !SSLLogger.isOn("sslctx")) {
                            continue;
                        }
                        SSLLogger.fine("Prime modulus p in Security Property, jdk.tls.server.defaultDHEParameters, is not a prime: " + primeModulus, new Object[0]);
                    }
                    else {
                        final String baseGenerator = paramsFinder.group(2);
                        final BigInteger g = new BigInteger(baseGenerator, 16);
                        final DHParameterSpec spec = new DHParameterSpec(p8193, g);
                        final int primeLen = p8193.bitLength();
                        defaultParams.put(primeLen, spec);
                    }
                }
            }
            else if (SSLLogger.isOn && SSLLogger.isOn("sslctx")) {
                SSLLogger.fine("Invalid Security Property, jdk.tls.server.defaultDHEParameters, definition", new Object[0]);
            }
        }
        final BigInteger TWO = BigInteger.valueOf(2L);
        final Map<Integer, DHParameterSpec> tempFFDHEs = new HashMap<Integer, DHParameterSpec>();
        for (final BigInteger p8194 : PredefinedDHParameterSpecs.ffdhePrimes) {
            final int primeLen2 = p8194.bitLength();
            final DHParameterSpec dhps = new DHParameterSpec(p8194, TWO);
            tempFFDHEs.put(primeLen2, dhps);
            defaultParams.putIfAbsent(primeLen2, dhps);
        }
        for (final BigInteger p8194 : PredefinedDHParameterSpecs.supportedPrimes) {
            final int primeLen2 = p8194.bitLength();
            if (defaultParams.get(primeLen2) == null) {
                defaultParams.put(primeLen2, new DHParameterSpec(p8194, TWO));
            }
        }
        ffdheParams = Collections.unmodifiableMap((Map<? extends Integer, ? extends DHParameterSpec>)tempFFDHEs);
        definedParams = Collections.unmodifiableMap((Map<? extends Integer, ? extends DHParameterSpec>)defaultParams);
    }
}
