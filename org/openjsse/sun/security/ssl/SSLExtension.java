...
enum SSLExtension implements SSLStringizer
{
    ...
    CH_CERTIFICATE_AUTHORITIES(47, "certificate_authorities", SSLHandshake.CLIENT_HELLO, ProtocolVersion.PROTOCOLS_OF_13, CertificateAuthorityExtension.chNetworkProducer, CertificateAuthorityExtension.chOnLoadConsumer, (HandshakeAbsence)null, CertificateAuthorityExtension.chOnTradeConsumer, (HandshakeAbsence)null, CertificateAuthorityExtension.ssStringizer), 
    CR_CERTIFICATE_AUTHORITIES(47, "certificate_authorities", SSLHandshake.CERTIFICATE_REQUEST, ProtocolVersion.PROTOCOLS_OF_13, CertificateAuthorityExtension.crNetworkProducer, CertificateAuthorityExtension.crOnLoadConsumer, (HandshakeAbsence)null, CertificateAuthorityExtension.crOnTradeConsumer, (HandshakeAbsence)null, CertificateAuthorityExtension.ssStringizer), 
    ...
    private SSLExtension(final int id, final String name, final SSLHandshake handshakeType, final ProtocolVersion[] supportedProtocols, final HandshakeProducer producer, final ExtensionConsumer onLoadConsumer, final HandshakeAbsence onLoadAbsence, final HandshakeConsumer onTradeConsumer, final HandshakeAbsence onTradeAbsence, final SSLStringizer stringize) {
        this.id = id;
    ...
    static SSLExtension valueOf(final SSLHandshake handshakeType, final int extensionType) {
        for (final SSLExtension ext : values()) {
            if (ext.id == extensionType && ext.handshakeType == handshakeType) {
                return ext;
            }
        }
        return null;
    ...
    static String nameOf(final int extensionType) {
        for (final SSLExtension ext : values()) {
    ...
    static boolean isConsumable(final int extensionType) {
        for (final SSLExtension ext : values()) {
            if (ext.id == extensionType && ext.onLoadConsumer != null) {
                return true;
            }
        }
        return false;
    ...
    public void consumeOnLoad(final ConnectionContext context, final SSLHandshake.HandshakeMessage message, final ByteBuffer buffer) throws IOException {
    ...
    public void consumeOnTrade(final ConnectionContext context, final SSLHandshake.HandshakeMessage message) throws IOException {
    ...
    void absentOnLoad(final ConnectionContext context, final SSLHandshake.HandshakeMessage message) throws IOException {
    ...
    void absentOnTrade(final ConnectionContext context, final SSLHandshake.HandshakeMessage message) throws IOException {
    ...
    public String toString(final ByteBuffer byteBuffer) {
        ...
        if (this.stringizer == null) {
            ...
            final String encoded = extData = hexEncoder.encode(byteBuffer.duplicate());
        }
        else {
            extData = this.stringizer.toString(byteBuffer);
        }
        final Object[] messageFields = { this.name, this.id, Utilities.indent(extData) };
    ...
    static final class ClientExtensions
    {
        ...
        static {
            final Collection<SSLExtension> extensions = new LinkedList<SSLExtension>();
            ...
            enableExtension = (Utilities.getBooleanProperty("jsse.enableMFLNExtension", false) || Utilities.getBooleanProperty("jsse.enableMFLExtension", false));
            if (!enableExtension) {
                extensions.remove(SSLExtension.CH_MAX_FRAGMENT_LENGTH);
            }
            defaults = Collections.unmodifiableCollection((Collection<? extends SSLExtension>)extensions);
        }
    }
