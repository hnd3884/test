...
enum CipherSuite
{
    ...
    private CipherSuite(final int id, final boolean isDefaultEnabled, final String name, final ProtocolVersion[] supportedProtocols, final SSLCipher bulkCipher, final HashAlg hashAlg) {
        this(id, isDefaultEnabled, name, "", supportedProtocols, null, bulkCipher, MacAlg.M_NULL, hashAlg);
    }
    
    private CipherSuite(final int id, final boolean isDefaultEnabled, final String name, final String aliases, final ProtocolVersion[] supportedProtocols, final KeyExchange keyExchange, final SSLCipher cipher, final MacAlg macAlg, final HashAlg hashAlg) {
        this.id = id;
    ...
    static CipherSuite nameOf(final String ciperSuiteName) {
        for (final CipherSuite cs : values()) {
    ...
    static CipherSuite valueOf(final int id) {
        for (final CipherSuite cs : values()) {
            if (cs.id == id) {
                return cs;
            }
    ...
    static Collection<CipherSuite> allowedCipherSuites() {
        final Collection<CipherSuite> cipherSuites = new LinkedList<CipherSuite>();
        for (final CipherSuite cs : values()) {
    ...
    static List<CipherSuite> validValuesOf(final String[] names) {
        if (names == null) {
            throw new IllegalArgumentException("CipherSuites cannot be null");
        }
        final List<CipherSuite> cipherSuites = new ArrayList<CipherSuite>(names.length);
    ...
    static String[] namesOf(final List<CipherSuite> cipherSuites) {
        final String[] names = new String[cipherSuites.size()];
        int i = 0;
    ...
    public boolean supports(final ProtocolVersion protocolVersion) {
        return this.supportedProtocols.contains(protocolVersion);
    }
    
    boolean isNegotiable() {
        return this != CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV && this.isAvailable();
    }
    ...
    int calculatePacketSize(final int fragmentSize, final ProtocolVersion protocolVersion, final boolean isDTLS) {
        int packetSize = fragmentSize;
        if (this.bulkCipher != null && this.bulkCipher != SSLCipher.B_NULL) {
            final int blockSize = this.bulkCipher.ivSize;
            switch (this.bulkCipher.cipherType) {
                case BLOCK_CIPHER: {
                    packetSize += this.macAlg.size;
                    packetSize = ++packetSize + (blockSize - packetSize % blockSize) % blockSize;
    ...
    int calculateFragSize(final int packetLimit, final ProtocolVersion protocolVersion, final boolean isDTLS) {
        ...
        if (this.bulkCipher != null && this.bulkCipher != SSLCipher.B_NULL) {
            ...
            switch (this.bulkCipher.cipherType) {
                case BLOCK_CIPHER: {
                    ...
                    fragSize = --fragSize - this.macAlg.size;
                    break;
                }
                case AEAD_CIPHER: {
                    final int n = fragSize;
                    this.bulkCipher.getClass();
                    fragSize = n - 16;
    ...
    enum KeyExchange
    {
        ...
        private KeyExchange(final String name, final boolean allowed, final boolean isAnonymous, final SupportedGroupsExtension.NamedGroupType groupType) {
            this.name = name;
    ...
    enum MacAlg
    {
        ...
        private MacAlg(final String name, final int size, final int hashBlockSize, final int minimalPaddingSize) {
            this.name = name;
            this.size = size;
    ...
    enum HashAlg
    {
        ...
        private HashAlg(final String hashAlg, final int hashLength, final int blockSize) {
            this.name = hashAlg;
