...
public class CTVectorVariantImpl extends XmlComplexContentImpl implements CTVectorVariant
{
    ...
    public CTVectorVariantImpl(final SchemaType schemaType) {
        super(schemaType);
    }
    ...
    public CTVector getVector() {
        synchronized (this.monitor()) {
            this.check_orphaned();
            final CTVector ctVector = (CTVector)this.get_store().find_element_user(CTVectorVariantImpl.VECTOR$0, 0);
            if (ctVector == null) {
                return null;
            }
            return ctVector;
    ...
    public void setVector(final CTVector ctVector) {
        this.generatedSetterHelperImpl((XmlObject)ctVector, CTVectorVariantImpl.VECTOR$0, 0, (short)1);
    }
    
    public CTVector addNewVector() {
        synchronized (this.monitor()) {
            this.check_orphaned();
            return (CTVector)this.get_store().add_element_user(CTVectorVariantImpl.VECTOR$0);
        }
    }
