...
public abstract class OperatorExpression extends Expression implements OperatorIds
{
    ...
    static {
        OperatorExpression.OperatorSignatures = new int[20][];
        classInitialize();
    }
    
    public static final void classInitialize() {
        OperatorExpression.OperatorSignatures[2] = get_AND();
        OperatorExpression.OperatorSignatures[0] = get_AND_AND();
        OperatorExpression.OperatorSignatures[9] = get_DIVIDE();
        OperatorExpression.OperatorSignatures[18] = get_EQUAL_EQUAL();
        OperatorExpression.OperatorSignatures[6] = get_GREATER();
        OperatorExpression.OperatorSignatures[7] = get_GREATER_EQUAL();
        OperatorExpression.OperatorSignatures[10] = get_LEFT_SHIFT();
        OperatorExpression.OperatorSignatures[4] = get_LESS();
        OperatorExpression.OperatorSignatures[5] = get_LESS_EQUAL();
        OperatorExpression.OperatorSignatures[13] = get_MINUS();
        OperatorExpression.OperatorSignatures[15] = get_MULTIPLY();
        OperatorExpression.OperatorSignatures[3] = get_OR();
        OperatorExpression.OperatorSignatures[1] = get_OR_OR();
        OperatorExpression.OperatorSignatures[14] = get_PLUS();
        OperatorExpression.OperatorSignatures[16] = get_REMAINDER();
        OperatorExpression.OperatorSignatures[17] = get_RIGHT_SHIFT();
        OperatorExpression.OperatorSignatures[19] = get_UNSIGNED_RIGHT_SHIFT();
        OperatorExpression.OperatorSignatures[8] = get_XOR();
    }
    
    public static final String generateTableTestCase() {
        final int[] operators = { 2, 0, 9, 6, 7, 10, 4, 5, 13, 15, 3, 1, 14, 16, 17, 19, 8 };
        class Decode
        {
            public final String constant(final int code) {
                switch (code) {
                    case 5: {
                        return "true";
                    }
            ...
            public final String type(final int code) {
                switch (code) {
                    case 5: {
                        return "z";
                    }
            ...
            public final String operator(final int operator) {
                switch (operator) {
                    case 18: {
                        return "==";
                    }
        ...
        for (int i = 0; i < operators.length; ++i) {
            final int operator = operators[i];
            for (int left = 0; left < 16; ++left) {
                for (int right = 0; right < 16; ++right) {
                    final int result = OperatorExpression.OperatorSignatures[operator][(left << 4) + right] & 0xF;
                    if (result != 0) {
                        s = String.valueOf(s) + "\t\t" + decode.type(result) + "0" + " = " + decode.type(left);
                        s = String.valueOf(s) + " " + decode.operator(operator) + " " + decode.type(right) + ";\n";
                        final String begin = (result == 11) ? "\t\tif (! " : "\t\tif ( ";
                        final String test = (result == 11) ? ".equals(" : " != (";
    ...
    public static final int[] get_MINUS() {
        final int[] table = get_PLUS().clone();
        table[183] = (table[179] = 0);
        table[182] = (table[180] = 0);
        table[177] = (table[187] = 0);
        table[185] = (table[184] = 0);
        table[178] = (table[181] = 0);
        table[188] = (table[186] = 0);
        table[123] = (table[59] = 0);
        table[107] = (table[75] = 0);
        table[139] = (table[27] = 0);
        table[91] = (table[155] = 0);
        table[171] = (table[43] = 0);
        table[204] = (table[203] = 0);
        return table;
    }
    ...
    public abstract StringBuffer printExpressionNoParenthesis(final int p0, final StringBuffer p1);
}
