...
public class TypeParameter extends AbstractVariableDeclaration
{
    ...
    public void getAllAnnotationContexts(final int targetType, final int typeParameterIndex, final List allAnnotationContexts) {
        final TypeReference.AnnotationCollector collector = new TypeReference.AnnotationCollector(this, targetType, typeParameterIndex, allAnnotationContexts);
        ...
        if (this.type != null) {
            ...
            if ((this.type.bits & 0x100000) != 0x0) {
                collector.info2 = boundIndex;
                this.type.traverse(collector, (BlockScope)null);
            }
        }
        if (this.bounds != null) {
            for (int boundsLength = this.bounds.length, j = 0; j < boundsLength; ++j) {
                final TypeReference bound = this.bounds[j];
                if ((bound.bits & 0x100000) != 0x0) {
                    collector.info2 = ++boundIndex;
    ...
    private void internalResolve(final Scope scope, final boolean staticContext) {
        if (this.binding != null) {
            final Binding existingType = scope.parent.getBinding(this.name, 4, this, false);
    ...
    public void resolveAnnotations(final Scope scope) {
        final BlockScope resolutionScope = Scope.typeAnnotationsResolutionScope(scope);
        if (resolutionScope != null) {
            final AnnotationBinding[] annotationBindings = ASTNode.resolveAnnotations(resolutionScope, this.annotations, this.binding, false);
            final boolean isAnnotationBasedNullAnalysisEnabled = scope.environment().globalOptions.isAnnotationBasedNullAnalysisEnabled;
    ...
    public void traverse(final ASTVisitor visitor, final BlockScope scope) {
        if (visitor.visit(this, scope)) {
            ...
            if (this.bounds != null) {
                for (int boundsLength = this.bounds.length, i = 0; i < boundsLength; ++i) {
