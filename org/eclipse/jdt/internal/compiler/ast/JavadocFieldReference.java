...
public class JavadocFieldReference extends FieldReference
{
    ...
    public JavadocFieldReference(final char[] source, final long pos) {
        super(source, pos);
        this.bits |= 0x8000;
    }
    
    protected TypeBinding internalResolveType(final Scope scope) {
        ...
        Binding fieldBinding = (this.receiver != null && this.receiver.isThis()) ? scope.classScope().getBinding(this.token, this.bits & 0x7, this, true) : scope.getField(this.actualReceiverType, this.token, this);
        if (!fieldBinding.isValidBinding()) {
            switch (fieldBinding.problemId()) {
                case 5:
                case 6:
                case 7: {
                    final FieldBinding closestMatch = ((ProblemFieldBinding)fieldBinding).closestMatch;
        ...
        if (this.actualReceiverType instanceof ReferenceBinding) {
            final ReferenceBinding refBinding = (ReferenceBinding)this.actualReceiverType;
            final char[] selector = this.token;
            ...
            if (possibleMethod.isValidBinding()) {
                ...
                final ProblemMethodBinding problemMethodBinding = (ProblemMethodBinding)possibleMethod;
