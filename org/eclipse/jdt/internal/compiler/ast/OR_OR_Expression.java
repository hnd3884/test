...
public class OR_OR_Expression extends BinaryExpression
{
    ...
    public OR_OR_Expression(final Expression left, final Expression right, final int operator) {
    ...
    public FlowInfo analyseCode(final BlockScope currentScope, final FlowContext flowContext, final FlowInfo flowInfo) {
        ...
        if (isLeftOptimizedTrue && (rightInfo.reachMode() & 0x3) == 0x0) {
        ...
        final FlowInfo leftInfoWhenTrueForMerging = leftInfo.initsWhenTrue().unconditionalCopy().addPotentialInitializationsFrom(rightInfo.unconditionalInitsWithoutSideEffect());
        final FlowInfo mergedInfo2 = FlowInfo.conditional(leftInfoWhenTrueForMerging.unconditionalInits().mergedWith(rightInfo.safeInitsWhenTrue().setReachMode(previousMode).unconditionalInits()), rightInfo.initsWhenFalse());
    ...
    public void generateCode(final BlockScope currentScope, final CodeStream codeStream, final boolean valueRequired) {
        ...
        final BranchLabel trueLabel = new BranchLabel(codeStream);
