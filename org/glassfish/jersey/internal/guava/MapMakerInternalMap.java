...
class MapMakerInternalMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable
{
    ...
    private MapMakerInternalMap(final MapMaker builder) {
    ...
    private static <K, V> void connectExpirables(final ReferenceEntry<K, V> previous, final ReferenceEntry<K, V> next) {
        previous.setNextExpirable(next);
        next.setPreviousExpirable(previous);
    }
    
    private static <K, V> void nullifyExpirable(final ReferenceEntry<K, V> nulled) {
        final ReferenceEntry<K, V> nullEntry = nullEntry();
        nulled.setNextExpirable(nullEntry);
        nulled.setPreviousExpirable(nullEntry);
    }
    
    private static <K, V> void connectEvictables(final ReferenceEntry<K, V> previous, final ReferenceEntry<K, V> next) {
        previous.setNextEvictable(next);
        next.setPreviousEvictable(previous);
    }
    
    private static <K, V> void nullifyEvictable(final ReferenceEntry<K, V> nulled) {
        final ReferenceEntry<K, V> nullEntry = nullEntry();
    ...
    private Segment<K, V> createSegment(final int initialCapacity, final int maxSegmentSize) {
        return new Segment<K, V>(this, initialCapacity, maxSegmentSize);
    }
    
    private V getLiveValue(final ReferenceEntry<K, V> entry) {
    ...
    enum EntryFactory
    {
        ...
        abstract <K, V> ReferenceEntry<K, V> newEntry(final Segment<K, V> p0, final K p1, final int p2, final ReferenceEntry<K, V> p3);
         ...
         <K, V> void copyExpirableEntry(final ReferenceEntry<K, V> original, final ReferenceEntry<K, V> newEntry) {
            ...
            connectExpirables(original.getPreviousExpirable(), (ReferenceEntry<Object, Object>)newEntry);
            connectExpirables(newEntry, (ReferenceEntry<Object, Object>)original.getNextExpirable());
            nullifyExpirable((ReferenceEntry<Object, Object>)original);
        }
        
         <K, V> void copyEvictableEntry(final ReferenceEntry<K, V> original, final ReferenceEntry<K, V> newEntry) {
            connectEvictables(original.getPreviousEvictable(), (ReferenceEntry<Object, Object>)newEntry);
            connectEvictables(newEntry, (ReferenceEntry<Object, Object>)original.getNextEvictable());
            nullifyEvictable((ReferenceEntry<Object, Object>)original);
        }
    ...
    static final class StrongExpirableEntry<K, V> extends StrongEntry<K, V> implements ReferenceEntry<K, V>
    {
        ...
        StrongExpirableEntry(final K key, final int hash, final ReferenceEntry<K, V> next) {
            super(key, hash, next);
            this.time = Long.MAX_VALUE;
            this.nextExpirable = (ReferenceEntry<K, V>)nullEntry();
            this.previousExpirable = (ReferenceEntry<K, V>)nullEntry();
        }
    ...
    static final class StrongEvictableEntry<K, V> extends StrongEntry<K, V> implements ReferenceEntry<K, V>
    {
        ...
        StrongEvictableEntry(final K key, final int hash, final ReferenceEntry<K, V> next) {
            super(key, hash, next);
            this.nextEvictable = (ReferenceEntry<K, V>)nullEntry();
            this.previousEvictable = (ReferenceEntry<K, V>)nullEntry();
        }
    ...
    static final class StrongExpirableEvictableEntry<K, V> extends StrongEntry<K, V> implements ReferenceEntry<K, V>
    {
        ...
        StrongExpirableEvictableEntry(final K key, final int hash, final ReferenceEntry<K, V> next) {
            super(key, hash, next);
            this.time = Long.MAX_VALUE;
            this.nextExpirable = (ReferenceEntry<K, V>)nullEntry();
            this.previousExpirable = (ReferenceEntry<K, V>)nullEntry();
    ...
    static final class WeakExpirableEntry<K, V> extends WeakEntry<K, V> implements ReferenceEntry<K, V>
    {
        ...
        WeakExpirableEntry(final ReferenceQueue<K> queue, final K key, final int hash, final ReferenceEntry<K, V> next) {
    ...
    static final class WeakEvictableEntry<K, V> extends WeakEntry<K, V> implements ReferenceEntry<K, V>
    {
        ...
        WeakEvictableEntry(final ReferenceQueue<K> queue, final K key, final int hash, final ReferenceEntry<K, V> next) {
            super(queue, key, hash, next);
            this.nextEvictable = (ReferenceEntry<K, V>)nullEntry();
            this.previousEvictable = (ReferenceEntry<K, V>)nullEntry();
        }
    ...
    static final class WeakExpirableEvictableEntry<K, V> extends WeakEntry<K, V> implements ReferenceEntry<K, V>
    {
        ...
        WeakExpirableEvictableEntry(final ReferenceQueue<K> queue, final K key, final int hash, final ReferenceEntry<K, V> next) {
            super(queue, key, hash, next);
            this.time = Long.MAX_VALUE;
            this.nextExpirable = (ReferenceEntry<K, V>)nullEntry();
            this.previousExpirable = (ReferenceEntry<K, V>)nullEntry();
    ...
    static class Segment<K, V> extends ReentrantLock
    {
        ...
        Segment(final MapMakerInternalMap<K, V> map, final int initialCapacity, final int maxSegmentSize) {
            ...
            this.recencyQueue = (Queue<ReferenceEntry<K, V>>)((map.evictsBySize() || map.expiresAfterAccess()) ? new ConcurrentLinkedQueue<ReferenceEntry<K, V>>() : discardingQueue());
            this.evictionQueue = (Queue<ReferenceEntry<K, V>>)(map.evictsBySize() ? new EvictionQueue() : discardingQueue());
            this.expirationQueue = (Queue<ReferenceEntry<K, V>>)(map.expires() ? new ExpirationQueue() : discardingQueue());
        }
        ...
        ReferenceEntry<K, V> newEntry(final K key, final int hash, final ReferenceEntry<K, V> next) {
            return ((MapMakerInternalMap<Object, Object>)this.map).entryFactory.newEntry(this, key, hash, next);
        }
        ...
        ReferenceEntry<K, V> copyEntry(final ReferenceEntry<K, V> original, final ReferenceEntry<K, V> newNext) {
            ...
            final ReferenceEntry<K, V> newEntry = ((MapMakerInternalMap<Object, Object>)this.map).entryFactory.copyEntry(this, original, newNext);
            newEntry.setValueReference(valueReference.copyFor(this.valueReferenceQueue, value, newEntry));
            return newEntry;
        }
        
        void setValue(final ReferenceEntry<K, V> entry, final V value) {
            final ValueReference<K, V> valueReference = ((MapMakerInternalMap<Object, Object>)this.map).valueStrength.referenceValue(this, entry, value);
        ...
        void recordRead(final ReferenceEntry<K, V> entry) {
            if (this.map.expiresAfterAccess()) {
                this.recordExpirationTime(entry, ((MapMakerInternalMap<Object, Object>)this.map).expireAfterAccessNanos);
            }
        ...
        void recordWrite(final ReferenceEntry<K, V> entry) {
            ...
            this.evictionQueue.add(entry);
            if (this.map.expires()) {
                final long expiration = this.map.expiresAfterAccess() ? ((MapMakerInternalMap<Object, Object>)this.map).expireAfterAccessNanos : ((MapMakerInternalMap<Object, Object>)this.map).expireAfterWriteNanos;
        ...
        void recordExpirationTime(final ReferenceEntry<K, V> entry, final long expirationNanos) {
            entry.setExpirationTime(((MapMakerInternalMap<Object, Object>)this.map).ticker.read() + expirationNanos);
        }
        ...
        void expireEntries() {
            ...
            final long now = ((MapMakerInternalMap<Object, Object>)this.map).ticker.read();
            ReferenceEntry<K, V> e;
        ...
        void enqueueNotification(final K key, final int hash, final V value, final MapMaker.RemovalCause cause) {
            if (((MapMakerInternalMap<Object, Object>)this.map).removalNotificationQueue != MapMakerInternalMap.DISCARDING_QUEUE) {
                final MapMaker.RemovalNotification<K, V> notification = new MapMaker.RemovalNotification<K, V>(key, value, cause);
                ((MapMakerInternalMap<Object, Object>)this.map).removalNotificationQueue.offer(notification);
            }
        }
            ...
            if (this.count != 0) {
                for (ReferenceEntry<K, V> e = this.getFirst(hash); e != null; e = e.getNext()) {
                    if (e.getHash() == hash) {
                        ...
                        else if (((MapMakerInternalMap<Object, Object>)this.map).keyEquivalence.equivalent(key, entryKey)) {
                            return e;
                        }
                    }
                }
        ...
        V put(final K key, final int hash, final V value, final boolean onlyIfAbsent) {
            ...
            try {
                ...
                while (e != null) {
                    final K entryKey = e.getKey();
                    if (e.getHash() == hash && entryKey != null && ((MapMakerInternalMap<Object, Object>)this.map).keyEquivalence.equivalent(key, entryKey)) {
            ...
            try {
                ...
                while (e != null) {
                    ...
                    if (e.getHash() == hash && entryKey != null && ((MapMakerInternalMap<Object, Object>)this.map).keyEquivalence.equivalent(key, entryKey)) {
                        ...
                        if (((MapMakerInternalMap<Object, Object>)this.map).valueEquivalence.equivalent(oldValue, entryValue)) {
                            ++this.modCount;
                            this.enqueueNotification(key, hash, entryValue, MapMaker.RemovalCause.REPLACED);
            ...
            try {
                ...
                for (ReferenceEntry<K, V> first = e = table.get(index); e != null; e = e.getNext()) {
                    ...
                    if (e.getHash() == hash && entryKey != null && ((MapMakerInternalMap<Object, Object>)this.map).keyEquivalence.equivalent(key, entryKey)) {
                        ...
                        final V entryValue = valueReference.get();
                        MapMaker.RemovalCause cause;
                        if (((MapMakerInternalMap<Object, Object>)this.map).valueEquivalence.equivalent(value, entryValue)) {
                            cause = MapMaker.RemovalCause.EXPLICIT;
                        }
                        else {
                            if (!this.isCollected(valueReference)) {
    ...
    static final class EvictionQueue<K, V> extends AbstractQueue<ReferenceEntry<K, V>>
    {
        ...
        public boolean offer(final ReferenceEntry<K, V> entry) {
            connectEvictables(entry.getPreviousEvictable(), (ReferenceEntry<Object, Object>)entry.getNextEvictable());
            connectEvictables(this.head.getPreviousEvictable(), (ReferenceEntry<Object, Object>)entry);
            connectEvictables(entry, (ReferenceEntry<Object, Object>)this.head);
            return true;
        }
        ...
        public boolean remove(final Object o) {
            ...
            connectEvictables(previous, (ReferenceEntry<Object, Object>)next);
            nullifyEvictable((ReferenceEntry<Object, Object>)e);
    ...
    static final class ExpirationQueue<K, V> extends AbstractQueue<ReferenceEntry<K, V>>
    {
        ...
        @Override
        public boolean offer(final ReferenceEntry<K, V> entry) {
            connectExpirables(entry.getPreviousExpirable(), (ReferenceEntry<Object, Object>)entry.getNextExpirable());
            connectExpirables(this.head.getPreviousExpirable(), (ReferenceEntry<Object, Object>)entry);
            connectExpirables(entry, (ReferenceEntry<Object, Object>)this.head);
            return true;
        }
        ...
        public boolean remove(final Object o) {
            ...
            connectExpirables(previous, (ReferenceEntry<Object, Object>)next);
            nullifyExpirable((ReferenceEntry<Object, Object>)e);
            return next != NullEntry.INSTANCE;
        }
    ...
    abstract class HashIterator<E> implements Iterator<E>
    {
        ...
        boolean advanceTo(final ReferenceEntry<K, V> entry) {
            try {
                ...
                final V value = (V)MapMakerInternalMap.this.getLiveValue(entry);
                if (value != null) {
