...
public class Cache<K, V> implements Computable<K, V>
{
    ...
    public Cache(final Computable<K, V> computable) {
        this((Computable<Object, V>)computable, Cache.EMPTY_HANDLER);
    }
    
    public Cache(final Computable<K, V> computable, final CycleHandler<K> cycleHandler) {
    ...
    public V compute(final K key) {
        ...
        if (f == null) {
            final OriginThreadAwareFuture ft = new OriginThreadAwareFuture(this, key);
    ...
    static {
        EMPTY_HANDLER = new CycleHandler<Object>() {
            @Override
    ...
    private class OriginThreadAwareFuture implements Future<V>
    {
        ...
        OriginThreadAwareFuture(final Cache<K, V> cache, final K key) {
            this.threadId = Thread.currentThread().getId();
            final Callable<V> eval = new Callable<V>() {
                @Override
                public V call() throws Exception {
                    try {
                        return Cache.this.computable.compute(key);
                    }
                    finally {
