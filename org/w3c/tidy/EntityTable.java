...
public final class EntityTable
{
    ...
    private Map entityHashtable;
    
    private EntityTable() {
        this.entityHashtable = new Hashtable();
    }
    
    private Entity install(final Entity entity) {
        return this.entityHashtable.put(entity.getName(), entity);
    }
    
    public Entity lookup(final String s) {
        return this.entityHashtable.get(s);
    }
    
    public int entityCode(final String s) {
        if (s.length() <= 1) {
            return 0;
        }
        if (s.charAt(1) == '#') {
            int n = 0;
            try {
                if (s.length() >= 4 && s.charAt(2) == 'x') {
                    n = Integer.parseInt(s.substring(3), 16);
                }
                else if (s.length() >= 3) {
        ...
        final Entity lookup = this.lookup(s.substring(1));
        if (lookup != null) {
    ...
    public String entityName(final short n) {
        String name = null;
        final Iterator iterator = this.entityHashtable.values().iterator();
