...
public class CreateSignedMail
{
    ...
    static AuthorityKeyIdentifier createAuthorityKeyId(final PublicKey publicKey) throws IOException {
        return new AuthorityKeyIdentifier(SubjectPublicKeyInfo.getInstance((Object)publicKey.getEncoded()));
    }
    ...
    static X509Certificate makeCertificate(final KeyPair keyPair, final String s, final KeyPair keyPair2, final String s2) throws GeneralSecurityException, IOException, OperatorCreationException {
        ...
        final JcaX509v3CertificateBuilder jcaX509v3CertificateBuilder = new JcaX509v3CertificateBuilder(new X500Name(s2), BigInteger.valueOf(CreateSignedMail.serialNo++), new Date(System.currentTimeMillis()), new Date(System.currentTimeMillis() + 8640000000L), new X500Name(s), public1);
        ((X509v3CertificateBuilder)jcaX509v3CertificateBuilder).addExtension(X509Extension.subjectKeyIdentifier, false, (ASN1Encodable)createSubjectKeyId(public1));
        ((X509v3CertificateBuilder)jcaX509v3CertificateBuilder).addExtension(X509Extension.authorityKeyIdentifier, false, (ASN1Encodable)createAuthorityKeyId(public2));
