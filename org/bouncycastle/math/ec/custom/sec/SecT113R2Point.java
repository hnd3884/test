...
{
    @Deprecated
    public SecT113R2Point(final ECCurve ecCurve, final ECFieldElement ecFieldElement, final ECFieldElement ecFieldElement2) {
        this(ecCurve, ecFieldElement, ecFieldElement2, false);
    }
    
    @Deprecated
    public SecT113R2Point(final ECCurve ecCurve, final ECFieldElement ecFieldElement, final ECFieldElement ecFieldElement2, final boolean withCompression) {
    ...
    SecT113R2Point(final ECCurve ecCurve, final ECFieldElement ecFieldElement, final ECFieldElement ecFieldElement2, final ECFieldElement[] array, final boolean withCompression) {
        ...
        if (x.isZero()) {
            ...
            if (!add2.isZero()) {
                ...
                if (rawXCoord.isZero()) {
                    ...
                    ecFieldElement2 = divide.square().add(divide).add(xCoord).add(curve.getA());
                ...
                return new SecT113R2Point(curve, ecFieldElement2, ecFieldElement3, new ECFieldElement[] { ecFieldElement4 }, this.withCompression);
            }
            if (add.isZero()) {
                return this.twice();
            }
    ...
    public ECPoint twice() {
        ...
        final ECFieldElement add = y.square().add(ecFieldElement2).add(one ? a : a.multiply(ecFieldElement3));
        if (add.isZero()) {
        ...
        return new SecT113R2Point(curve, square, (one ? x : x.multiply(ecFieldElement)).squarePlusProduct(add, ecFieldElement2).add(square).add(ecFieldElement4), new ECFieldElement[] { ecFieldElement4 }, this.withCompression);
    }
    ...
    public ECPoint negate() {
        ...
        final ECFieldElement y = this.y;
        final ECFieldElement ecFieldElement = this.zs[0];
        return new SecT113R2Point(this.curve, x, y.add(ecFieldElement), new ECFieldElement[] { ecFieldElement }, this.withCompression);
    }
}
