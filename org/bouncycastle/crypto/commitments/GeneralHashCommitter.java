...
public class GeneralHashCommitter implements Committer
{
    ...
    public GeneralHashCommitter(final ExtendedDigest digest, final SecureRandom random) {
        this.digest = digest;
    ...
    private byte[] calculateCommitment(final byte[] array, final byte[] array2) {
        ...
        this.digest.update((byte)(array2.length >>> 8));
        this.digest.update((byte)array2.length);
