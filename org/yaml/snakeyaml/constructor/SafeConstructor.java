...
public class SafeConstructor extends BaseConstructor
{
    ...
    public SafeConstructor(final LoaderOptions loadingConfig) {
        super(loadingConfig);
    ...
    protected void processDuplicateKeys(final MappingNode node) {
        ...
        final Iterator<Integer> indices2remove = toRemove.descendingIterator();
        while (indices2remove.hasNext()) {
            nodeValue.remove((int)indices2remove.next());
        }
    }
    ...
    private Number createNumber(final int sign, String number, final int radix) {
        ...
        final int[] maxArr = (int[])((radix < SafeConstructor.RADIX_MAX.length) ? SafeConstructor.RADIX_MAX[radix] : null);
        if (maxArr != null) {
    ...
    static {
        ...
        final int[] arr$;
        final int[] radixList = arr$ = new int[] { 2, 8, 10, 16 };
        for (final int radix : arr$) {
    ...
    public class ConstructYamlBool extends AbstractConstruct
    {
        @Override
        public Object construct(final Node node) {
            final String val = SafeConstructor.this.constructScalar((ScalarNode)node);
