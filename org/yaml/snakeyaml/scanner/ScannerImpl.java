...
public final class ScannerImpl implements Scanner
{
    ...
    public ScannerImpl setEmitComments(final boolean emitComments) {
    ...
    private CommentToken scanDirectiveIgnoredLine(final Mark startMark) {
        ...
        if (this.reader.peek() == 35) {
            final Mark commentStartMark = this.reader.getMark();
            int length;
            for (length = 0; Constant.NULL_OR_LINEBR.hasNo(this.reader.peek(length)); ++length) {}
            final String comment = this.reader.prefixForward(length);
            if (this.emitComments) {
                final Mark commentEndMark = this.reader.getMark();
    ...
    private List<Token> makeTokenList(final Token... tokens) {
        ...
        for (int ix = 0; ix < tokens.length; ++ix) {
            if (tokens[ix] != null) {
                if (this.emitComments || !(tokens[ix] instanceof CommentToken)) {
