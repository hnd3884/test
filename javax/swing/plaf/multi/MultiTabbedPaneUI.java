...
public class MultiTabbedPaneUI extends TabbedPaneUI
{
    ...
    public int tabForCoordinate(final JTabbedPane tabbedPane, final int n, final int n2) {
        final int tabForCoordinate = this.uis.elementAt(0).tabForCoordinate(tabbedPane, n, n2);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((TabbedPaneUI)this.uis.elementAt(i)).tabForCoordinate(tabbedPane, n, n2);
        }
    ...
    public Rectangle getTabBounds(final JTabbedPane tabbedPane, final int n) {
        final Rectangle tabBounds = this.uis.elementAt(0).getTabBounds(tabbedPane, n);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((TabbedPaneUI)this.uis.elementAt(i)).getTabBounds(tabbedPane, n);
        }
        return tabBounds;
    }
    ...
    public int getTabRunCount(final JTabbedPane tabbedPane) {
        final int tabRunCount = this.uis.elementAt(0).getTabRunCount(tabbedPane);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((TabbedPaneUI)this.uis.elementAt(i)).getTabRunCount(tabbedPane);
        }
        return tabRunCount;
    }
    ...
    public static ComponentUI createUI(final JComponent component) {
        final MultiTabbedPaneUI multiTabbedPaneUI = new MultiTabbedPaneUI();
        return MultiLookAndFeel.createUIs(multiTabbedPaneUI, multiTabbedPaneUI.uis, component);
    }
