...
public class MultiOptionPaneUI extends OptionPaneUI
{
    ...
    public void selectInitialValue(final JOptionPane optionPane) {
        for (int i = 0; i < this.uis.size(); ++i) {
            ((OptionPaneUI)this.uis.elementAt(i)).selectInitialValue(optionPane);
        }
    }
    
    @Override
    public boolean containsCustomComponents(final JOptionPane optionPane) {
        final boolean containsCustomComponents = this.uis.elementAt(0).containsCustomComponents(optionPane);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((OptionPaneUI)this.uis.elementAt(i)).containsCustomComponents(optionPane);
        }
        return containsCustomComponents;
    }
    ...
    public static ComponentUI createUI(final JComponent component) {
        final MultiOptionPaneUI multiOptionPaneUI = new MultiOptionPaneUI();
        return MultiLookAndFeel.createUIs(multiOptionPaneUI, multiOptionPaneUI.uis, component);
    }
