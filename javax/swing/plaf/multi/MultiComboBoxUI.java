...
public class MultiComboBoxUI extends ComboBoxUI
{
    ...
    public boolean isFocusTraversable(final JComboBox comboBox) {
        final boolean focusTraversable = this.uis.elementAt(0).isFocusTraversable(comboBox);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((ComboBoxUI)this.uis.elementAt(i)).isFocusTraversable(comboBox);
        }
    ...
    public void setPopupVisible(final JComboBox comboBox, final boolean b) {
        for (int i = 0; i < this.uis.size(); ++i) {
            ((ComboBoxUI)this.uis.elementAt(i)).setPopupVisible(comboBox, b);
        }
    }
    
    @Override
    public boolean isPopupVisible(final JComboBox comboBox) {
        final boolean popupVisible = this.uis.elementAt(0).isPopupVisible(comboBox);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((ComboBoxUI)this.uis.elementAt(i)).isPopupVisible(comboBox);
        }
        return popupVisible;
    }
    ...
    public static ComponentUI createUI(final JComponent component) {
        final MultiComboBoxUI multiComboBoxUI = new MultiComboBoxUI();
        return MultiLookAndFeel.createUIs(multiComboBoxUI, multiComboBoxUI.uis, component);
    }
