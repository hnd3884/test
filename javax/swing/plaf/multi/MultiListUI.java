...
public class MultiListUI extends ListUI
{
    ...
    public int locationToIndex(final JList list, final Point point) {
        final int locationToIndex = this.uis.elementAt(0).locationToIndex(list, point);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((ListUI)this.uis.elementAt(i)).locationToIndex(list, point);
        }
    ...
    public Point indexToLocation(final JList list, final int n) {
        final Point indexToLocation = this.uis.elementAt(0).indexToLocation(list, n);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((ListUI)this.uis.elementAt(i)).indexToLocation(list, n);
        }
    ...
    public Rectangle getCellBounds(final JList list, final int n, final int n2) {
        final Rectangle cellBounds = this.uis.elementAt(0).getCellBounds(list, n, n2);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((ListUI)this.uis.elementAt(i)).getCellBounds(list, n, n2);
        }
        return cellBounds;
    }
    ...
    public static ComponentUI createUI(final JComponent component) {
        final MultiListUI multiListUI = new MultiListUI();
        return MultiLookAndFeel.createUIs(multiListUI, multiListUI.uis, component);
    }
