...
public class MultiPopupMenuUI extends PopupMenuUI
{
    ...
    public boolean isPopupTrigger(final MouseEvent mouseEvent) {
        final boolean popupTrigger = this.uis.elementAt(0).isPopupTrigger(mouseEvent);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((PopupMenuUI)this.uis.elementAt(i)).isPopupTrigger(mouseEvent);
        }
        return popupTrigger;
    }
    ...
    public Popup getPopup(final JPopupMenu popupMenu, final int n, final int n2) {
        final Popup popup = this.uis.elementAt(0).getPopup(popupMenu, n, n2);
        for (int i = 1; i < this.uis.size(); ++i) {
            ((PopupMenuUI)this.uis.elementAt(i)).getPopup(popupMenu, n, n2);
        }
        return popup;
    }
    ...
    public static ComponentUI createUI(final JComponent component) {
        final MultiPopupMenuUI multiPopupMenuUI = new MultiPopupMenuUI();
        return MultiLookAndFeel.createUIs(multiPopupMenuUI, multiPopupMenuUI.uis, component);
    }
